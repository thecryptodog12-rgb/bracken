...
1. NEVER hand-edit the synced files (`circuits/lib/src/configs/committee/active.nr`,
   `circuits/bin/.active-preset.json`, `packages/bracken-contracts/scripts/utils.ts`, parity
...
2. Run `pnpm build:circuits --committee <name> [--preset <preset>]`. This requires `nargo` and `bb`
   on PATH (`bracken noir setup` installs a matching toolchain) — if they're missing, stop and tell
