...
CLAUDE.md, .cursor/rules/loxley.mdc, .clinerules, .windsurfrules, etc.) should reference this file
...
- `packages/` — TypeScript: Solidity contracts (`loxley-contracts`), SDK, React, MCP server
...
| `packages/loxley-contracts/scripts/utils.ts` (`BFV_DKG_H`) | regenerated by `scripts/build-circuits.ts` |
...
lifecycle of the Loxley protocol — from node registration through DKG, computation, decryption,
...
| LOXLEY bonding, tLOXLEY/USDC tickets, activation thresholds, exit queue                    | `02_TOKENS_AND_ACTIVATION.md`       |
