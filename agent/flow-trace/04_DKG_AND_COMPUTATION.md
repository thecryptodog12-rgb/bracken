...
│         │  matching current production `Bracken` deployment config)
...
│   │         failure instead of only surfacing as BrackenError telemetry
│   │
...
└─ CiphernodeRegistrySolWriter receives PublicKeyAggregated:
  ...
  └─ Calls contract.publishCommitteePublicKey(e3_id, publicKey) after the
     commitment is available, including after restart
        ...
        │  │    7. bracken.onCommitteePublished(e3Id, pkCommitment) │
        │  │       │                                             │
        │  │       │  ┌─ Bracken.sol ────────────────────────┐  │
...
same `pkCommitment` and use `BrackenSDK.validatePublicKeyCommitment()` before accepting the bytes;
...
`Bracken` recomputes `decryptionDomain` over
`(chainId, Bracken address, e3Id, committeeHash, ciphertextOutputHash, committeePublicKey)`. The
...
The RISC Zero guest commits nine 32-byte fields in this order: chain ID, Bracken address, E3 ID,
...
(`Bracken.publishCiphertextOutput` has no authorization modifier) and one-shot, so any party could do
...
```
BrackenSolReader decodes CiphertextOutputPublished event
│
...
│       }
│
└─ BrackenSolWriter receives PlaintextAggregated:
  ├─ Requires EffectsEnabled
  ├─ Requires active_aggregators[e3_id] == true
  ...
  └─ Calls contract.publishPlaintextOutput(e3Id, output, proof)
        ...
        │  │  //   - bracken.claimReward(e3Id) or                │
        │  │  //     bracken.claimRewards(e3Ids[])               │
        │  │  //   - bracken.treasuryClaim(token)                │
