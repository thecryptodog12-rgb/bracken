...
| `e3-evm`        | parser/router/hub, chain gateway, readers, registry/bracken/slashing writers, log fetcher         | Each chain capability owns its actor plus events/workflow/effects. `log_fetching` is explicitly an adapter capability; provider and transaction work is no longer hidden below actor `runtime/`.                                                       |
