# Bracken TypeScript SDK

A powerful, type-safe TypeScript SDK for interacting with Bracken smart contracts. This SDK provides
...
- **React integration**: Includes React hooks for easy frontend integration (via `@bracken/react`)
...
```bash
pnpm add @bracken/sdk
```

## Quick Start

```typescript
import { BrackenSDK, BrackenEventType, RegistryEventType } from '@bracken/sdk'
...
// Create SDK instance
const sdk = new BrackenSDK({
  publicClient,
  walletClient,
  contracts: {
    bracken: '0x...', // Your Bracken contract address
...
sdk.onBrackenEvent(BrackenEventType.E3_REQUESTED, (event) => {
  console.log('E3 Requested:', event.data)
})

sdk.onBrackenEvent(RegistryEventType.COMMITTEE_REQUESTED, (event) => {
...
For a simpler setup (especially on the server), use the static `BrackenSDK.create()` factory:

```typescript
import { BrackenSDK } from '@bracken/sdk'
import { sepolia } from 'viem/chains'
...
- Exclude the `@bracken/wasm` package from bundling optimization.
...
```typescript
interface BrackenEvent<T extends AllEventTypes> {
  type: T
...
The SDK includes a React hook via the `@bracken/react` package:

```bash
pnpm add @bracken/react
```
...
function MyComponent() {
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      onBrackenEvent(BrackenEventType.E3_REQUESTED, handler)
      return () => off(BrackenEventType.E3_REQUESTED, handler)
    }
  }, [isInitialized])
...
import { generatePublicKey, encryptNumber } from '@bracken/sdk/crypto'

// Contract client and types
import { ContractClient } from '@bracken/sdk/contracts'
import type { ContractAddresses, E3 } from '@bracken/sdk/contracts'

// Event listener and types
import { EventListener, BrackenEventType, RegistryEventType } from '@bracken/sdk/events'
```

All sub-module exports are also re-exported from the main `@bracken/sdk` entry point for convenience.
...
```typescript
sdk.onBrackenEvent(eventType: AllEventTypes, callback: EventCallback);
...
await sdk.onBrackenEvent(RegistryEventType.COMMITTEE_PUBLISHED, async (event) => {
...
on-chain `paramSet` index registered in the Bracken contract:
...
```typescript
import { SDKError } from '@bracken/sdk'
...
- **BrackenSDK** (`bracken-sdk.ts`): Main orchestrator class that delegates to sub-modules
