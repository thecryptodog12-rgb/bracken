...
import { CiphernodeRegistryOwnable__factory, Bracken__factory, BrackenToken__factory } from '@bracken/contracts/types'
...
export class ContractClient {
  ...
  private contractInfo: {
    bracken: { address: `0x${string}`; abi: Abi }
  ...
  constructor(config: ContractClientConfig) {
    ...
    if (!isValidAddress(contracts.bracken)) {
      throw new SDKError('Invalid Bracken contract address', 'INVALID_ADDRESS')
    }
  ...
  public async approveFeeToken(amount: bigint): Promise<Hash> {
    ...
    try {
      ...
      const { request } = await this.publicClient.simulateContract({
        ...
        args: [this.contracts.bracken, amount],
        account,
      })
  ...
  public async requestE3(params: E3RequestParams): Promise<Hash> {
    ...
    try {
      ...
      const expectedCryptoConfigId = await this.publicClient.readContract({
        address: this.contracts.bracken,
        abi: Bracken__factory.abi,
