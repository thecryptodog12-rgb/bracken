...
import { CiphernodeRegistryOwnable__factory, Bracken__factory } from '@bracken/contracts/types'

import {
  RegistryEventType,
  type AllEventTypes,
  type BrackenEvent,
  type BrackenEventData,
  type BrackenEventType as BrackenEventTypeT,
  type EventCallback,
...
export class EventListener implements SDKEventEmitter {
  ...
  // Registry-exclusive event names that don't collide with BrackenEventType.
  ...
  // runtime; those default to the Bracken contract.
  ...
  private resolveContract(eventType: AllEventTypes): { address: `0x${string}`; abi: Abi } {
    ...
    return {
      address: isRegistryEvent ? this.contracts.ciphernodeRegistry : this.contracts.bracken,
      abi: isRegistryEvent ? CiphernodeRegistryOwnable__factory.abi : Bracken__factory.abi,
    }
  }

  public async onBrackenEvent<T extends AllEventTypes>(eventType: T, callback: EventCallback<T>): Promise<void> {
  ...
  public async once<T extends AllEventTypes>(type: T, callback: EventCallback<T>): Promise<void> {
    ...
    return this.onBrackenEvent(type, handler)
  }
        ...
        const unwatch = this.publicClient.watchContractEvent({
          ...
          onLogs(logs: Log[]) {
            for (let i = 0; i < logs.length; i++) {
              const log = logs[i]
              if (!log) break
              const event: BrackenEvent<T> = {
                type: eventType,
                data: (log as unknown as { args: unknown }).args as T extends BrackenEventTypeT
                  ? BrackenEventData[T]
  ...
  public emit<T extends AllEventTypes>(event: BrackenEvent<T>): void {
