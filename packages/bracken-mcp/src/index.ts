...
const STATIC_DOC_PAGES: DocPage[] = [
  ...
  { slug: 'building-with-bracken', title: 'Building with Bracken', url: '/building-with-bracken' },
  ...
  { slug: 'sdk', title: 'Bracken SDK', url: '/sdk' },
...
server.registerResource('docs-index', 'docs://index', { description: 'Index of all Bracken documentation pages' }, async () => ({
  contents: [
    {
      uri: 'docs://index',
...
server.registerTool(
  'read_doc',
  {
    description: 'Fetch and read a specific Bracken documentation page by slug',
...
server.registerTool(
  'search_docs',
  {
    description: 'Search for a keyword or phrase across all Bracken documentation pages',
...
server.registerTool('list_docs', { description: 'List all available Bracken documentation pages' }, async () => {
  const list = DOC_PAGES.map((p) => `- **${p.title}** → slug: \`${p.slug}\``).join('\n')
  return { content: [{ type: 'text', text: `# Bracken Documentation Pages\n\n${list}` }] }
})
