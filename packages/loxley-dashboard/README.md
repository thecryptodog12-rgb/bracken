...
Loxley / CRISP public observation dashboard. Three tabs:
...
```bash
pnpm install
pnpm --filter @loxley/dashboard dev
```
...
The dashboard reads live data from the Sepolia deployment of Loxley
(`packages/loxley-contracts/deployed_contracts.json`). ABIs come from the canonical typechain
factories in `@loxley/contracts/types` so they cannot drift from the deployed contracts. The
`E3Stage` enum is mirrored locally in `src/lib/chain.ts` (matching `ILoxley.E3Stage`).
...
- `CRISPProgram` — emits `InputPublished` for every ballot. (Loxley's own `InputPublished` is
...
- `VITE_LOXLEY_ADDRESS`, `VITE_CIPHERNODE_REGISTRY_ADDRESS`, `VITE_CRISP_PROGRAM_ADDRESS` —
  contracts.
...
- `VITE_DEPLOY_BLOCK` — first block to scan from (the Loxley deploy block).
...
```bash
pnpm --filter @loxley/dashboard build       # vite build → dist/
pnpm --filter @loxley/dashboard typecheck   # tsc --noEmit
pnpm --filter @loxley/dashboard preview     # serve dist/
```
...
1. New Project → import this repo → set **Root Directory** to `packages/loxley-dashboard`.
2. `vercel.json` (committed here) drives the rest:
   - installs the whole pnpm workspace (`cd ../.. && pnpm install`),
   - builds `@loxley/contracts` first (typechain ABIs the dashboard imports), then the dashboard,
   - serves `dist/`.
...
The dashboard intentionally has **no dependency on `@loxley/sdk`** (which needs a Rust/Noir
toolchain to build) — only `@loxley/contracts`, which is plain `hardhat compile` + `tsc`.
