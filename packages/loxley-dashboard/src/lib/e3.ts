...
import { CONTRACTS, DEPLOY_BLOCK, E3Stage, TIMEOUTS, ciphernodeRegistryAbi, loxleyAbi, publicClient } from './chain'
...
// CRISP votes are NOT published through Loxley (its ILoxley.InputPublished
...
// Other E3s on the same Loxley deployment run different programs and must not
...
export type E3FullDetails = E3Summary & {
  ...
  // amount currently held for the E3 — note Loxley zeroes it on settlement or
...
export async function fetchE3List(opts: FetchE3Opts = {}): Promise<E3Summary[]> {
  ...
  const logs = await getLogsChunked<any>(
    {
      address: CONTRACTS.Loxley,
      event: LOXLEY_E3_REQUESTED,
    },
    DEPLOY_BLOCK,
    head,
  )
...
export async function fetchE3Details(e3Id: bigint, toBlock?: bigint): Promise<E3FullDetails> {
  ...
  // registry; the key-publish moment from the Loxley E3StageChanged → KeyPublished
  ...
  // Inputs come from the E3 program, not Loxley. We only understand CRISP's
  ...
  const [inputs, results, rewards] = await Promise.all([
    ...
    getLogsChunked<any>(
      {
        address: CONTRACTS.Loxley,
        event: LOXLEY_PLAINTEXT_PUBLISHED,
        args: { e3Id },
      } as any,
      fromBlock,
      head,
    ...
    getLogsChunked<any>(
      {
        address: CONTRACTS.Loxley,
        event: LOXLEY_REWARDS_DISTRIBUTED,
        args: { e3Id },
      } as any,
      fromBlock,
      head,
