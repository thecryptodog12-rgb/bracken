...
//
// Stage numbers are the real ILoxley.E3Stage enum values (see lib/chain.ts), so
// the picture cannot drift from the contract without the enum drifting too.
  ...
  return (
    <figure className='lifecycle'>
      <figcaption className='lifecycle__cap'>
        <span className='section__eyebrow'>How an E3 runs</span>
        <p>
          ...
          — not to a ciphernode, not to the requester, not to whoever is running this dashboard.
        </p>
      </figcaption>
      ...
      <svg className='lifecycle__svg' viewBox={`0 0 ${W} ${H}`} role='img' aria-labelledby='lc-title lc-desc'>
        ...
        {STEPS.slice(0, -1).map((s, i) => {
          ...
          const isReveal = next.stage === E3Stage.Complete
          return (
            <path
              key={s.stage}
      ...
      <p className='lifecycle__foot'>
        There is a sixth value in the enum — <code>Failed ({E3Stage.Failed})</code>. An E3 that never gathers enough decryption shares ends
        there, the fee is refunded through the E3RefundManager, and the inputs stay sealed forever. Nothing half-opens.
      </p>
    </figure>
  )
}
