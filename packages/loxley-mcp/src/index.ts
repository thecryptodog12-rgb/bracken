...
const BASE_URL = 'https://docs.theloxley.com'
...
const STATIC_DOC_PAGES: DocPage[] = [
  ...
  { slug: 'building-with-loxley', title: 'Building with Loxley', url: '/building-with-loxley' },
  ...
  { slug: 'sdk', title: 'Loxley SDK', url: '/sdk' },
...
server.registerResource('docs-index', 'docs://index', { description: 'Index of all Loxley documentation pages' }, async () => ({
  contents: [
    {
      uri: 'docs://index',
...
server.registerTool(
  'read_doc',
  {
    description: 'Fetch and read a specific Loxley documentation page by slug',
...
server.registerTool(
  'search_docs',
  {
    description: 'Search for a keyword or phrase across all Loxley documentation pages',
...
server.registerTool('list_docs', { description: 'List all available Loxley documentation pages' }, async () => {
  const list = DOC_PAGES.map((p) => `- **${p.title}** → slug: \`${p.slug}\``).join('\n')
  return { content: [{ type: 'text', text: `# Loxley Documentation Pages\n\n${list}` }] }
})
