...
const normalizeArtifactFqn = (fqn: string): string =>
  fqn
    .replace(/^project\//, "")
    .replace(/^npm\/@loxley\/contracts@[^/]+\//, "@loxley/contracts/");
...
/** Package root of loxley-contracts. Used when script runs from another project (e.g. CRISP). */
const getLoxleyContractsRoot = (): string => {
 ...
 * Uses loxley-contracts package root so discovery works when run from consuming projects (e.g. CRISP).
 */
export const discoverVerifierContracts = (): string[] => {
  const honkDir = path.join(getLoxleyContractsRoot(), BFV_HONK_VERIFIER_DIR);
  if (!fs.existsSync(honkDir)) {
    return [];
  }
