...
export async function deployProtocolContracts(
  ...
  const ticketFactory = await ethers.getContractFactory("LoxleyTicketToken");
  ...
  const pricingLibFactory = await ethers.getContractFactory("LoxleyPricing");
  const pricingLib = await pricingLibFactory.deploy();
  await pricingLib.waitForDeployment();
  const loxleyPricing = await deployedAddress(pricingLib);
  ...
  const loxleyLifecycle = await deployedAddress(lifecycleLib);
  ...
  const loxleyImpl = await loxleyFactory.deploy();
  await loxleyImpl.waitForDeployment();
  const loxleyImplementation = await deployedAddress(loxleyImpl);
  const loxleyProxy = await deployProxy(
    ethers,
    loxleyImplementation,
    config.protocolOwner,
    loxleyFactory.interface.encodeFunctionData("initialize", [
      config.protocolOwner,
      registryProxy.proxy,
      ...
      timeoutConfig(config.loxley.timeoutConfig),
      initialE3Program,
    ]),
  );
  ...
  return {
    contracts: {
      ...
      loxley: loxleyProxy.proxy,
      loxleyImplementation,
      loxleyProxyAdmin: loxleyProxy.proxyAdmin,
      loxleyLifecycle,
      loxleyPricing,
    ...
    interfaces: {
      ...
      loxley: loxleyFactory.interface,
