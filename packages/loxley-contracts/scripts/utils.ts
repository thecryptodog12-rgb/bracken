...
/** Monorepo root (`loxley/`). Works from `scripts/` and compiled `dist/scripts/`. */
function resolveRepoRoot(): string {
  ...
  throw new Error(
    "Could not find loxley repo root (expected package.json name @loxley/main)",
  );
}
...
/**
 * Defines the Loxley.config.yaml structure
 */
export interface LoxleyConfig {
  chains: Array<{
    name: string;
    rpc_url: string;
    contracts: {
      e3_program?: { address: string; deploy_block: number };
      loxley?: { address: string; deploy_block: number };
...
/**
 * The function to update the loxley.config.yaml file with the deployed contract addresses.
 ...
 * @param pathToConfigFile - The path to the loxley.config.yaml file
...
export const updateE3Config = (
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect contract key line (e.g., `      loxley:`)
  ...
  console.log("\n✓ loxley.config.yaml updated successfully!");
};
