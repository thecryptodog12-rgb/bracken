...
const SALE_AMOUNT = ethers.parseEther("120000000"); // 120M LOXLEY
const LP_RESERVE = ethers.parseEther("1000000"); // 1M LOXLEY
...
describe("LoxleyTokenSaleDeployer", function () {
  ...
  it("deploys LOXLEY + CCA through LiquidityLauncher/LBPStrategy without a predicted auction", async function () {
    const ctx = await setup();
    const plan = await buildLbpSaleConfig(ctx);
  ...
  it("hands LOXLEY ownership to the Safe, which sets CLAIM_SOURCE once", async function () {
    const ctx = await setup();
    const plan = await buildLbpSaleConfig(ctx);
    const { foldAddress, auctionAddress } = await deploySale(ctx, plan);
    const fold = LoxleyTokenFactory.connect(foldAddress, ctx.operator);
