...
describe("LoxleyToken", function () {
  // ── Helpers ─────────────────────────────────────────────────────────────

  /// Deploy a minimal MockBondingRegistry + LoxleyToken for standalone tests.
  ...
  async function deploy() {
    ...
    const token = await new LoxleyTokenFactory(deployer).deploy(
  ...
  describe("constructor", function () {
    ...
    it("reverts when bondingRegistry is zero address", async function () {
      ...
      await expect(
        new LoxleyTokenFactory(deployer).deploy(
          await deployer.getAddress(),
          ccaStart,
          ccaEnd,
          noMoreLocks,
        ...
        { interface: LoxleyTokenFactory.createInterface() },
        "ZeroAddress",
      );
    });
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // BondingRegistry integration (uses deployLoxleySystem)
  // ═════════════════════════════════════════════════════════════════════════
  ...
  describe("BondingRegistry integration", function () {
    ...
    it("transferableBalanceOf counts bonded LOXLEY toward the locked floor", async function () {
      const signers = await ethers.getSigners();
      const [, beneficiary, , operator] = signers;
      const beneficiaryAddress = await beneficiary.getAddress();
      const operatorAddress = await operator.getAddress();
    ...
    it("blocks bond-owner rotation that would detach locked LOXLEY", async function () {
      const signers = await ethers.getSigners();
