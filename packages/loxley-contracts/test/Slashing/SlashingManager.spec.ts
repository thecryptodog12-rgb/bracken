...
describe("SlashingManager", function () {
  ...
  async function setup() {
    ...
    const mockLoxley = await ethers.deployContract("MockSlashingLoxley");
    await slashingManager.setLoxley(await mockLoxley.getAddress());
    await slashingManager.setE3RefundManager(addressOne);
    const now = await time.latest();
    await mockLoxley.snapshotDependencies(slashingManager, 0, now + 3600);
    await mockLoxley.snapshotDependencies(slashingManager, 1, now + 3600);
  ...
  describe("setSlashPolicy()", function () {
    ...
    it("requires policies to be refreshed after an asset rotation", async function () {
      ...
      await networkHelpers.setBalance(loxleyAddress, ethers.parseEther("1"));
      await networkHelpers.impersonateAccount(loxleyAddress);
      await slashingManager
        .connect(await ethers.getSigner(loxleyAddress))
        .snapshotE3Dependencies(42);
      await networkHelpers.stopImpersonatingAccount(loxleyAddress);
