...
// Pure helpers (no deploys). Compose with `deployLoxleySystem`.
 ...
 *
 * @param loxley       Loxley instance.
 * @param usdcToken     MockUSDC instance funding the request.
 * @param requestParams Struct passed to `loxley.request`.
...
export const makeRequest = async (
  ...
  const fee = await loxley.getE3Quote(quoteParams);

  await tokenContract.approve(await loxley.getAddress(), fee);
  return loxleyContract.request({ ...quoteParams, maxFee: fee });
};
...
export interface BuildRequestParamsOptions {
  ...
  customParams?: string;
  /** Param-set id registered on the Loxley. Defaults to `0`. */
  paramSet?: number;
}

/**
 * Build a fresh `Loxley.request(...)` struct anchored at the current block
