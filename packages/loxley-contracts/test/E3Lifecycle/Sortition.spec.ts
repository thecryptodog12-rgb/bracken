...
async function deployStack() {
  ...
  const makeRequest = async () => {
    ...
    const tx = await loxley.connect(requester).request(req);
    await mine(1);
    return tx;
  };
...
describe("Sortition & E3 lifecycle", function () {
  it("prevents timeout failure when the committee is ready", async function () {
    ...
    const { loxley, ciphernodeRegistry, other, op1, op2, op3 } = ctx;
    ...
    await expect(
      loxley.connect(other).markE3Failed(firstE3Id),
    ).to.be.revertedWithCustomError(loxley, "FailureConditionNotMet");
    ...
    const { dkgWindow } = await loxley.getE3TimeoutConfig(firstE3Id);
    expect((await loxley.getDeadlines(firstE3Id)).dkgDeadline).to.equal(
      deadline + dkgWindow,
    );
  });
  ...
  it("expires a ready committee at its request-time DKG cutoff", async function () {
    ...
    expect((await loxley.getDeadlines(firstE3Id)).dkgDeadline).to.equal(0);
    ...
    await expect(loxley.connect(other).markE3Failed(firstE3Id))
  ...
  describe("markE3Failed grace period", function () {
    it("inside grace window: third party reverts, requester succeeds", async function () {
      const ctx = await loadFixture(deployStack);
      const { loxley, requester, other, makeRequest } = ctx;

      const grace = 600;
      await loxley.setMarkFailedGracePeriod(grace);
      await makeRequest();
      ...
      await expect(
        loxley.connect(other).markE3Failed(e3Id),
      ).to.be.revertedWithCustomError(loxley, "MarkE3FailedInGracePeriod");

      await expect(loxley.connect(requester).markE3Failed(e3Id)).to.emit(
        loxley,
        "E3Failed",
      );
    });
    ...
    it("after grace window: anyone can call markE3Failed", async function () {
      const ctx = await loadFixture(deployStack);
      const { loxley, other, makeRequest } = ctx;
      ...
      await expect(loxley.connect(other).markE3Failed(e3Id)).to.emit(
        loxley,
        "E3Failed",
      );
    });
    ...
    it("rejects a provisional member during the grace window", async function () {
      const ctx = await loadFixture(deployStack);
      const { loxley, ciphernodeRegistry, op1, makeRequest } = ctx;
      ...
      await expect(
        loxley.connect(op1).markE3Failed(firstE3Id),
      ).to.be.revertedWithCustomError(loxley, "MarkE3FailedInGracePeriod");
    });

    it("setMarkFailedGracePeriod is owner-only and emits event", async function () {
      const ctx = await loadFixture(deployStack);
      const { loxley, other } = ctx;

      await expect(
        loxley.connect(other).setMarkFailedGracePeriod(42),
      ).to.be.revertedWithCustomError(loxley, "OwnableUnauthorizedAccount");

      await expect(loxley.setMarkFailedGracePeriod(42))
        .to.emit(loxley, "MarkFailedGracePeriodSet")
        .withArgs(42);
      expect(await loxley.markFailedGracePeriod()).to.equal(42n);
    });
  });
