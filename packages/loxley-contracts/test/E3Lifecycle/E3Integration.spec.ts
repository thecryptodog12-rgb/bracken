 ...
 * - Loxley.sol (main coordinator with integrated lifecycle management)
...
describe("E3 Integration - Refund/Timeout Mechanism", function () {
  ...
  const setup = async () => {
    ...
    const makeRequest = async (
      ...
      await requestToken.connect(signer).approve(loxleyAddress, fee);
      await loxley.connect(signer).request(requestParams);
      await mine(1);
  ...
  describe("E3 Request with Lifecycle Integration", function () {
    it("rejects requests until the dependency graph is activated", async function () {
      ...
      await expect(
        makeRequest(sys.loxley, sys.usdcToken, sys.request),
      ).to.be.revertedWithCustomError(sys.loxley, "RequestsPaused");
    });
    ...
    it("initializes E3 lifecycle when request is made", async function () {
      ...
      const storedRequester = await loxley.getRequester(firstE3Id);
    ...
    it("keeps a selected operator's queued collateral slashable until the E3 ends", async function () {
      ...
      await loxley.markE3Failed(firstE3Id);
      await expect(registry.releaseCommittee(firstE3Id))
    ...
    it("allows only the requester to cancel an active E3", async function () {
      const { loxley, makeReadyRequest, owner, requester } =
        await loadFixture(setup);
      await makeReadyRequest();

      await expect(loxley.connect(owner).cancelE3(firstE3Id))
        .to.be.revertedWithCustomError(loxley, "NotRequester")
        .withArgs(firstE3Id, await owner.getAddress());

      await expect(loxley.connect(requester).cancelE3(firstE3Id))
        .to.emit(loxley, "E3Failed")
        .withArgs(firstE3Id, 1, 9);
      expect(await loxley.getFailureReason(firstE3Id)).to.equal(9);

      await expect(loxley.connect(requester).cancelE3(firstE3Id))
        .to.be.revertedWithCustomError(loxley, "E3NotCancellable")
    ...
    it("pays only completed milestones when the requester cancels", async function () {
      ...
      for (const scenario of scenarios) {
        ...
        if (scenario.stage === 1) {
          ...
          if (scenario.stage === 4) {
            const e3 = await ctx.loxley.getE3(firstE3Id);
            await time.increaseTo(e3.inputWindow[1]);
            const ciphertext = "0x" + "ab".repeat(100);
            await ctx.loxley.publishCiphertextOutput(
              firstE3Id,
              ciphertext,
    ...
    it("rejects invalid failure reasons from an authorized dependency", async function () {
      ...
      for (const reason of [0, 9, 13, 255]) {
        await expect(
          loxley.connect(registrySigner).onE3Failed(firstE3Id, reason),
        )
          .to.be.revertedWithCustomError(loxley, "InvalidFailureReason")
          .withArgs(reason);
      }
    ...
    it("routes zero-value node shares to the treasury", async function () {
      ...
      const deadlines = await loxley.getDeadlines(firstE3Id);
      await time.increaseTo(deadlines.computeDeadline + 1n);
      await loxley.markE3Failed(firstE3Id);
      await loxley.processE3Failure(firstE3Id);
  ...
  describe("Committee Formed Integration", function () {
    ...
    it("emits CommitteeFormed event when committee is published", async function () {
      ...
      await expect(
        ...
        .to.emit(loxley, "CommitteeFormed")
        .withArgs(firstE3Id);
    });

    it("rejects committee publication after the DKG deadline", async function () {
      const { loxley, registry, finalizeReadyCommittee } =
        await loadFixture(setup);
      await finalizeReadyCommittee();

      const publicKey = "0x1234567890abcdef1234567890abcdef";
      const pkCommitment = ethers.keccak256(publicKey);
      const { dkgDeadline } = await loxley.getDeadlines(firstE3Id);
      ...
      ).to.be.revertedWithCustomError(loxley, "DKGDeadlinePassed");
    });
  });
  ...
  describe("processE3Failure()", function () {
    it("reverts if lifecycle is not a valid contract", async function () {
      ...
      // Create a new Loxley with addressOne as the refund manager placeholder.
      const newLoxleyContract = await ignition.deploy(LoxleyModule, {
        parameters: {
          Loxley: {
            owner: await owner.getAddress(),
            maxDuration: THIRTY_DAYS,
            registry: await emptyRegistry.getAddress(),
            bondingRegistry: await loxley.bondingRegistry(),
            e3RefundManager: addressOne,
            feeToken: await loxley.feeToken(),
            initialE3Program: await e3Program.getAddress(),
          },
        },
      });
      const newLoxley = LoxleyFactory.connect(
        await newLoxleyContract.loxley.getAddress(),
        owner,
      );
    ...
    it("reverts if E3 not in failed state", async function () {
      ...
      await expect(
        loxley.processE3Failure(firstE3Id),
      ).to.be.revertedWithCustomError(loxley, "E3NotFailed");
    });
    ...
    it("processes failure and calculates refund for committee formation timeout", async function () {
      ...
      // Process the failure
      await expect(loxley.processE3Failure(firstE3Id)).to.emit(
        loxley,
        "E3FailureProcessed",
      );
    ...
    it("rolls back failure processing when the registry lookup reverts", async function () {
      ...
      const e3Id = await sys.loxley.nexte3Id();
      await makeRequest(sys.loxley, sys.usdcToken, sys.request);
      const payment = await sys.loxley.e3Payments(e3Id);
      const registryAddress = await registry.getAddress();
      ...
      await expect(
        sys.loxley.processE3Failure(e3Id),
      ).to.be.revertedWithCustomError(registry, "ActiveCommitteeLookupFailed");
      expect(await sys.loxley.e3Payments(e3Id)).to.equal(payment);
      const distribution =
    ...
    it("rejects sender fees from fee escrow and refund custody", async function () {
      ...
      await loxley.markE3Failed(firstE3Id);
      await loxley.markE3Failed(firstE3Id + 1n);

      const firstPayment = await loxley.e3Payments(firstE3Id);
      const senderFee = firstPayment / 100n;
      await token.setFeeBps(100);
      await expect(loxley.processE3Failure(firstE3Id))
        .to.be.revertedWithCustomError(loxley, "AssetTransferMismatch")
        .withArgs(tokenAddress, firstPayment, firstPayment + senderFee);
      expect(await loxley.e3Payments(firstE3Id)).to.equal(firstPayment);

      await token.setFeeBps(0);
      await loxley.processE3Failure(firstE3Id);
      await loxley.processE3Failure(firstE3Id + 1n);
    ...
    it("reverts if trying to process failure twice", async function () {
      ...
      await expect(
        loxley.processE3Failure(firstE3Id),
      ).to.be.revertedWithCustomError(loxley, "NoPaymentToRefund");
    });
  ...
  describe("Slashed Funds Escrow", function () {
    it("E2E: slash via SlashingManager pays honest nodes without reducing the requester refund", async function () {
      ...
      //    underlying through Loxley into E3RefundManager escrow.
    ...
    it("AUD-M05: reserves a failed slash route and retries it permissionlessly", async function () {
      ...
      await expect(slashingManager.connect(requester).retrySlashRoute(0))
        ...
        .and.to.emit(loxley, "SlashedFundsEscrowed")
    ...
    it("routes failed-E3 slashes to treasury when no honest nodes exist", async function () {
      ...
      // Call from the Loxley frozen in the E3 policy snapshot. Rotating the
      // manager's live pointer must not grant settlement authority for old E3s.
      const originalLoxley = await e3RefundManager.loxley();
      ...
      await e3RefundManager
        .connect(await ethers.getSigner(originalLoxley))
        .escrowSlashedFunds(
          firstE3Id,
          100,
          await operator1.getAddress(),
    ...
    it("credits every failed-E3 slash to honest nodes without requester compensation", async function () {
      ...
      const loxleySigner = await ethers.getSigner(originalLoxley);
      const usdcAddress = await usdcToken.getAddress();

      await e3RefundManager
        .connect(loxleySigner)
        .escrowSlashedFunds(
          firstE3Id,
          100,
          await operator1.getAddress(),
  ...
  describe("Failure Claim Roles and DKG Timeout", function () {
    ...
    it("complete flow: request -> committee formed -> DKG timeout -> fail -> process -> claim", async function () {
      ...
      const failureReason = await loxley.getFailureReason(firstE3Id);
  ...
  describe("Multiple E3 Requests Isolation", function () {
    it("tracks multiple E3s independently", async function () {
      ...
      expect(await loxley.getE3Stage(firstE3Id)).to.equal(1);
      expect(await loxley.getE3Stage(firstE3Id + 1n)).to.equal(1);
      expect(await loxley.getE3Stage(firstE3Id + 2n)).to.equal(1);
      ...
      expect(await loxley.getE3Stage(firstE3Id)).to.equal(6); // Failed
      expect(await loxley.getE3Stage(firstE3Id + 1n)).to.equal(1); // Still Requested
      expect(await loxley.getE3Stage(firstE3Id + 2n)).to.equal(1); // Still Requested

      // E3 #1 and #2 also can be failed now (their deadlines have also passed)
      const [canFail1] = await loxley.checkFailureCondition(firstE3Id + 1n);
      const [canFail2] = await loxley.checkFailureCondition(firstE3Id + 2n);
      ...
      await loxley.markE3Failed(firstE3Id + 2n);
      expect(await loxley.getE3Stage(firstE3Id + 2n)).to.equal(6); // Now Failed
      expect(await loxley.getE3Stage(firstE3Id + 1n)).to.equal(1); // Still Requested

      // Verify each E3 has independent failure reasons
      expect(await loxley.getFailureReason(firstE3Id)).to.equal(1); // CommitteeFormationTimeout
      expect(await loxley.getFailureReason(firstE3Id + 2n)).to.equal(1); // CommitteeFormationTimeout
    });
  ...
  describe("Success Path (Complete E3)", function () {
    it("distributes escrowed slashed funds to nodes and treasury on successful completion", async function () {
      ...
      expect(await loxley.getE3Stage(firstE3Id)).to.equal(3); // KeyPublished
      ...
      expect(await loxley.getE3Stage(firstE3Id)).to.equal(4); // CiphertextReady

      // Record the E3 payment (normal rewards) before completion zeroes it
      const e3Payment = await loxley.e3Payments(firstE3Id);
      ...
      expect(await loxley.getE3Stage(firstE3Id)).to.equal(5); // Complete
    ...
    it("transitions through all stages to completion", async function () {
      ...
      // 1. Make request
      await makeRequest();
      expect(await loxley.getE3Stage(firstE3Id)).to.equal(1); // Requested
      ...
      expect(await loxley.getE3Stage(firstE3Id)).to.equal(2); // CommitteeFinalized
      ...
      const plaintextOutput = "0x" + "cd".repeat(100);
      await loxley.publishPlaintextOutput(firstE3Id, plaintextOutput, proof);
      expect(await loxley.getE3Stage(firstE3Id)).to.equal(5); // Complete

      // Cannot mark completed E3 as failed
      await expect(
        loxley.markE3Failed(firstE3Id),
      ).to.be.revertedWithCustomError(loxley, "E3AlreadyComplete");
    });
