...
// Loxley / CiphernodeRegistry / BondingRegistry / E3RefundManager /
...
import { BFV_PARAMS_DEFAULT, deployLoxleySystem, ethers } from "../fixtures";
...
describe("Governance — access control, bounds & events", function () {
  describe("Ownable2Step + renounceOwnership disabled", function () {
    it("Loxley: transferOwnership is two-step", async function () {
      const { loxley, other, ownerAddress } = await deployAll();
      const otherAddress = await other.getAddress();
      await loxley.transferOwnership(otherAddress);
      expect(await loxley.owner()).to.equal(ownerAddress);
      expect(await loxley.pendingOwner()).to.equal(otherAddress);
      await loxley.connect(other).acceptOwnership();
      expect(await loxley.owner()).to.equal(otherAddress);
    });
    ...
    it("LoxleyToken: renounceOwnership reverts", async function () {
    ...
    it("LoxleyTicketToken: renounceOwnership reverts", async function () {
    ...
    it("Loxley: renounceOwnership reverts", async function () {
      const { loxley } = await deployAll();
      await expect(loxley.renounceOwnership()).to.be.revertedWithCustomError(
        loxley,
        "RenounceOwnershipDisabled",
      );
    });
  ...
  describe("Loxley bounds exposed", function () {
    it("setMaxDuration reverts above MAX_DURATION_CAP", async function () {
      const { loxley } = await deployAll();
      const cap = await loxley.MAX_DURATION_CAP();
      await expect(
        loxley.setMaxDuration(cap + 1n),
  ...
  describe("PkVerifierSet event", function () {
    ...
    it("rejects verifiers compiled for another committee", async function () {
      ...
      await expect(
        loxley.setPkVerifier(schemeId, await wrongPkVerifier.getAddress()),
      )
        .to.be.revertedWithCustomError(loxley, "VerifierThresholdMismatch")
        .withArgs(5, 2);
      await expect(
