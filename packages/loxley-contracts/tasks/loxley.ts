...
async function getLoxleyConnection(hre: any) {
  const { ethers } = await hre.network.connect();
  const [signer] = await ethers.getSigners();
  const chain = hre.globalOptions.network;
  const deployment = readDeploymentArgs("Loxley", chain);

  if (!deployment?.address) {
    throw new Error("Loxley deployment not found");
  }
...
export const requestCommittee = task(
  ...
  .setAction(async () => ({
    default: async (
      ...
      const [signer] = await ethers.getSigners();
      const loxleyContract = loxley.connect(signer);
      const mockUSDCContract = mockUSDC.connect(signer);

      const loxleyArgs = readDeploymentArgs(
        "Loxley",
        hre.globalOptions.network,
      );

      if (!loxleyArgs) {
        throw new Error("Loxley deployment arguments not found");
      }
      ...
      const quoteParams = {
        ...
        expectedCryptoConfigId: await loxleyContract.activeCryptoConfigId(),
        maxFee: MaxUint256,
      };

      const fee = await loxleyContract.getE3Quote(quoteParams);
      ...
      const requestedLog = receipt.logs.find(
        (log: Log) =>
          log.address.toLowerCase() === loxleyAddress &&
      ...
      const requestedEvent = loxleyContract.interface.parseLog(requestedLog);
      if (!requestedEvent) {
...
export const enableE3 = task("loxley:enableE3", "Enable an E3 program")
  .addOption({
    name: "e3Address",
    description: "address of the E3 program",
    defaultValue: ZeroAddress,
  ...
  .setAction(async () => ({
    default: async ({ e3Address }, hre) => {
      const { ethers, loxley } = await getLoxleyConnection(hre);

      if (await loxley.e3Programs(e3Address)) {
        console.log(`E3 program already enabled: ${e3Address}`);
        return;
      }

      const ownerAddress = (await loxley.owner()).toLowerCase();
      ...
      if (!ownerSigner) {
        throw new Error(
          `Loxley owner ${ownerAddress} is not an available signer. Submit registerE3Program through the owner Safe.`,
        );
      }
...
export const publishCiphertext = task(
  ...
  .setAction(async () => ({
    default: async (
      ...
      const tx = await loxley.publishCiphertextOutput(
        e3Id,
        dataToSend,
        commitmentToSend,
        proofToSend,
...
export const publishPlaintext = task(
  ...
  .setAction(async () => ({
    default: async ({ e3Id, data, dataFile, proof, proofFile }, hre) => {
      ...
      const tx = await loxley.publishPlaintextOutput(
        e3Id,
        dataToSend,
        proofToSend,
      );
