...
library BondingSlashingLib {
    ...
    function snapshotE3(
        ...
        require(
            refundManager != address(0) && loxley != address(0),
        ...
        state.e3Loxley[manager][e3Id] = loxley;
    ...
    function releaseE3(
        ...
        BondingSlashingStorage.Layout storage state = _layout();
        address loxley = state.e3Loxley[manager][e3Id];
        if (loxley == address(0)) {
        ...
        delete state.e3Loxley[manager][e3Id];
        state.managers[manager].e3Assignments--;
