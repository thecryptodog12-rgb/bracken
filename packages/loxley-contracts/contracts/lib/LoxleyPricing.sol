...
/**
 * @title LoxleyPricing
 * @notice External library extracted from {Loxley} to keep its deployed
 ...
 *         out of the Loxley runtime bytecode.
 */
library LoxleyPricing {
    ...
    function validateFeeAssetConfig(
        ...
        if (actualDecimals != config.expectedDecimals) {
            revert ILoxley.FeeTokenDecimalsMismatch(
                token,
    ...
    function _validatePricingConfig(
        ILoxley.PricingConfig calldata config,
        uint16 maxMarginBps,
        uint16 maxProtocolShareBps
    ) private pure {
        if (config.marginBps > maxMarginBps)
            revert ILoxley.BpsExceedsMax(config.marginBps);
        ...
        ) revert ILoxley.TreasuryRequired();
        if (config.minCommitteeSize < config.minThreshold)
            revert ILoxley.MinSizeBelowMinThreshold();
    }
    ...
    ///      of Loxley's size-constrained runtime bytecode.
    ...
    function distributeRewards(
        ...
        emit ILoxley.RewardsDistributed(e3Id, activeNodes, amounts);
        refundManager.distributeSlashedFundsOnSuccess(e3Id, paymentToken);
    }
    ...
    function _transferExact(
        ...
        uint256 spent = custodyBefore > custodyAfter
            ...
            : 0;
        if (spent != amount) {
            revert ILoxley.AssetTransferMismatch(token, amount, spent);
        }
    }
    ...
    function _requireExactReceipt(
        ...
        uint256 actual = balanceAfter > balanceBefore
            ...
            : 0;
        if (actual != expected) {
            revert ILoxley.AssetTransferMismatch(token, expected, actual);
        }
    }

    /// @notice Pure fee quote math. The caller (Loxley) is responsible for
    ...
    function quote(
        ILoxley.PricingConfig calldata pc,
        ILoxley.E3TimeoutConfig calldata tc,
        ...
        if (pc.minThreshold > 0 && threshold[0] < pc.minThreshold)
            revert ILoxley.ThresholdTooSmall(threshold[0]);
        ...
        if (fee == 0) revert ILoxley.PaymentRequired(fee);
    }
    ...
    function _validateQuoteWindow(
        ...
        if (inputWindowStart < requestTime)
            revert ILoxley.InvalidInputDeadlineStart(inputWindowStart);
