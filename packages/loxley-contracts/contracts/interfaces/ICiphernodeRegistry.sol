...
import { ILoxley } from "./ILoxley.sol";
...
interface ICiphernodeRegistry {
    ...
    ///      canonical {ILoxley.CommitteeFinalized} surface.
    /// @param e3Id ID of the E3 computation
    /// @param committee Array of selected ciphernode addresses
    /// @param scores Array of sortition scores corresponding to each committee member
    ...
    /// @notice This event MUST be emitted when `loxley` is set.
    /// @param loxley Address of the loxley contract.
    event LoxleySet(address indexed loxley);
    ...
    /// @notice Caller is not the Loxley contract
    error OnlyLoxley();
    ...
    /// @dev This function MUST revert when not called by the Loxley contract.
    ...
    /// @dev Permissionless and bound to the request-time Loxley and bonding registry.
    ...
    /// @notice This function should be called by the Loxley contract to get the public key of a committee.
    ...
    /// @notice This function should be called by the Loxley contract to get the committee for a given E3.
    /// @dev This function MUST revert if no committee has been requested for the given E3.
    ...
    /// @notice Sets the Loxley contract address
    /// @dev Only callable by owner
    /// @param _loxley Address of the Loxley contract
    function setLoxley(ILoxley _loxley) external;
