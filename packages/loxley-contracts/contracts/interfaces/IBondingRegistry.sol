...
interface IBondingRegistry {
    ...
    ///         less wallet-plus-bonded LOXLEY than its current locked balance.
    ...
    event BondingAssetConfigUpdated(
        LoxleyTicketToken indexed ticketToken,
    ...
    /**
     * @notice Get LOXLEY that still counts toward an account's locked-floor collateral.
     * @dev Includes active ciphernode bond plus pending LOXLEY exits that remain slashable/not returned.
     * @param account Bond owner whose aggregate LOXLEY bond credit is queried
     ...
     * @dev Settable once per ciphernode bond token. Bonded LOXLEY is transferred to this registry and never
    ...
    function ticketToken() external view returns (LoxleyTicketToken);
     ...
     *      only when doing so preserves the previous owner's locked-LOXLEY coverage.
     */
