 ...
 *        - a domain-binding slot supplied by Loxley and derived from
...
interface IDecryptionVerifier {
    ...
    ///         domain recomputed by Loxley and the DKG anchors stored for it.
    ...
    /// @param decryptionDomain `keccak256(abi.encode(chainId, loxley, e3Id,
    ///        committeeHash, ciphertextOutputHash, committeePublicKey))`.
    /// @param plaintextOutputHash `keccak256(plaintextOutput)` expected by the Loxley.
