...
contract E3RefundManager is IE3RefundManager, Ownable2StepUpgradeable {
    ...
    /// @notice Emitted whenever {loxley} is updated.
    ...
    //                                                        //
    ////////////////////////////////////////////////////////////
    /// @notice Restricts function to Loxley contract only
    modifier onlyLoxley() {
        if (msg.sender != address(loxley)) revert Unauthorized();
        _;
    }

    /// @dev Authorizes the Loxley contract frozen for an existing E3.
    modifier onlyE3Loxley(uint256 e3Id) {
        E3PolicySnapshot storage policy = _e3PolicySnapshots[e3Id];
        if (!policy.initialized || msg.sender != policy.loxley) {
            revert Unauthorized();
        }
        _;
    }
    ...
    function initialize(
        ...
        require(_loxley != address(0), "Invalid loxley");
        require(_treasury != address(0), "Invalid treasury");

        loxley = ILoxley(_loxley);
        bondingRegistry = loxley.bondingRegistry();
        treasury = _treasury;
    ...
    function calculateRefund(
        ...
        // {Loxley._distributeRewards} success-path).
        ...
        )
    {
        ...
        ILoxley.E3Stage failedAt = reason ==
    ...
    function _getFailedAtStage(
        ILoxley.FailureReason reason
    ) internal pure returns (ILoxley.E3Stage) {
        ...
        if (
            ...
            return ILoxley.E3Stage.Requested;
        }
        if (
            reason == ILoxley.FailureReason.DKGTimeout ||
            reason == ILoxley.FailureReason.DKGInvalidShares
        ) {
            return ILoxley.E3Stage.CommitteeFinalized;
        }
        if (reason == ILoxley.FailureReason.NoInputsReceived) {
            return ILoxley.E3Stage.KeyPublished;
        }
        if (
    ...
    function claimRequesterRefund(
        ...
        address requester = _loxleyFor(e3Id).getRequester(e3Id);
    ...
    function snapshotE3Policy(
        ...
        snapshot.loxley = msg.sender;
        snapshot.registry = registry;
        snapshot.bondingRegistry = address(
            ILoxley(msg.sender).bondingRegistry()
        );
        snapshot.slashingManager = address(
            ILoxley(msg.sender).slashingManager()
        );
        require(
    ...
    function _loxleyFor(uint256 e3Id) internal view returns (ILoxley) {
        E3PolicySnapshot storage policy = _e3PolicySnapshots[e3Id];
        return ILoxley(policy.loxley);
    }
    ...
    /// @notice Set the Loxley contract address
    /// @param _loxley New Loxley address
    function setLoxley(address _loxley) external onlyOwner {
        require(_loxley != address(0), "Invalid loxley");
        address oldValue = address(loxley);
        loxley = ILoxley(_loxley);
        emit LoxleyUpdated(oldValue, _loxley);
    }
