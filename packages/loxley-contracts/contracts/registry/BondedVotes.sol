...
// LOXLEY's lock schedule, the same surface the bonding registry reads. Lock-encumbered LOXLEY sits in
...
/**
 * @title BondedVotes
 * @notice Voting power that counts LOXLEY bonded as an operator alongside a primary vote source.
 *
 * @dev Bonding transfers LOXLEY to `BondingRegistry`, which never delegates it. Under ERC20Votes an
 ...
 * LOXLEY still counts in `getPastTotalSupply`, so it raises the quorum denominator while being
 ...
 *
 * TWO TOKEN REFERENCES, ON PURPOSE. `token` is LOXLEY: it supplies the metadata and, critically,
 ...
 *
 *   votesSource == token          wallet-held LOXLEY votes, via the token's own ERC20Votes
 *                                 delegation. The original behaviour.
 *   votesSource == an escrow      only LOXLEY locked in the escrow votes. Idle wallet LOXLEY carries
 *                                 no weight, so holders must lock to participate — while
 *                                 operators keep their weight by bonding instead.
 *
 * Keeping the denominator on LOXLEY in both cases is what makes the ratio sound. Every unit the
 * numerator can produce — locked or bonded — is a LOXLEY that is still inside LOXLEY's total supply,
 ...
 *
 * A THIRD SOURCE, under an escrow votes source only: LOXLEY still encumbered by LOXLEY's own vesting
 * locks. That LOXLEY sits in the holder's own wallet and {LoxleyToken._update} refuses to move
 ...
 *
 * Escrowed and bonded LOXLEY cannot overlap: the escrow and the registry custody the token at two
 ...
 * {LoxleyToken.transferableBalanceOf}, which lets a wallet move its locked LOXLEY to the extent
 * the bond already covers the obligation — so an account that bonds its locked LOXLEY reports the
...
contract BondedVotes is IERC5805 {
    /// @notice The LOXLEY token. Supplies the metadata and the quorum denominator.
    IVotes public immutable token;

    /// @notice Where per-account voting power is read from: the token itself, or an escrow
    /// adapter when only locked LOXLEY may vote.
    IVotes public immutable votesSource;

    /// @notice The escrow behind `votesSource`, or zero when the votes source is the token.
    /// @dev Held so {balanceOf} can attribute locked LOXLEY without going through the adapter's
    /// own `balanceOf`, which counts lock NFTs rather than tokens.
    address public immutable escrow;

    /// @notice Records bonded totals over time for the registry holding bonded LOXLEY.
    IBondedCheckpoints public immutable checkpoints;

    /// @notice The registry that custodies the bonded LOXLEY and writes the history.
    ...
    /**
     * @param _token The LOXLEY token: metadata, total supply, and the quorum denominator.
     * @param _votesSource Where per-account voting power is read. Pass `_token` itself to count
     * wallet-held LOXLEY, or an escrow IVotes adapter to count only locked LOXLEY.
    ...
    /// token's supply does not back — the denominator would be LOXLEY while the numerator counted
    ...
    /// LOXLEY, plus — under an escrow votes source — the vesting-locked LOXLEY it cannot escrow. All
    /// three are LOXLEY-denominated and read at the same timepoint.
    ///
    ...
    ///
    /// Zero unless the votes source is an escrow. When the token votes for itself, locked LOXLEY is
    /// wallet LOXLEY and the token has already counted it — adding it again would simply double
    /// every locked holder's weight.
    ///
    /// Netted, because a bond satisfies a lock: LOXLEY that is bonded is reported by BOTH
    ...
    /// cutting the lock, so an operator that had already moved locked LOXLEY out on the strength of
    /// that bond is left owing more than it holds — and the uncapped term would vote with the
    /// difference, LOXLEY that is now in the slash recipient's hands and countable there too.
    ///
    ...
    /// locked LOXLEY are both already counted here: each was transferred, not burned. Adding either
    ...
    /// @notice Get the LOXLEY attributable to an account: held in its wallet plus bonded under it.
    /// @dev Not a spendable balance, and nothing here can move it. Unlike {getVotes} this ignores
    /// delegation, so it answers "how much LOXLEY is this account's" rather than "how much can it
    /// vote with". A holder that never delegated reads a balance above its votes, which is the
    /// intended signal.
    ///
    /// The registry is netted down by what it merely custodies. Bonding moves LOXLEY into the
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        if (account == registry) {
            ...
            // leaving it here too would place the same LOXLEY at two addresses and push summed
            ...
            // be the more dangerous guess. The cost is that LOXLEY donated to the escrow on its own
        ...
        // Locked LOXLEY is custodied by the escrow for the same reason bonded LOXLEY is custodied by
    ...
    /// @dev Passed through for the same reason as {getPastTotalSupply}: bonded LOXLEY was
