...
// LOX's lock schedule, the same surface the bonding registry reads. Lock-encumbered LOX sits in
...
/**
 * @title BondedVotes
 * @notice Voting power that counts LOX bonded as an operator alongside a primary vote source.
 *
 * @dev Bonding transfers LOX to `BondingRegistry`, which never delegates it. Under ERC20Votes an
 ...
 * LOX still counts in `getPastTotalSupply`, so it raises the quorum denominator while being
 ...
 *
 * TWO TOKEN REFERENCES, ON PURPOSE. `token` is LOX: it supplies the metadata and, critically,
 ...
 *
 *   votesSource == token          wallet-held LOX votes, via the token's own ERC20Votes
 *                                 delegation. The original behaviour.
 *   votesSource == an escrow      only LOX locked in the escrow votes. Idle wallet LOX carries
 *                                 no weight, so holders must lock to participate — while
 *                                 operators keep their weight by bonding instead.
 *
 * Keeping the denominator on LOX in both cases is what makes the ratio sound. Every unit the
 * numerator can produce — locked or bonded — is a LOX that is still inside LOX's total supply,
 ...
 *
 * A THIRD SOURCE, under an escrow votes source only: LOX still encumbered by LOX's own vesting
 * locks. That LOX sits in the holder's own wallet and {LoxleyToken._update} refuses to move
 ...
 *
 * Escrowed and bonded LOX cannot overlap: the escrow and the registry custody the token at two
 ...
 * {LoxleyToken.transferableBalanceOf}, which lets a wallet move its locked LOX to the extent
 * the bond already covers the obligation — so an account that bonds its locked LOX reports the
...
contract BondedVotes is IERC5805 {
    /// @notice The LOX token. Supplies the metadata and the quorum denominator.
    IVotes public immutable token;

    /// @notice Where per-account voting power is read from: the token itself, or an escrow
    /// adapter when only locked LOX may vote.
    IVotes public immutable votesSource;

    /// @notice The escrow behind `votesSource`, or zero when the votes source is the token.
    /// @dev Held so {balanceOf} can attribute locked LOX without going through the adapter's
    /// own `balanceOf`, which counts lock NFTs rather than tokens.
    address public immutable escrow;

    /// @notice Records bonded totals over time for the registry holding bonded LOX.
    IBondedCheckpoints public immutable checkpoints;

    /// @notice The registry that custodies the bonded LOX and writes the history.
    ...
    /**
     * @param _token The LOX token: metadata, total supply, and the quorum denominator.
     * @param _votesSource Where per-account voting power is read. Pass `_token` itself to count
     * wallet-held LOX, or an escrow IVotes adapter to count only locked LOX.
    ...
    /// token's supply does not back — the denominator would be LOX while the numerator counted
    ...
    /// LOX, plus — under an escrow votes source — the vesting-locked LOX it cannot escrow. All
    /// three are LOX-denominated and read at the same timepoint.
    ///
    ...
    ///
    /// Zero unless the votes source is an escrow. When the token votes for itself, locked LOX is
    /// wallet LOX and the token has already counted it — adding it again would simply double
    /// every locked holder's weight.
    ///
    /// Netted, because a bond satisfies a lock: LOX that is bonded is reported by BOTH
    ...
    /// cutting the lock, so an operator that had already moved locked LOX out on the strength of
    /// that bond is left owing more than it holds — and the uncapped term would vote with the
    /// difference, LOX that is now in the slash recipient's hands and countable there too.
    ///
    ...
    /// locked LOX are both already counted here: each was transferred, not burned. Adding either
    ...
    /// @notice Get the LOX attributable to an account: held in its wallet plus bonded under it.
    /// @dev Not a spendable balance, and nothing here can move it. Unlike {getVotes} this ignores
    /// delegation, so it answers "how much LOX is this account's" rather than "how much can it
    /// vote with". A holder that never delegated reads a balance above its votes, which is the
    /// intended signal.
    ///
    /// The registry is netted down by what it merely custodies. Bonding moves LOX into the
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        if (account == registry) {
            ...
            // leaving it here too would place the same LOX at two addresses and push summed
            ...
            // be the more dangerous guess. The cost is that LOX donated to the escrow on its own
        ...
        // Locked LOX is custodied by the escrow for the same reason bonded LOX is custodied by
    ...
    /// @dev Passed through for the same reason as {getPastTotalSupply}: bonded LOX was
