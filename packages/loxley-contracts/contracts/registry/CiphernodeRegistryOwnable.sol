    ...
    Ownable2StepUpgradeable
{
    ...
    /// @notice Address of the Loxley contract authorized to request committees
    ...
    /// @dev Restricts function access to only the Loxley contract
    modifier onlyLoxley() {
        require(msg.sender == address(loxley), OnlyLoxley());
        _;
    }
    ...
    function requestCommittee(
    ...
    ) external onlyLoxley returns (bool success) {
        ...
        dependencies.loxleyContract = ILoxley(msg.sender);
        ...
        // {block.timestamp}. This matches the LoxleyTicketToken's timestamp-mode clock so
    ...
    function publishCommittee(
        ...
        E3 memory e3 = _loxleyFor(e3Id).getE3(e3Id);
        ...
        _loxleyFor(e3Id).onCommitteePublished(e3Id, pkCommitment);
    ...
    function finalizeCommittee(uint256 e3Id) external returns (bool success) {
        ...
        _loxleyFor(e3Id).onCommitteeFinalized(e3Id);
    ...
    function setLoxley(ILoxley _loxley) public onlyOwner {
        require(address(_loxley) != address(0), ZeroAddress());
        _requireGenerationDrained(address(loxley));
        loxley = _loxley;
        emit LoxleySet(address(_loxley));
    }
