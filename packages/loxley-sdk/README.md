...
- **React integration**: Includes React hooks for easy frontend integration (via `@loxley/react`)
...
```typescript
import { LoxleySDK, LoxleyEventType, RegistryEventType } from '@loxley/sdk'
...
const sdk = new LoxleySDK({
  ...
  walletClient,
  contracts: {
    loxley: '0x...', // Your Loxley contract address
...
sdk.onLoxleyEvent(LoxleyEventType.E3_REQUESTED, (event) => {
  console.log('E3 Requested:', event.data)
})

sdk.onLoxleyEvent(RegistryEventType.COMMITTEE_REQUESTED, (event) => {
...
For a simpler setup (especially on the server), use the static `LoxleySDK.create()` factory:
...
- Exclude the `@loxley/wasm` package from bundling optimization.
...
```typescript
interface LoxleyEvent<T extends AllEventTypes> {
  type: T
...
The SDK includes a React hook via the `@loxley/react` package:

```bash
pnpm add @loxley/react
```
...
function MyComponent() {
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      onLoxleyEvent(LoxleyEventType.E3_REQUESTED, handler)
      return () => off(LoxleyEventType.E3_REQUESTED, handler)
    }
  }, [isInitialized])
...
import { generatePublicKey, encryptNumber } from '@loxley/sdk/crypto'

// Contract client and types
import { ContractClient } from '@loxley/sdk/contracts'
import type { ContractAddresses, E3 } from '@loxley/sdk/contracts'
...
```typescript
sdk.onLoxleyEvent(eventType: AllEventTypes, callback: EventCallback);
...
await sdk.onLoxleyEvent(RegistryEventType.COMMITTEE_PUBLISHED, async (event) => {
...
on-chain `paramSet` index registered in the Loxley contract:
...
```typescript
import { SDKError } from '@loxley/sdk'
...
- **LoxleySDK** (`loxley-sdk.ts`): Main orchestrator class that delegates to sub-modules
