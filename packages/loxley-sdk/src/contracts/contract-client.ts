...
import { CiphernodeRegistryOwnable__factory, Loxley__factory, LoxleyToken__factory } from '@loxley/contracts/types'
...
export class ContractClient {
  ...
  private contractInfo: {
    loxley: { address: `0x${string}`; abi: Abi }
  ...
  constructor(config: ContractClientConfig) {
    ...
    if (!isValidAddress(contracts.loxley)) {
      throw new SDKError('Invalid Loxley contract address', 'INVALID_ADDRESS')
    }
  ...
  public async approveFeeToken(amount: bigint): Promise<Hash> {
    ...
    try {
      ...
      const { request } = await this.publicClient.simulateContract({
        ...
        args: [this.contracts.loxley, amount],
        account,
      })
