...
/** Matches `ILoxley.CommitteeSize.Minimum` and `DEFAULT_E3_CONFIG.committeeSize`. */
...
async function assertNodeCircuits(): Promise<void> {
  ...
  try {
    ...
    throw new SDKError(
      `Missing ${activePresetPath}. Run \`pnpm -C packages/loxley-sdk compile:circuits\` first.`,
  ...
  try {
    ...
    throw new SDKError(
      `Could not parse ${activePresetPath} — run \`pnpm -C packages/loxley-sdk compile:circuits\`.`,
