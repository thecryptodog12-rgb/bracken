...
import { CiphernodeRegistryOwnable__factory, Loxley__factory } from '@loxley/contracts/types'

import {
  RegistryEventType,
  type AllEventTypes,
  type LoxleyEvent,
  type LoxleyEventData,
  type LoxleyEventType as LoxleyEventTypeT,
  type EventCallback,
...
export class EventListener implements SDKEventEmitter {
  ...
  // Registry-exclusive event names that don't collide with LoxleyEventType.
  ...
  // runtime; those default to the Loxley contract.
  ...
  private resolveContract(eventType: AllEventTypes): { address: `0x${string}`; abi: Abi } {
    ...
    return {
      address: isRegistryEvent ? this.contracts.ciphernodeRegistry : this.contracts.loxley,
      abi: isRegistryEvent ? CiphernodeRegistryOwnable__factory.abi : Loxley__factory.abi,
    }
  }

  public async onLoxleyEvent<T extends AllEventTypes>(eventType: T, callback: EventCallback<T>): Promise<void> {
  ...
  public async once<T extends AllEventTypes>(type: T, callback: EventCallback<T>): Promise<void> {
    ...
    return this.onLoxleyEvent(type, handler)
  }
        ...
        const unwatch = this.publicClient.watchContractEvent({
          ...
          onLogs(logs: Log[]) {
            for (let i = 0; i < logs.length; i++) {
              const log = logs[i]
              if (!log) break
              const event: LoxleyEvent<T> = {
                type: eventType,
                data: (log as unknown as { args: unknown }).args as T extends LoxleyEventTypeT
                  ? LoxleyEventData[T]
  ...
  public emit<T extends AllEventTypes>(event: LoxleyEvent<T>): void {
