# Bracken Smart Contracts

## Contract Overview

| Contract                        | Description                                                                                      |
| ------------------------------- | ------------------------------------------------------------------------------------------------ |
| `Bracken.sol`                    | Main protocol coordinator — handles E3 requests, param sets, fee routing, and output publication |
| `CiphernodeRegistryOwnable.sol` | Ciphernode registration and committee selection                                                  |
| `BondingRegistry.sol`           | BRACKEN token bonding for ciphernodes; tracks bond amounts and manages bond lifecycle                |
| `BrackenToken.sol`               | BRACKEN governance/utility token                                                                     |
| `BrackenTicketToken.sol`         | Collateral-backed tickets used by ciphernodes for sortition entry                                |
...
| `IBracken`          | External interface to the main Bracken contract                                |
...
import {
    IE3Program,
} from "@bracken/contracts/contracts/interfaces/IE3Program.sol";
...
Phase 1 deploys BRACKEN plus the CCA sale:

```sh
pnpm sale --network sepolia --action prepare --safe 0xSafe
pnpm sale --network sepolia --action plan --config packages/bracken-contracts/deploy/sale/sepolia-sale.config.json
pnpm sale --network sepolia --action deploy --config packages/bracken-contracts/deploy/sale/sepolia-sale.config.json --propose-safe
pnpm sale --network sepolia --action validate --config packages/bracken-contracts/deploy/sale/sepolia-sale.config.json --allow-pending-owner
```
...
plain sale this is `BRACKEN.acceptOwnership()`; for a Predicate-gated sale the same
...
```sh
pnpm protocol --network sepolia --action check-config --config packages/bracken-contracts/deploy/protocol/sepolia-protocol.config.json
...
setters, or reentrancy hooks. Bracken still verifies each BFV ciphertext proof
and committee decryption proof. Do not set `bindInitialE3Program` for this
option.

`Bracken.initialize` registers the selected program before the governance
...
The canonical outputs live under `packages/bracken-contracts/deploy/`. The
...
If you are running Bracken locally, you can first start a local hardhat (or
...
pnpm protocol --network localhost --action deploy --config packages/bracken-contracts/deploy/protocol/localhost-protocol.config.json --sync-integration-config
```
...
- `--ciphernode-bond-amount`: Amount of BRACKEN to bond (default: 1000 BRACKEN)
