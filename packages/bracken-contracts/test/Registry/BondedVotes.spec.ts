...
/// Bonded BRACKEN is transferred to `BondingRegistry`, which never delegates it. Under ERC20Votes an
...
describe("BondedVotes", function () {
  ...
  async function setup() {
    ...
    const bondedVotes = await ethers.deployContract("BondedVotes", [
      ...
      // Votes source == the token: wallet-held BRACKEN votes, the original behaviour.
      await ciphernodeBondToken.getAddress(),
  ...
  describe("holders who never bond", function () {
    it("counts wallet BRACKEN exactly as the token does", async function () {
  ...
  describe("bonding", function () {
    ...
    it("counts an owner whose BRACKEN is entirely bonded", async function () {
  ...
  describe("unbonding and exit", function () {
    /// Unbonding queues the bond for exit. The BRACKEN is still held by the registry, so the weight
    /// stays with the owner — counting it as wallet BRACKEN as well would double it.
    ...
    it("moves power back to the wallet once the exit is claimed", async function () {
      ...
      // Unchanged throughout: the BRACKEN only ever moved between wallet and registry.
      expect(await settledVotes(bondOwnerAddress)).to.equal(MINTED);
  ...
  describe("slashing", function () {
    it("reduces power by the slashed amount", async function () {
      ...
      // Slashed BRACKEN is gone from the owner's control, so its weight goes with it.
  ...
  describe("history", function () {
    ...
    /// or an owner could hold BRACKEN at the snapshot, bond it afterwards, and be counted twice.
    ...
    it("reports timestamps as its clock", async function () {
      ...
      // Must match BrackenToken, or the adapter sums two unrelated points in time.
  ...
  describe("delegation", function () {
    /// Wallet BRACKEN follows the token's delegation. Bonded BRACKEN cannot be delegated — the registry
  ...
  describe("total supply", function () {
    /// Bonded BRACKEN was transferred, not burned, so it is already in the supply. Adding it again
    ...
    /// BRACKEN twice is the failure this whole design exists to avoid.
  ...
  describe("Aragon plugin compatibility", function () {
    ...
    it("counts wallet and bonded BRACKEN in the balance, ignoring delegation", async function () {
    ...
    it("keeps total supply a pass-through so bonded BRACKEN is never counted twice", async function () {
      const { bondedVotes, ciphernodeBondToken, bond } =
        await loadFixture(setup);
      ...
      // Bonding moves BRACKEN to the registry; it is not burned, so supply is unchanged.
  ...
  describe("audit regressions", function () {
    ...
    it("refuses a history whose registry bonds a different token", async function () {
      ...
      // Registry is an EOA here — it custodies no BRACKEN and cannot answer for a ciphernode bond token.
    ...
    /// Counting it at both addresses put the same BRACKEN in two places, so summed balances exceeded
  ...
  describe("wiring", function () {
    it("rejects a checkpoint contract bound to another registry", async function () {
      ...
      const { bondingRegistry } = await deployBrackenSystem({
        useMockCiphernodeRegistry: true,
        setupOperators: 0,
        wireSlashingManager: false,
        mintUsdcTo: [],
    ...
    it("rejects an address that names this registry but cannot record history", async function () {
      // `registry()` is not unique to a checkpoint contract: BrackenTicketToken answers it with
      ...
      const { bondingRegistry, ticketToken } = await deployBrackenSystem({
        useMockCiphernodeRegistry: true,
        setupOperators: 0,
        wireSlashingManager: false,
        mintUsdcTo: [],
  ...
  /// into the same history and be counted as BRACKEN, so an operator could hold voting power the
  ...
  describe("ciphernode-bond-token rotation", function () {
    ...
    it("keeps a replacement token's bonds out of the previous token's voting power", async function () {
      ...
      // Bonded in a different token, so it must not add to BRACKEN voting power. Before the
      // detachment this bond entered the BRACKEN history and pushed the owner's votes above what
      // BRACKEN's own supply backs.
  ...
  /// Lock-to-vote. `votesSource` is an escrow adapter instead of the token, so idle wallet BRACKEN
  ...
  ///
  /// The property that makes this sound is that the DENOMINATOR stays on BRACKEN. Locked and bonded
  /// BRACKEN were both transferred rather than burned, so both are still inside BRACKEN's total supply
  ...
  describe("escrow votes source", function () {
    ...
    it("counts locked BRACKEN and bonded BRACKEN, but not idle wallet BRACKEN", async function () {
    ...
    it("gives a holder who locked nothing and bonded nothing no voting power", async function () {
      ...
      // Holds MINTED BRACKEN and has self-delegated on the token. Under lock-to-vote that is
    ...
    it("measures quorum against BRACKEN's supply, not the escrow's", async function () {
      const {
        veBondedVotes,
        adapter,
        ciphernodeBondToken,
    ...
    it("reads metadata from BRACKEN, which the escrow adapter cannot answer for", async function () {
    ...
    it("attributes locked BRACKEN to the locker in balanceOf", async function () {
      const {
        veBondedVotes,
        adapter,
        escrow,
    ...
    /// The mirror of the registry netting. The escrow custodies real BRACKEN and every unit of it is
    ...
    it("nets the escrow down to nothing in balanceOf", async function () {
      ...
      // Stands in for BRACKEN deposited by lockers: the escrow holds it, and it is attributed to the
    ...
    /// over a different token would mint voting power BRACKEN's supply does not back, and the
    ...
    /// BRACKEN's own vesting locks, the third source under an escrow votes source.
    ///
    /// Lock-encumbered BRACKEN sits in the holder's wallet and the token's transfer hook refuses to
    ...
    describe("vesting-locked BRACKEN", function () {
      ...
      it("counts locked BRACKEN for a holder who escrowed and bonded nothing", async function () {
      ...
      it("adds locked BRACKEN to escrowed and bonded BRACKEN", async function () {
        const {
          veBondedVotes,
          ciphernodeBondToken,
          adapter,
        ...
        // is BRACKEN the wallet is still holding and still cannot escrow.
        expect(
          await veBondedVotes.getPastVotes(bondOwnerAddress, timepoint),
      ...
      /// lock — {BrackenToken.transferableBalanceOf} nets the bond against the obligation — so
      /// bonded BRACKEN is reported by both `lockedBalanceAt` and the bonded history while existing
      /// exactly once. Summing both would let an operator vote twice with the same token.
      it("does not count BRACKEN twice when the bond covers the lock", async function () {
      ...
      it("refuses an escrow votes source over a token with no lock schedule", async function () {
        ...
        // Control: the same wiring over BRACKEN, which does answer, constructs. Without this the
      ...
      /// the bond without touching the lock — so an operator that had already moved locked BRACKEN
      /// out on the strength of that bond is left owing more than it holds. Uncapped, the locked
      /// term would vote with BRACKEN that is now in the slash recipient's hands and countable
      /// there too.
      it("stops counting locked BRACKEN a slash left unbacked", async function () {
        ...
        // A wallet holding exactly one lock and an equal amount of free BRACKEN.
        ...
        // allocation — but the wallet holds none of it, and the slashed BRACKEN votes elsewhere.
        expect(
      ...
      /// unbonded part of its lock, and never reaches into BRACKEN the lock does not encumber.
      ...
      /// When the token votes for itself, locked BRACKEN is wallet BRACKEN and the token's own
