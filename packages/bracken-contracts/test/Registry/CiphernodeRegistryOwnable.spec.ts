...
describe("CiphernodeRegistryOwnable", function () {
  ...
  // Helper to make a request through the Bracken contract
  ...
  describe("constructor / initialize()", function () {
    it("correctly sets `_owner` and `bracken` ", async function () {
  ...
  describe("requestCommittee()", function () {
    ...
    it("AUD-M03: fails closed after governance updates until operators refresh", async function () {
      ...
      await expect(
        makeRequest(bracken, usdcToken, mockE3Program, mockDecryptionVerifier),
      )
  ...
  describe("setBracken()", function () {
    it("reverts if the caller is not the owner", async function () {
      const { registry, notTheOwner } = await loadFixture(setup);
      await expect(
        registry.connect(notTheOwner).setBracken(AddressTwo),
      ).to.be.revertedWithCustomError(registry, "OwnableUnauthorizedAccount");
    });
    it("sets the bracken address", async function () {
      const { ciphernodeRegistry: registry } = await deployBrackenSystem({
        setupOperators: 0,
      });
      expect(await registry.setBracken(AddressTwo));
      expect(await registry.bracken()).to.equal(AddressTwo);
    });
    it("emits an BrackenSet event", async function () {
      const { ciphernodeRegistry: registry } = await deployBrackenSystem({
        setupOperators: 0,
      });
      await expect(await registry.setBracken(AddressTwo))
        .to.emit(registry, "BrackenSet")
        .withArgs(AddressTwo);
    });
  });
  ...
  describe("exit timing", function () {
    ...
    it("keeps exit claims behind request-time committee deadlines", async function () {
      ...
      await bracken.setTimeoutConfig({
        dkgWindow: ONE_DAY,
