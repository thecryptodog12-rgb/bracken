...
// Full Bracken system deployment used by spec files. Composes the existing
// ignition modules + token/registry/slashing wiring + (optional) operator
// onboarding into one entry point: `deployBrackenSystem(opts?)`.
...
import BrackenTicketTokenModule from "../../ignition/modules/brackenTicketToken";
import BrackenTokenModule from "../../ignition/modules/brackenToken";
...
import type { BrackenTicketToken } from "../../types/contracts/token/BrackenTicketToken";
import type { BrackenToken } from "../../types/contracts/token/BrackenToken";
...
/** Timeout configuration accepted by `Bracken`. */
...
/**
 * `[CommitteeSize enum value, [M, N]]` passed to `Bracken.setCommitteeThresholds`.
...
/** Options accepted by {@link deployBrackenSystem}. All optional. */
export interface DeployBrackenSystemOptions {
  /** Override the sortition submission window (seconds). */
  submissionWindow?: number;
  /** Override `Bracken.maxDuration` (seconds). */
  maxDuration?: number;
   ...
   *  - `slashingManager.{setCiphernodeRegistry,setBracken,setE3RefundManager}`
   *
  ...
  /** Program registered atomically by `Bracken.initialize`. */
...
/** Mock contract bundle returned by {@link deployBrackenSystem}. */
export interface BrackenSystemMocks {
...
/** Bundle returned by {@link deployBrackenSystem}. */
export interface BrackenSystem {
  // Core
  bracken: Bracken;
  brackenLifecycle: string;
  brackenPricing: string;
  ...
  ticketToken: BrackenTicketToken;
  usdcToken: MockUSDC;
  // Mocks
  mocks: BrackenSystemMocks;
  // Signers
  owner: Signer;
  notTheOwner: Signer;
  operators: Signer[];
  ...
  /** Default `Bracken.request(...)` params anchored at the fixture's `time.latest()`. */
  request: IBracken.E3RequestParamsStruct;
}

/**
 * Deploy a fully-wired Bracken system and return typed handles. Call from a
 ...
 * production deploy (`scripts/deployBracken.ts`) uses `[H, N]` instead — same
 * storage slot, different first component for Minimum (T=1 vs H=2).
 */
export async function deployBrackenSystem(
  opts: DeployBrackenSystemOptions = {},
): Promise<BrackenSystem> {
  ...
  // Deferred: BrackenToken is deployed after BondingRegistry so the
  // immutable BONDING_REGISTRY reference can be set. See below.

  const { brackenTicketToken } = await ignition.deploy(BrackenTicketTokenModule, {
    parameters: {
      BrackenTicketToken: {
  ...
  const ticketToken = BrackenTicketTokenFactory.connect(
    await brackenTicketToken.getAddress(),
    owner,
  );
  ...
  // ── BrackenToken (deployed after BondingRegistry for immutable ref) ──
  ...
  const { brackenToken } = await ignition.deploy(BrackenTokenModule, {
    parameters: {
      BrackenToken: {
        owner: ownerAddress,
        ccaStart,
  ...
  const ciphernodeBondToken = BrackenTokenFactory.connect(
    await brackenToken.getAddress(),
    owner,
  );
  ...
  // Deploy the default program before Bracken so initialization can validate it.
  ...
  const {
    bracken: _bracken,
    brackenLifecycle: _brackenLifecycle,
    brackenPricing: _brackenPricing,
  } = await ignition.deploy(BrackenModule, {
    parameters: {
      Bracken: {
        owner: ownerAddress,
        maxDuration,
  ...
  const brackenAddress = await _bracken.getAddress();
  const bracken = BrackenFactory.connect(brackenAddress, owner);
  const brackenLifecycle = await _brackenLifecycle.getAddress();
  const brackenPricing = await _brackenPricing.getAddress();
  await e3Program.setBracken(brackenAddress);
  ...
  await bracken.setE3RefundManager(e3RefundManagerAddress);

  // ── Wire base contracts ───────────────────────────────────────────────────
  const registryAddress = await bracken.ciphernodeRegistry();
  if (registryAddress !== effectiveRegistryAddress) {
    await bracken.setCiphernodeRegistry(effectiveRegistryAddress);
  }
  // `setBracken` / `setBondingRegistry` are present (matching selectors) on
  ...
  await registryForWiring.setBracken(brackenAddress);
  ...
  await bondingRegistry.setRewardDistributor(brackenAddress);

  if (wireSlashingManager) {
    await bracken.setSlashingManager(await slashingManager.getAddress());
    await registryForWiring.setSlashingManager(
      await slashingManager.getAddress(),
    );
    await slashingManager.setCiphernodeRegistry(effectiveRegistryAddress);
    await slashingManager.setBracken(brackenAddress);
    await slashingManager.setE3RefundManager(e3RefundManagerAddress);
  }

  if (wireSlashingManager) {
    await bracken.setRequestsPaused(false);
  }
  ...
  if (!(await bracken.e3Programs(await e3Program.getAddress()))) {
    await bracken.registerE3Program(await e3Program.getAddress());
  }
  await bracken.setParamSet(0, BFV_PARAMS_DEFAULT);
  await bracken.setDecryptionVerifier(
    ENCRYPTION_SCHEME_ID,
    await decryptionVerifier.getAddress(),
  );
  await bracken.setPkVerifier(
    ENCRYPTION_SCHEME_ID,
  ...
  for (const [size, [m, n]] of committeeThresholds) {
    await bracken.setCommitteeThresholds(size, [m, n]);
  }
  ...
  const request: IBracken.E3RequestParamsStruct = {
