...
// ── Committee sizes (matches `IBracken.CommitteeSize`) ─────────────────────
...
/**
 * Production `setCommitteeThresholds` values from `scripts/deployBracken.ts`:
 * `[H, N]` (minimum honest roster, committee size). On-chain `threshold[0]`
 * is registry viability **M** (`activeCount >= M`); production sets M = H.
 *
 * Pass via `deployBrackenSystem({ committeeThresholds: [...] })` when a
