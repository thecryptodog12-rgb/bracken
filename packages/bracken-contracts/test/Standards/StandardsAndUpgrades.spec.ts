...
//     Bracken, registries or the refund manager. Add an IERC1271 check if
...
import {
  ...
  IBracken__factory as IBrackenFactory,
  ISlashingManager__factory as ISlashingManagerFactory,
} from "../../types";
import { deployBrackenSystem, ethers } from "../fixtures";
...
describe("Standards & upgradeability hygiene", function () {
  describe("ERC-165 supportsInterface", function () {
    it("Bracken: supports IBracken + IERC165, rejects 0xffffffff", async function () {
      const { bracken } = await deployAll();
      const iBrackenId = interfaceIdOf(IBrackenFactory.createInterface());
      expect(await bracken.supportsInterface(iBrackenId)).to.equal(true);
      expect(await bracken.supportsInterface(IERC165_ID)).to.equal(true);
      expect(await bracken.supportsInterface(INVALID_ID)).to.equal(false);
    });
  ...
  describe("EIP-6372 clock advertisement (ERC20Votes)", function () {
    it("BrackenTicketToken: CLOCK_MODE() and clock() report timestamp mode", async function () {
    ...
    it("BrackenToken: CLOCK_MODE() and clock() report timestamp mode", async function () {
  ...
  describe("storage gaps on upgradeable contracts", function () {
    ...
    it("all upgradeable contracts deploy without storage collision", async function () {
      ...
      expect(await all.bracken.getAddress()).to.properAddress;
  ...
  describe("Bracken library links", function () {
    it("deployment links the lifecycle and pricing libraries", async function () {
      const all = await deployAll();
      const proxy = await all.bracken.getAddress();
      ...
      expectLibraryLinks(bytecode, all.brackenLifecycle, all.brackenPricing);
    });

    it("upgrade links new lifecycle and pricing library addresses", async function () {
      const all = await deployAll();
      const lifecycle = await ethers.deployContract("BrackenLifecycle");
      const pricing = await ethers.deployContract("BrackenPricing");
      ...
      const brackenFactory = await ethers.getContractFactory("Bracken", {
        libraries: {
          BrackenLifecycle: lifecycleAddress,
          BrackenPricing: pricingAddress,
        },
      });
      const implementation = await brackenFactory.deploy();
