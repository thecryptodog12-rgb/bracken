...
describe("SlashingManager", function () {
  ...
  async function setup() {
    ...
    await mockBracken.snapshotDependencies(slashingManager, 1, now + 3600);
  ...
  describe("setSlashPolicy()", function () {
    ...
    it("requires policies to be refreshed after an asset rotation", async function () {
      ...
      await networkHelpers.setBalance(brackenAddress, ethers.parseEther("1"));
      await networkHelpers.impersonateAccount(brackenAddress);
      await slashingManager
        .connect(await ethers.getSigner(brackenAddress))
        .snapshotE3Dependencies(42);
      await networkHelpers.stopImpersonatingAccount(brackenAddress);
