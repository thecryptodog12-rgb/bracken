...
describe("Protocol deployment", function () {
  ...
  it("rejects a zero protocol owner and accepts a missing-owner override", function () {
    ...
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "bracken-protocol-config-"),
    );
  ...
  it("rejects config names that cannot be used as deployment file names", function () {
    ...
    const config = JSON.parse(fs.readFileSync(source, "utf8"));
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "bracken-protocol-config-name-"),
    );
    const configFile = path.join(tempDir, "protocol.json");
  ...
  it("normalizes the optional escrow votes adapter", function () {
    ...
    const config = JSON.parse(fs.readFileSync(source, "utf8"));
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "bracken-protocol-escrow-votes-"),
    );
    const configFile = path.join(tempDir, "protocol.json");
  ...
  it("rejects external wiring for the deployed MockE3Program", function () {
    ...
    const config = JSON.parse(fs.readFileSync(source, "utf8"));
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "bracken-mock-program-config-"),
    );
    const configFile = path.join(tempDir, "protocol.json");
  ...
  it("rejects configuring and deploying a ciphertext verifier at the same time", function () {
    ...
    const config = JSON.parse(fs.readFileSync(source, "utf8"));
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "bracken-mock-ciphertext-config-"),
    );
    const configFile = path.join(tempDir, "protocol.json");
  ...
  it("uses separate fee and ticket collateral tokens", async function () {
    ...
    // below are fed into further deployments, and Bracken rejects a program
    ...
    // BRACKEN has to be a real votes token: the deployment builds `BondedVotes` against it, and that
    ...
    config.bracken.pricing.protocolTreasury = await safe.getAddress();
    config.verifiers = {
      deploy: false,
    ...
    const bracken = await ethers.getContractAt(
      "Bracken",
      result.contracts.bracken,
    );
    ...
    expect(await bracken.feeToken()).to.equal(await feeToken.getAddress());
    expect(await bracken.e3Programs(result.contracts.initialE3Program)).to.equal(
      true,
    );
    ...
    const ciphertextSelector = bracken.interface.getFunction(
      "setCiphertextVerifier",
    )!.selector;
    const ciphertextTx = txs.filter(
      (tx) =>
        tx.to.toLowerCase() === result.contracts.bracken.toLowerCase() &&
