...
describe("E3 Pricing", function () {
  ...
  describe("getE3Quote()", function () {
    ...
    it("computes fee correctly using the parametric formula", async function () {
      ...
      // Get pricing config
      const pc = await bracken.getPricingConfig();

      // Get timeout config
      const config = await bracken.getTimeoutConfig();
      ...
      const actualFee = await bracken.getE3Quote(request);
    ...
    it("charges each required decryption share", async function () {
      ...
      const feeWithoutDecryption = await bracken.getE3Quote(request);
      ...
      const feeWithDecryption = await bracken.getE3Quote(request);
    ...
    it("fee increases with longer input window", async function () {
      ...
      const shortFee = await bracken.getE3Quote(request);
      ...
      const longFee = await bracken.getE3Quote(longRequest);
    ...
    it("charges for an equal-length input window scheduled later", async function () {
      ...
      const nearFee = await bracken.getE3Quote({
        ...request,
        inputWindow: [now + 10, now + 10 + windowLength] as [number, number],
      });
      const delayedFee = await bracken.getE3Quote({
        ...request,
    ...
    it("fee reflects margin changes", async function () {
      ...
      const fee10Pct = await bracken.getE3Quote(request);

      // Set margin to 20%
      const pc = toPlainConfig(await bracken.getPricingConfig());
      await setPricingConfig(bracken, { ...pc, marginBps: 2000 });
      const fee20Pct = await bracken.getE3Quote(request);

      expect(fee20Pct).to.be.gt(fee10Pct);

      // Set margin to 0%
      const pc2 = toPlainConfig(await bracken.getPricingConfig());
      await setPricingConfig(bracken, { ...pc2, marginBps: 0 });
      const feeZero = await bracken.getE3Quote(request);
  ...
  describe("setFeeAssetConfig()", function () {
    it("reverts if not called by owner", async function () {
      ...
      await expect(
        setPricingConfig(bracken.connect(notTheOwner), defaultPricingConfig),
    ...
    it("updates config and emits event", async function () {
      ...
      await expect(setPricingConfig(bracken, newConfig)).to.emit(
        bracken,
        "FeeAssetConfigUpdated",
      );

      const stored = await bracken.getPricingConfig();
    ...
    it("updates the token scale and raw-unit prices together", async function () {
      ...
      await expect(
        ...
        .to.be.revertedWithCustomError(bracken, "FeeTokenDecimalsMismatch")
      ...
      expect(await bracken.feeToken()).to.equal(tokenAddress);
      expect(await bracken.feeTokenDecimals()).to.equal(18);
      expect((await bracken.getPricingConfig()).publicationBase).to.equal(
        ethers.parseEther("1"),
      );
    });
    ...
    it("changes the fee returned by getE3Quote", async function () {
      ...
      const feeBefore = await bracken.getE3Quote(request);
      ...
      const feeAfter = await bracken.getE3Quote(request);
      expect(feeAfter).to.be.gt(feeBefore);
    });

    it("enforces the public margin cap", async function () {
      const { bracken } = await loadFixture(setup);
      const cap = await bracken.MAX_MARGIN_BPS();
      await expect(
        setPricingConfig(bracken, {
          ...defaultPricingConfig,
          marginBps: cap,
        }),
      ...
      ).to.be.revertedWithCustomError(bracken, "BpsExceedsMax");
    });
    ...
    it("enforces the public protocol-share cap", async function () {
      const { bracken, treasury } = await loadFixture(setup);
      const cap = await bracken.MAX_PROTOCOL_SHARE_BPS();
    ...
    it("reverts if minCommitteeSize < minThreshold", async function () {
      ...
      ).to.be.revertedWithCustomError(bracken, "MinSizeBelowMinThreshold");
    });

    it("accepts only circuit-backed committee configurations", async function () {
      const { bracken } = await loadFixture(setup);

      expect(await bracken.MAX_COMMITTEE_SIZE()).to.equal(3);
      await (await bracken.setCommitteeThresholds(0, [2, 3])).wait();
      await expect(
        bracken.setCommitteeThresholds(0, [1, 3]),
      ).to.be.revertedWithCustomError(bracken, "UnsupportedCryptoConfig");
      await expect(
        bracken.setCommitteeThresholds(1, [5, 9]),
      ).to.be.revertedWithCustomError(bracken, "UnsupportedCryptoConfig");
      await expect(
        bracken.setCommitteeThresholds(2, [10, 19]),
      ).to.be.revertedWithCustomError(bracken, "UnsupportedCryptoConfig");
    });
    ...
    it("applies pricing minimums to canonical committee configurations", async function () {
      ...
      await expect(
        bracken.setCommitteeThresholds(0, [2, 3]),
      ).to.be.revertedWithCustomError(bracken, "BelowMinCommitteeSize");
      ...
      await expect(
        bracken.setCommitteeThresholds(0, [2, 3]),
      ).to.be.revertedWithCustomError(bracken, "BelowMinThreshold");
  ...
  describe("Protocol treasury share on success", function () {
    it("sends 100% to CNs when protocolShareBps is 0 (default)", async function () {
      ...
      await bracken.request(freshRequest);
      const e3Id = firstE3Id;
      const fee = await bracken.e3Payments(e3Id);
      ...
      expect(await bracken.pendingReward(e3Id, bondOwner)).to.equal(fee);
      expect(await bracken.pendingReward(e3Id, newOwnerAddress)).to.equal(0);
  ...
  describe("End-to-end request with parametric pricing", function () {
    it("charges the computed fee and completes successfully", async function () {
      const { bracken, usdcToken, request, owner } = await loadFixture(setup);

      const fee = await bracken.getE3Quote(request);
      const ownerAddr = await owner.getAddress();
      const balanceBefore = await usdcToken.balanceOf(ownerAddr);

      await usdcToken.approve(await bracken.getAddress(), fee);
    ...
    it("reverts if USDC allowance is less than computed fee", async function () {
      const { bracken, usdcToken, request } = await loadFixture(setup);

      // Approve only 1 unit
      await usdcToken.approve(await bracken.getAddress(), 1);

      await expect(bracken.request(request)).to.be.revertedWithCustomError(
        usdcToken,
  ...
  describe("M-06 — duration precision", function () {
    it("sums weighted timeouts before dividing by BPS_BASE", async function () {
      const { bracken, request, ciphernodeRegistryContract, owner } =
        await loadFixture(setup);
      ...
      await bracken.connect(owner).setTimeoutConfig({
        dkgWindow: 2,
        computeWindow: 2,
        decryptionWindow: 2,
      });
      ...
      const pc = await bracken.getPricingConfig();
      await setPricingConfig(bracken.connect(owner), {
        ...toPlainConfig(pc),
      ...
      const pc2 = await bracken.getPricingConfig();
      const n = 3n;
      const h = 2n;
      const proofsPerNode = 14n + 4n * (n - 1n);
