...
import {
  ...
  BrackenTokenSaleDeployer__factory as SaleDeployerFactory,
} from "../../types";
...
const SALE_AMOUNT = ethers.parseEther("120000000"); // 120M BRACKEN
const LP_RESERVE = ethers.parseEther("1000000"); // 1M BRACKEN
...
function auctionAt(
  address: string,
  runner: Parameters<typeof BrackenTokenFactory.connect>[1],
) {
  const abi = [
...
describe("BrackenTokenSaleDeployer", function () {
  ...
  it("deploys BRACKEN + CCA through LiquidityLauncher/LBPStrategy without a predicted auction", async function () {
    const ctx = await setup();
    const plan = await buildLbpSaleConfig(ctx);
    ...
    const fold = BrackenTokenFactory.connect(foldAddress, ctx.operator);
  ...
  it("hands BRACKEN ownership to the Safe, which sets CLAIM_SOURCE once", async function () {
    const ctx = await setup();
    const plan = await buildLbpSaleConfig(ctx);
    const { foldAddress, auctionAddress } = await deploySale(ctx, plan);
    const fold = BrackenTokenFactory.connect(foldAddress, ctx.operator);
