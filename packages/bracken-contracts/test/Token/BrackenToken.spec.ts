...
import { deployBrackenSystem, ethers, networkHelpers } from "../fixtures";
...
describe("BrackenToken", function () {
  // ── Helpers ─────────────────────────────────────────────────────────────

  /// Deploy a minimal MockBondingRegistry + BrackenToken for standalone tests.
  ...
  async function deploy() {
    ...
    const token = await new BrackenTokenFactory(deployer).deploy(
  ...
  describe("constructor", function () {
    ...
    it("reverts when bondingRegistry is zero address", async function () {
      ...
      await expect(
        new BrackenTokenFactory(deployer).deploy(
          await deployer.getAddress(),
          ccaStart,
          ccaEnd,
          noMoreLocks,
        ...
        { interface: BrackenTokenFactory.createInterface() },
        "ZeroAddress",
      );
    });
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // BondingRegistry integration (uses deployBrackenSystem)
  // ═════════════════════════════════════════════════════════════════════════
  ...
  describe("BondingRegistry integration", function () {
    ...
    it("transferableBalanceOf counts bonded BRACKEN toward the locked floor", async function () {
      const signers = await ethers.getSigners();
      const [, beneficiary, , operator] = signers;
      const beneficiaryAddress = await beneficiary.getAddress();
      const operatorAddress = await operator.getAddress();
    ...
    it("blocks bond-owner rotation that would detach locked BRACKEN", async function () {
      const signers = await ethers.getSigners();
