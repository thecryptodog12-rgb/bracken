...
describe("BrackenTicketToken", function () {
  async function deploy() {
    ...
    const token = await new BrackenTicketTokenFactory(deployer).deploy(
  ...
  describe("H-02 — registry initialization", function () {
    ...
    it("constructor emits RegistryChanged(0, registry_)", async function () {
      ...
      const factory = new BrackenTicketTokenFactory(deployer);
