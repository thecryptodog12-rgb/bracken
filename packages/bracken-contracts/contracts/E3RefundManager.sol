...
contract E3RefundManager is IE3RefundManager, Ownable2StepUpgradeable {
    ...
    /// @notice Emitted whenever {bracken} is updated.
    event BrackenUpdated(address indexed previous, address indexed next);
    ...
    //                                                        //
    ////////////////////////////////////////////////////////////
    /// @notice Bracken contract (contains lifecycle functionality)
    ...
    //                                                        //
    ////////////////////////////////////////////////////////////
    /// @notice Restricts function to Bracken contract only
    modifier onlyBracken() {
        if (msg.sender != address(bracken)) revert Unauthorized();
        _;
    }

    /// @dev Authorizes the Bracken contract frozen for an existing E3.
    modifier onlyE3Bracken(uint256 e3Id) {
        E3PolicySnapshot storage policy = _e3PolicySnapshots[e3Id];
        if (!policy.initialized || msg.sender != policy.bracken) {
            revert Unauthorized();
        }
        _;
    }
    ...
    /// @param _bracken Bracken contract address
    ...
    function initialize(
        ...
        require(_bracken != address(0), "Invalid bracken");
        require(_treasury != address(0), "Invalid treasury");

        bracken = IBracken(_bracken);
        bondingRegistry = bracken.bondingRegistry();
        treasury = _treasury;
    ...
    function calculateRefund(
        ...
        IBracken.FailureReason reason = _brackenFor(e3Id).getFailureReason(e3Id);
        (
        ...
        // {Bracken._distributeRewards} success-path).
        ...
        )
    {
        ...
        IBracken.E3Stage failedAt = reason ==
    ...
    function _getFailedAtStage(
        IBracken.FailureReason reason
    ) internal pure returns (IBracken.E3Stage) {
        ...
        if (
            ...
            return IBracken.E3Stage.Requested;
        }
        if (
            reason == IBracken.FailureReason.DKGTimeout ||
            reason == IBracken.FailureReason.DKGInvalidShares
        ) {
            return IBracken.E3Stage.CommitteeFinalized;
        }
        if (reason == IBracken.FailureReason.NoInputsReceived) {
        ...
        if (
            ...
            return IBracken.E3Stage.CiphertextReady;
        }

        return IBracken.E3Stage.None;
    }

    /// @notice Derives the cancellation stage from monotonic lifecycle markers.
    function _getCancellationStage(
        uint256 e3Id
    ) internal view returns (IBracken.E3Stage) {
        IBracken.E3Deadlines memory deadlines = _brackenFor(e3Id).getDeadlines(
            e3Id
        );
    ...
    function _calculateWorkValue(
        ...
        if (
            stage == IBracken.E3Stage.Requested || stage == IBracken.E3Stage.None
        ) {
    ...
    function claimRequesterRefund(
        ...
        address requester = _brackenFor(e3Id).getRequester(e3Id);
    ...
    function snapshotE3Policy(
        ...
        snapshot.bracken = msg.sender;
        snapshot.registry = registry;
        snapshot.bondingRegistry = address(
            IBracken(msg.sender).bondingRegistry()
        );
        snapshot.slashingManager = address(
            IBracken(msg.sender).slashingManager()
        );
        require(
    ...
    function _brackenFor(uint256 e3Id) internal view returns (IBracken) {
        E3PolicySnapshot storage policy = _e3PolicySnapshots[e3Id];
        return IBracken(policy.bracken);
    }
    ...
    /// @notice Set the Bracken contract address
    /// @param _bracken New Bracken address
    function setBracken(address _bracken) external onlyOwner {
        require(_bracken != address(0), "Invalid bracken");
        address oldValue = address(bracken);
        bracken = IBracken(_bracken);
        emit BrackenUpdated(oldValue, _bracken);
    }
