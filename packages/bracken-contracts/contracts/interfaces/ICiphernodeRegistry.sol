...
interface ICiphernodeRegistry {
    ...
    ///      canonical {IBracken.CommitteeFinalized} surface.
    /// @param e3Id ID of the E3 computation
    /// @param committee Array of selected ciphernode addresses
    /// @param scores Array of sortition scores corresponding to each committee member
    ...
    /// @notice This event MUST be emitted when `bracken` is set.
    /// @param bracken Address of the bracken contract.
    ...
    /// @notice Caller is not the Bracken contract
    error OnlyBracken();
    ...
    /// @dev This function MUST revert when not called by the Bracken contract.
    ...
    /// @dev Permissionless and bound to the request-time Bracken and bonding registry.
    ...
    /// @notice This function should be called by the Bracken contract to get the public key of a committee.
    ...
    /// @notice This function should be called by the Bracken contract to get the committee for a given E3.
    /// @dev This function MUST revert if no committee has been requested for the given E3.
