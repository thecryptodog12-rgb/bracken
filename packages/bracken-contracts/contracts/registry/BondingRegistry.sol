    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice Ticket token (tBRACKEN with underlying USDC) used for collateral
    BrackenTicketToken public ticketToken;

    /// @notice Ciphernode bond token (BRACKEN) required for operator registration
    ...
    /// @dev Multiple contracts (Bracken, E3RefundManager) need to distribute rewards.
    ...
    function _setBondingAssetConfig(
        ...
        ticketToken = BrackenTicketToken(config.ticketToken);
    ...
    /// @dev Only callable by owner. Supports multiple authorized distributors (Bracken + E3RefundManager)
