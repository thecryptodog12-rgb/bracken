...
// BRACKEN's lock schedule, the same surface the bonding registry reads. Lock-encumbered BRACKEN sits in
...
/**
 * @title BondedVotes
 * @notice Voting power that counts BRACKEN bonded as an operator alongside a primary vote source.
 *
 * @dev Bonding transfers BRACKEN to `BondingRegistry`, which never delegates it. Under ERC20Votes an
 ...
 * BRACKEN still counts in `getPastTotalSupply`, so it raises the quorum denominator while being
 ...
 *
 * TWO TOKEN REFERENCES, ON PURPOSE. `token` is BRACKEN: it supplies the metadata and, critically,
 ...
 *
 *   votesSource == token          wallet-held BRACKEN votes, via the token's own ERC20Votes
 *                                 delegation. The original behaviour.
 *   votesSource == an escrow      only BRACKEN locked in the escrow votes. Idle wallet BRACKEN carries
 *                                 no weight, so holders must lock to participate — while
 *                                 operators keep their weight by bonding instead.
 *
 * Keeping the denominator on BRACKEN in both cases is what makes the ratio sound. Every unit the
 * numerator can produce — locked or bonded — is a BRACKEN that is still inside BRACKEN's total supply,
 ...
 *
 * A THIRD SOURCE, under an escrow votes source only: BRACKEN still encumbered by BRACKEN's own vesting
 * locks. That BRACKEN sits in the holder's own wallet and {BrackenToken._update} refuses to move
 ...
 *
 * Escrowed and bonded BRACKEN cannot overlap: the escrow and the registry custody the token at two
 ...
 * {BrackenToken.transferableBalanceOf}, which lets a wallet move its locked BRACKEN to the extent
 * the bond already covers the obligation — so an account that bonds its locked BRACKEN reports the
...
contract BondedVotes is IERC5805 {
    /// @notice The BRACKEN token. Supplies the metadata and the quorum denominator.
    IVotes public immutable token;

    /// @notice Where per-account voting power is read from: the token itself, or an escrow
    /// adapter when only locked BRACKEN may vote.
    IVotes public immutable votesSource;

    /// @notice The escrow behind `votesSource`, or zero when the votes source is the token.
    /// @dev Held so {balanceOf} can attribute locked BRACKEN without going through the adapter's
    /// own `balanceOf`, which counts lock NFTs rather than tokens.
    address public immutable escrow;

    /// @notice Records bonded totals over time for the registry holding bonded BRACKEN.
    IBondedCheckpoints public immutable checkpoints;

    /// @notice The registry that custodies the bonded BRACKEN and writes the history.
    ...
    /**
     * @param _token The BRACKEN token: metadata, total supply, and the quorum denominator.
     * @param _votesSource Where per-account voting power is read. Pass `_token` itself to count
     * wallet-held BRACKEN, or an escrow IVotes adapter to count only locked BRACKEN.
    ...
    /// token's supply does not back — the denominator would be BRACKEN while the numerator counted
    ...
    /// BRACKEN, plus — under an escrow votes source — the vesting-locked BRACKEN it cannot escrow. All
    /// three are BRACKEN-denominated and read at the same timepoint.
    ///
    ...
    ///
    /// Zero unless the votes source is an escrow. When the token votes for itself, locked BRACKEN is
    /// wallet BRACKEN and the token has already counted it — adding it again would simply double
    /// every locked holder's weight.
    ///
    /// Netted, because a bond satisfies a lock: BRACKEN that is bonded is reported by BOTH
    ...
    /// cutting the lock, so an operator that had already moved locked BRACKEN out on the strength of
    /// that bond is left owing more than it holds — and the uncapped term would vote with the
    /// difference, BRACKEN that is now in the slash recipient's hands and countable there too.
    ///
    ...
    /// locked BRACKEN are both already counted here: each was transferred, not burned. Adding either
    ...
    /// @notice Get the BRACKEN attributable to an account: held in its wallet plus bonded under it.
    /// @dev Not a spendable balance, and nothing here can move it. Unlike {getVotes} this ignores
    /// delegation, so it answers "how much BRACKEN is this account's" rather than "how much can it
    /// vote with". A holder that never delegated reads a balance above its votes, which is the
    /// intended signal.
    ///
    /// The registry is netted down by what it merely custodies. Bonding moves BRACKEN into the
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        if (account == registry) {
            ...
            // leaving it here too would place the same BRACKEN at two addresses and push summed
            ...
            // be the more dangerous guess. The cost is that BRACKEN donated to the escrow on its own
        ...
        // Locked BRACKEN is custodied by the escrow for the same reason bonded BRACKEN is custodied by
    ...
    /// @dev Passed through for the same reason as {getPastTotalSupply}: bonded BRACKEN was
