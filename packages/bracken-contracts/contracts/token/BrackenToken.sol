...
/**
 * @title BrackenToken
 * @notice The governance and utility token for the Bracken protocol, with
 *         wallet-level lock enforcement designed around the Uniswap CCA
 *         distribution flow.
 *
 */
contract BrackenToken is
    ERC20,
    ERC20Permit,
    ERC20Votes,
    Ownable2Step,
    AccessControl
{
    ...
    /// @notice Registry whose bonded BRACKEN counts toward locked balances.
    ...
    /**
     * @notice Deploys BRACKEN with no TGE set.
     ...
     * @param bondingRegistry_ Registry whose bonded BRACKEN
     */
    constructor(
    ...
    ) ERC20("Bracken", "BRACKEN") ERC20Permit("Bracken") Ownable(initialOwner_) {
    ...
    /// @notice Plain vanilla admin mint: BRACKEN with no lock attached. Only
    ...
    /// @notice EIP-6372 clock — block.timestamp, aligned with other Bracken
    ///         contracts.
