    ...
    EIP712
{
    ...
    /// @notice Emitted when {bracken} is updated.
    ...
    /// @notice Reference to the Bracken contract for E3 failure signaling
    ...
    /// @dev Must match `ProofPayload::typehash()` in `crates/events/src/bracken_event/signed_proof.rs`.
    ...
    string public constant EIP712_DOMAIN_NAME = "BrackenSlashing";
        ...
        )
    {
        ...
        return (
            ...
            address(dependencies.brackenContract),
    ...
    function snapshotE3Dependencies(uint256 e3Id) external {
        ...
        dependencies.brackenContract = bracken;
    ...
    /// @notice Updates the Bracken contract
    /// @param newBracken The new IBracken contract
    function setBracken(
        IBracken newBracken
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(address(newBracken) != address(0), ZeroAddress());
        _requireGenerationDrained(address(bracken));
        address oldValue = address(bracken);
        bracken = newBracken;
        emit BrackenUpdated(oldValue, address(newBracken));
    }
    ...
    function _executeSlash(uint256 proposalId, Lane lane) internal {
        ...
        // bracken routing) so that reentrancy via those paths cannot re-enter
        ...
        if (p.affectsCommittee) {
            ...
            if (activeCount < thresholdM) {
                IBracken.E3Stage stage = dependencies.brackenContract.getE3Stage(
                    p.e3Id
                );
                if (
                    stage != IBracken.E3Stage.Complete &&
                    stage != IBracken.E3Stage.Failed
                ) {
                    // This call must succeed with the expulsion. A revert rolls
                    // back the penalties, ban, and committee membership change.
                    dependencies.brackenContract.onE3Failed(
                        p.e3Id,
                        uint8(
    ...
    function routePendingSlashFunds(
        ...
        dependencies.brackenContract.escrowSlashedFunds(
            route.e3Id,
            proposalId,
            route.operator,
            IERC20(route.token),
