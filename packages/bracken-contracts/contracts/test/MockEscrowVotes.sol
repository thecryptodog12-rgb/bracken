 ...
 *
 * @dev Stands in for the real escrow, whose voting-power curve is flat — locked BRACKEN produces
 * exactly its own amount in voting power, with no decay and no boost. That is what makes escrow
 * power addable to bonded BRACKEN at all, so the mock reproduces it by storing amounts directly.
 */
