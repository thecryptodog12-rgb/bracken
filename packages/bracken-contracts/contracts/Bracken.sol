...
import { IBracken, E3, IE3Program } from "./interfaces/IBracken.sol";
...
import { BrackenLifecycle } from "./lib/BrackenLifecycle.sol";
import { BrackenPricing } from "./lib/BrackenPricing.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice Initializes the Bracken contract with initial configuration.
    ...
    ///         loss of administrative control over Bracken.
    ...
    function request(
        ...
        // {BrackenLifecycle.validateRequest} (external library link, EIP-170 cap).
        require(
        ...
        BrackenLifecycle.validateQuoteLimit(
            address(feeToken),
        ...
        _e3LifecycleDeadlines[e3Id] =
            block.timestamp +
            BrackenLifecycle.requestLifecycleDuration(
        ...
        BrackenLifecycle.bindCryptoConfig(
            e3Id,
            encryptionSchemeId,
            paramSetRegistry[requestParams.paramSet]
        );
        ...
        BrackenPricing.transferFromExact(
            feeToken,
            msg.sender,
            address(this),
            quotedFee
    ...
    function publishCiphertextOutput(
        ...
        return
            BrackenLifecycle.publishCiphertext(
                e3s,
                _e3Stages,
                _e3Deadlines,
    ...
    function _verifyPlaintext(
        ...
        BrackenLifecycle.verifyPlaintext(
    ...
    function _distributeRewards(uint256 e3Id) internal {
        BrackenPricing.distributeRewards(
            e3Payments,
            _e3FeeTokens,
            _e3Requesters,
            _e3ProtocolShareBps,
    ...
    function setDecryptionVerifier(
        ...
        BrackenLifecycle.validateDecryptionVerifier(address(decryptionVerifier));
        require(
    ...
    function setPkVerifier(
        ...
        BrackenLifecycle.validatePkVerifier(address(pkVerifier));
        require(
    ...
    function setCiphertextVerifier(
        ...
        BrackenLifecycle.setCiphertextVerifier(
            encryptionSchemeId,
            ciphertextVerifier
        );
    ...
    function setParamSet(
        ...
        BrackenLifecycle.validateParamSet(
            paramSet,
    ...
    function processE3Failure(uint256 e3Id) external {
        ...
        address[] memory honestNodes = BrackenLifecycle.honestNodes(
            address(_registryFor(e3Id)),
            e3Id
        );
    ...
    function escrowSlashedFunds(
        ...
        BrackenLifecycle.validateSlashCaller(
            msg.sender,
    ...
    function onCommitteeFinalized(uint256 e3Id) external {
        uint256 dkgDeadline = BrackenLifecycle
    ...
    function onCommitteePublished(
        ...
        BrackenLifecycle.validateCommitteePublication(
            msg.sender,
            address(_registryFor(e3Id)),
            e3Id,
            uint8(_e3Stages[e3Id]),
    ...
    function onE3Failed(uint256 e3Id, uint8 reason) external {
        E3Stage current = _e3Stages[e3Id];
        BrackenLifecycle.validateReportedFailure(
            msg.sender,
            address(_registryFor(e3Id)),
            address(_slashingManagerFor(e3Id)),
            e3Id,
    ...
    function markE3Failed(
        ...
        BrackenLifecycle.validateMarkFailedStage(e3Id, uint8(current));
        ...
        BrackenLifecycle.validateMarkFailedCaller(
            e3Id,
            deadline,
    ...
    {
        uint8 rawReason;
        (canFail, rawReason, deadline) = BrackenLifecycle.failureCondition(
            address(_registryFor(e3Id)),
            e3Id,
            uint8(stage),
            _e3Deadlines[e3Id],
    ...
    function _setTimeoutConfig(E3TimeoutConfig calldata config) internal {
        BrackenLifecycle.validateTimeoutConfig(config, MAX_TIMEOUT_WINDOW);
    ...
    function setCommitteeThresholds(
        ...
        BrackenLifecycle.validateCommitteeThresholds(
            uint8(size),
            threshold,
            pc.minCommitteeSize,
            pc.minThreshold
    ...
    function _setFeeAssetConfig(FeeAssetConfig calldata config) internal {
        // Validation is delegated to {BrackenPricing.validateFeeAssetConfig}
        // (external library link) to keep the deployed Bracken runtime
        // bytecode under the EIP-170 24,576-byte cap. Revert selectors are
        // preserved via shared {IBracken} error declarations.
        BrackenPricing.validateFeeAssetConfig(
            config,
            MAX_MARGIN_BPS,
    ...
    function getE3Quote(
        ...
        BrackenLifecycle.validateRequest(
        ...
        // Pure fee math is delegated to {BrackenPricing.quote} (external
        // library link) to keep the deployed Bracken runtime bytecode under
    ...
    function getCiphertextVerifier(
        ...
        return BrackenLifecycle.getCiphertextVerifier(encryptionSchemeId);
    }
    ...
    function treasuryClaim(IERC20 token) external returns (uint256 amount) {
        amount = BrackenPricing.claimTreasury(
            _pendingTreasury,
            msg.sender,
            token
        );
    ...
    function _validateDependencyGraph() private view {
        BrackenLifecycle.validateDependencyGraph(
    ...
    function _requireDependencyReplacementReady(
        ...
        BrackenLifecycle.validateGenerationDrained(
    ...
    /// @notice ERC-165 interface detection. Advertises {IBracken} and
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IBracken).interfaceId ||
