...
/** Monorepo root (`bracken/`). Works from `scripts/` and compiled `dist/scripts/`. */
function resolveRepoRoot(): string {
  ...
  throw new Error(
    "Could not find bracken repo root (expected package.json name @bracken/main)",
  );
}
...
/**
 * Defines the Bracken.config.yaml structure
 */
export interface BrackenConfig {
  chains: Array<{
    name: string;
    rpc_url: string;
    contracts: {
      e3_program?: { address: string; deploy_block: number };
      bracken?: { address: string; deploy_block: number };
...
/**
 * The function to update the bracken.config.yaml file with the deployed contract addresses.
 ...
 * @param pathToConfigFile - The path to the bracken.config.yaml file
...
export const updateE3Config = (
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect contract key line (e.g., `      bracken:`)
  ...
  console.log("\n✓ bracken.config.yaml updated successfully!");
};
