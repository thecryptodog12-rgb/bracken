...
export function findRepoRoot(): string {
  ...
  while (true) {
    if (
      ...
      fs.existsSync(path.join(dir, "packages", "bracken-contracts"))
    ) {
      return dir;
    }
...
export function configPath(required = true): string {
  ...
  if (required && !fs.existsSync(file)) {
    throw new Error(
      `Protocol config not found: ${file}. Pass --config or create it from packages/bracken-contracts/deploy/protocol/example.protocol.config.json.`,
    );
  }
  return file;
}
