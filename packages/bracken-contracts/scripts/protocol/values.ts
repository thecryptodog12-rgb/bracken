...
export function pricingConfig(config: PricingConfig) {
  return {
    ...
    protocolTreasury: address(
      ...
      "bracken.pricing.protocolTreasury",
    ),
...
export function loadConfig(file = configPath()): ProtocolConfigFile {
  ...
  if (
    !config.bracken ||
    typeof config.bracken.registerActiveBfvParamSet !== "boolean"
  ) {
    throw new Error(
      "bracken.registerActiveBfvParamSet is required and must be a boolean",
    );
  }
  ...
  if (config.bracken.pricing.protocolTreasury === ZERO) {
    config.bracken.pricing.protocolTreasury = config.protocolTreasury;
  }
...
function validateConfig(config: ProtocolConfigFile): void {
  ...
  config.bracken.pricing.protocolTreasury = address(
    config.bracken.pricing.protocolTreasury,
    "bracken.pricing.protocolTreasury",
  );
