...
export interface ProtocolConfigFile {
  ...
  fold: string;
  /**
   * Optional escrow IVotes adapter. When set, only BRACKEN locked in that escrow carries voting
   * power and idle wallet BRACKEN carries none — operators keep their weight by bonding instead.
   * Omit to count wallet-held BRACKEN, which is the original behaviour.
   */
...
export interface ProtocolDeployment {
  ...
  bracken: string;
  brackenImplementation: string;
  brackenProxyAdmin: string;
