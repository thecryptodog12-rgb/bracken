...
export async function actionActivateVoting(): Promise<void> {
  ...
  if (ciphernodeBondToken.toLowerCase() !== config.fold.toLowerCase()) {
    throw new Error(
      `BondingRegistry bonds ${ciphernodeBondToken}, not BRACKEN (${config.fold}). ` +
  ...
  // Defaults to BRACKEN itself, which counts wallet-held votes. Setting `escrowVotesAdapter` makes
  // locking a precondition for voting; the constructor checks the escrow custodies this same BRACKEN.
...
Bonded voting activated
  ...
  Votes source:      ${votesSource}${
    config.escrowVotesAdapter ? " (locked BRACKEN only)" : " (wallet-held BRACKEN)"
  }
