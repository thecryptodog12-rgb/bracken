...
function integrationConfigPath(): string {
  return path.join(repoRoot, "tests", "integration", "bracken.config.yaml");
}
...
export function syncProtocolDeploymentRecords(
  ...
  storeDeploymentArgs(
    {
      address: deployment.brackenPricing,
      blockNumber,
    },
    "BrackenPricing",
    opts.chain,
  ...
  storeDeploymentArgs(
    {
      address: deployment.brackenLifecycle,
      blockNumber,
    },
    "BrackenLifecycle",
    opts.chain,
  ...
  const brackenInitData = interfaces.bracken.encodeFunctionData("initialize", [
    ...
    {
      dkgWindow: BigInt(config.bracken.timeoutConfig.dkgWindow),
      computeWindow: BigInt(config.bracken.timeoutConfig.computeWindow),
      decryptionWindow: BigInt(config.bracken.timeoutConfig.decryptionWindow),
    },
    deployment.initialE3Program,
  ]);
  storeDeploymentArgs(
    {
      address: deployment.bracken,
      blockNumber,
      constructorArgs: {
        owner: config.protocolOwner,
        ...
        maxDuration: config.bracken.maxDuration,
        timeoutConfig: JSON.stringify(config.bracken.timeoutConfig),
        pricingConfig: JSON.stringify(config.bracken.pricing),
      ...
      proxyRecords: {
        initData: brackenInitData,
        initialOwner: config.protocolOwner,
        proxyAddress: deployment.bracken,
        proxyAdminAddress: deployment.brackenProxyAdmin,
        implementationAddress: deployment.brackenImplementation,
      },
    },
    "Bracken",
    opts.chain,
  ...
  const refundInitData = interfacesFor("E3RefundManager").encodeFunctionData(
    "initialize",
    [config.protocolOwner, deployment.bracken, config.protocolTreasury],
  );
  storeDeploymentArgs(
    {
