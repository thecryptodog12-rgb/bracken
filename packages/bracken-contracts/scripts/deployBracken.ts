...
import { deployAndSaveBracken } from "./deployAndSave/bracken";
import { deployAndSaveBrackenTicketToken } from "./deployAndSave/brackenTicketToken";
import { deployAndSaveBrackenToken } from "./deployAndSave/brackenToken";
...
// to avoid a cyclic dependency on @bracken/sdk.
const BFV_PARAMS = {
  insecure512: {
    degree: 512n,
...
/**
 * Deploys the Bracken contracts
 */
export const deployBracken = async (
  withMocks?: boolean,
  ...
  // ── CCA window ──────────────────────────────────────────────────────────
  // Alleen van belang als we BrackenToken zelf deployen. Die draagt de veiling:
  ...
  const ccaStartEnv = process.env.BRACKEN_CCA_START;
  let ccaStart: bigint;
  if (ccaStartEnv?.trim()) {
    ccaStart = parseRequiredUint64(ccaStartEnv.trim(), "BRACKEN_CCA_START");
  } else if (
    !deployingOwnToken ||
    ...
    if (deployingOwnToken) {
      console.warn(
        `[WARN] BRACKEN_CCA_START not set; using ${ccaStart} (block.timestamp + 1h) for ${networkName} deployment.`,
      );
    }
  } else {
    throw new Error(
      "BRACKEN_CCA_START must be set when deploying BrackenToken (the auction token) " +
  ...
  console.log("Deploying BrackenTicketToken...");
  const { brackenTicketToken } = await deployAndSaveBrackenTicketToken({
  ...
  console.log("BrackenTicketToken deployed to:", brackenTicketTokenAddress);
  ...
  // BondingRegistry is deployed before BRACKEN so its address can be passed to
  // the token constructor.  The ciphernode bond token is set to address(0) temporarily
  // and fixed after BRACKEN is deployed with the complete asset configuration.
  console.log("Deploying BondingRegistry...");
  const { bondingRegistry } = await deployAndSaveBondingRegistry({
    owner: ownerAddress,
    ticketToken: brackenTicketTokenAddress,
  ...
  const currentTicketRegistry = await brackenTicketToken.registry();
  if (currentTicketRegistry.toLowerCase() === bondingRegistryAddress.toLowerCase()) {
    console.log("BrackenTicketToken already points at BondingRegistry");
  } else {
    console.log("Setting BondingRegistry address in BrackenTicketToken...");
    await (await brackenTicketToken.setRegistry(bondingRegistryAddress)).wait();
  }

  // BRACKEN is deployed with BondingRegistry's real address. Local deployments set
  // the deployer as the one-time claim source placeholder; production sale
  // deployments set the actual auction after it exists.
  console.log("Deploying BRACKEN token...");
  // Een bestaande bond-token meegeven in plaats van BrackenToken deployen.
  //
  // BrackenToken bestaat om een CCA-veiling te draaien: CCA_START en CCA_END
  ...
  const { brackenToken } = externalBondToken
    ? { brackenToken: null }
    : await deployAndSaveBrackenToken({
    owner: ownerAddress,
    ccaStart,
    ccaEnd,
  ...
  const brackenTokenAddress = externalBondToken ?? (await brackenToken!.getAddress());
  if (!externalBondToken) {
    console.log("BrackenToken deployed to:", brackenTokenAddress);
  }

  // Fix up BondingRegistry's ciphernode bond token now that BRACKEN exists.
  console.log("Setting ciphernode bond token in BondingRegistry...");
  await (
    await bondingRegistry.setBondingAssetConfig({
      ticketToken: brackenTicketTokenAddress,
      ciphernodeBondToken: brackenTokenAddress,
  ...
  if (brackenTokenAddress.toLowerCase() === feeTokenAddress.toLowerCase()) {
    throw new Error(
      "MockUSDC and BrackenToken resolved to the same address. " +
  ...
  // setTransferWhitelisted bestaat alleen op BrackenToken. Een externe ERC-20
  // kent die functie niet -- en heeft hem ook niet nodig, want daar zijn
  // transfers sowieso vrij.
  if (!externalBondToken) {
    console.log("Whitelisting BondingRegistry in BRACKEN...");
    await (
      await brackenToken!.setTransferWhitelisted(bondingRegistryAddress, true)
    ).wait();
  }

  // ── Bonded voting ───────────────────────────────────────────────────────
  // Bonded BRACKEN is transferred to BondingRegistry and never delegated, so without a recorded
  ...
  // Deploy a public Faucet pre-funded with BRACKEN + mock USDC so testers can
  // self-serve tokens. BRACKEN is in the Virtual phase here (CCA_START is ~1h
  // out), so we mint unlocked BRACKEN and whitelist the faucet to bypass the
  ...
  if (networkName === "sepolia" && mockStableToken) {
    ...
    // Whitelist the faucet so faucet -> tester BRACKEN transfers pass the
    // pre-TGE gate (transferWhitelist[from] short-circuits the restriction).
    console.log("Whitelisting Faucet in BRACKEN...");
    await (
      await brackenToken!.setTransferWhitelisted(faucetAddress, true)
    ).wait();

    console.log("Minting BRACKEN to Faucet...");
    await (
    ...
    console.log(
      `Faucet funded with ${ethers.formatEther(FAUCET_FOLD_SUPPLY)} BRACKEN ` +
  ...
  console.log("Deploying Bracken...");
  const { bracken } = await deployAndSaveBracken({
    owner: ownerAddress,
  ...
  console.log("Setting E3RefundManager in Bracken...");
  await send(
    bracken.setE3RefundManager(e3RefundManagerAddress),
    "bracken.setE3RefundManager",
  );
  ...
  console.log("Setting Bracken address in CiphernodeRegistry...");
  await send(
    ciphernodeRegistry.setBracken(brackenAddress),
    "ciphernodeRegistry.setBracken",
  );
  ...
  console.log("Setting Bracken address in SlashingManager...");
  await send(
    slashingManager.setBracken(brackenAddress),
    "slashingManager.setBracken",
  );
  ...
  console.log("Setting SlashingManager address in Bracken...");
  await send(
    bracken.setSlashingManager(slashingManagerAddress),
    "bracken.setSlashingManager",
  );
  ...
  console.log("Setting Bracken as reward distributor in BondingRegistry...");
  await send(
    bondingRegistry.setRewardDistributor(brackenAddress),
    "bondingRegistry.setRewardDistributor",
  );

  // E3RefundManager already has correct bracken from deployment

  console.log("Setting the active committee configuration...");
  await send(
    bracken.setCommitteeThresholds(ACTIVE_BFV_COMMITTEE_SIZE, [
      BFV_DKG_H,
      ACTIVE_BFV_COMMITTEE_N,
    ]),
    "bracken.setCommitteeThresholds",
  );
  console.log(
  ...
  const existingParams = await bracken.paramSetRegistry(ACTIVE_BFV_PARAM_SET);
  if (existingParams && existingParams !== "0x") {
    ...
    await send(
      bracken.setParamSet(ACTIVE_BFV_PARAM_SET, activeParams),
      "bracken.setParamSet",
    );
  ...
  if (mockDeployments) {
    ...
    if (!shouldHaveZKVerification && mockDecryptionVerifierAddress) {
      const deployedDecryptionVerifier =
        await bracken.decryptionVerifiers(encryptionSchemeId);
      if (deployedDecryptionVerifier === mockDecryptionVerifierAddress) {
        console.log(`DecryptionVerifier already set in Bracken contract`);
      } else {
        const tx = await bracken.setDecryptionVerifier(
          encryptionSchemeId,
          mockDecryptionVerifierAddress,
        );
        await tx.wait();
        console.log(
          `Successfully set MockDecryptionVerifier in Bracken contract`,
        );
      }
    }

    if (!shouldHaveZKVerification && mockPkVerifierAddress) {
      const deployedPkVerifier = await bracken.pkVerifiers(encryptionSchemeId);
      if (deployedPkVerifier === mockPkVerifierAddress) {
        console.log(`PkVerifier already set in Bracken contract`);
      } else {
        const tx = await bracken.setPkVerifier(
          encryptionSchemeId,
          mockPkVerifierAddress,
        );
        await tx.wait();
        console.log(`Successfully set MockPkVerifier in Bracken contract`);
      }
    }

    if (!shouldHaveZKVerification && mockCiphertextVerifierAddress) {
      const deployedCiphertextVerifier =
        await bracken.getCiphertextVerifier(encryptionSchemeId);
      if (deployedCiphertextVerifier === mockCiphertextVerifierAddress) {
        console.log("CiphertextVerifier already set in Bracken contract");
      } else {
        const tx = await bracken.setCiphertextVerifier(
          encryptionSchemeId,
          mockCiphertextVerifierAddress,
        );
        await tx.wait();
        console.log(
          "Successfully set MockCiphertextVerifier in Bracken contract",
        );
      }
    }

    if (await bracken.e3Programs(e3ProgramAddress)) {
      console.log(`E3 Program already enabled in Bracken contract`);
    } else {
      const tx = await bracken.registerE3Program(e3ProgramAddress);
      await tx.wait();
      console.log(`Successfully enabled E3 Program in Bracken contract`);
    }
  }
  ...
  if (shouldHaveZKVerification) {
    ...
    if (deployedDecryptionVerifier !== bfvDecryptionVerifierAddress) {
      ...
      await tx.wait();
      console.log("Successfully set BfvDecryptionVerifier in Bracken contract");
    }
  }
  ...
  if (shouldHaveZKVerification) {
    ...
    if (deployedPkVerifier !== bfvPkVerifierAddress) {
      ...
      await tx.wait();
      console.log("Successfully set BfvPkVerifier in Bracken contract");
    }
  }
  ...
  const decryptionVerifierAddress =
    await bracken.decryptionVerifiers(encryptionSchemeId);
  const pkVerifierAddress = await bracken.pkVerifiers(encryptionSchemeId);
  ...
  const wiring: Array<[string, Promise<string>, string]> = [
    ...
    ["ciphernodeRegistry.bracken", ciphernodeRegistry.bracken(), brackenAddress],
    [
    ...
    ["bondedVotes.token", bondedVotes.token(), brackenTokenAddress],
    [
      "bondedVotes.checkpoints",
      bondedVotes.checkpoints(),
      bondedCheckpointsAddress,
    ],
    ["slashingManager.bracken", slashingManager.bracken(), brackenAddress],
    [
  ...
  if (!(await bondingRegistry.authorizedDistributors(brackenAddress))) {
    wiringErrors.push(
      `bondingRegistry.authorizedDistributors: ${brackenAddress} is not authorized`,
    );
  }
  ...
  await send(
    bracken.setRequestsPaused(false),
    "bracken.setRequestsPaused(false)",
  );
  ...
  console.log(`
    ...
    BrackenToken (BRACKEN): ${brackenTokenAddress}
    BrackenTicketToken: ${brackenTicketTokenAddress}
