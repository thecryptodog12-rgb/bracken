 ...
 * BRACKEN and funds it with BRACKEN + mock USDC.
 *
 * BRACKEN funding uses `mint()`, which is only valid while BRACKEN is in the Virtual
 * phase (before CCA_START). The deployer holds no BRACKEN of its own, so there is
...
const main = async () => {
  ...
  const foldAddress = readDeploymentArgs("BrackenToken", chain)?.address;
  const feeTokenAddress = readDeploymentArgs("MockUSDC", chain)?.address;
  if (!foldAddress || !feeTokenAddress) {
    throw new Error(
      "BrackenToken (BRACKEN) and/or MockUSDC not found in deployed_contracts.json. " +
        "Run the full deploy first.",
    );
  }

  const fold = BrackenTokenFactory.connect(foldAddress, signer);
  ...
  if (phase !== 0n) {
    throw new Error(
      `BRACKEN is no longer in the Virtual phase (phase=${phase}); mint() is closed, ` +
        "so the faucet cannot be funded with BRACKEN. Re-run the full deploy to reset the CCA window.",
    );
  }
  ...
  console.log(`
    ...
    Block:   ${blockNumber}
    BRACKEN:    ${ethers.formatEther(foldSupply)}
