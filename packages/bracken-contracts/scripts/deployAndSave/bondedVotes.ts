...
export interface BondedVotesArgs {
  /** BRACKEN. Supplies the metadata, the total supply and the quorum denominator. */
  token: string;
  /**
   * Where per-account voting power is read. Pass `token` to count wallet-held BRACKEN, or an escrow
   * IVotes adapter to count only locked BRACKEN. Defaults to `token`, preserving the original
