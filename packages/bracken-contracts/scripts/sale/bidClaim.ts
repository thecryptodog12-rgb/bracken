...
export async function bidClaim(
  ...
  if (balance === 0n) throw new Error("Buyer did not receive BRACKEN");
  if (lockCount === 0n) {
    throw new Error("Buyer claim did not create a BRACKEN lock");
  }
  await expectTransferRestricted(fold.connect(buyer), deployment.safe);
  console.log(
    `Buyer ${buyerAddress} claimed ${balance} BRACKEN wei with ${lockCount} lock entry`,
  );
  if (bidId !== undefined) {
    deployment.testBidId = bidId.toString();
    writeJson(deploymentPath(config), deployment);
...
async function expectTransferRestricted(fold: any, to: string): Promise<void> {
  ...
  throw new Error("Expected claimed BRACKEN to be transfer-restricted before TGE");
}
