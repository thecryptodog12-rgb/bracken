...
export async function actionFullTest(): Promise<void> {
  ...
  if (safe === operatorAddress) {
    ...
    console.log(`Accepted BRACKEN ownership: ${deployment.fold}`);
    await (await fold.setClaimSource(deployment.auction)).wait();
    console.log(`Set BRACKEN claim source: ${deployment.auction}`);
  } else {
    console.log(
      `BRACKEN ownership is pending Safe activation. Execute the Safe batch from ${safe}: acceptOwnership(), setClaimSource(), and Predicate hook setup if applicable.`,
    );
  }
