...
export async function validateDeployment(
  ...
  const saleDeployer = await ethers.getContractAt(
    "BrackenTokenSaleDeployer",
    deployment.saleDeployer,
  );
  const fold = await ethers.getContractAt("BrackenToken", deployment.fold);
  ...
  if (address(claimSource, "BRACKEN.CLAIM_SOURCE") === ZERO) {
    throw new Error(
      "BRACKEN.CLAIM_SOURCE is not set. Execute the Safe activation batch first: BRACKEN.acceptOwnership(), BRACKEN.setClaimSource(auction), and PredicateValidationHook.setAuction(auction) if applicable.",
    );
  }
  assertEq("BRACKEN.CLAIM_SOURCE", claimSource, deployment.auction);

  const bondingRegistry = await fold.BONDING_REGISTRY();
  assertEq(
    "BRACKEN.BONDING_REGISTRY",
    bondingRegistry,
  ...
  assertEq(
    "BRACKEN whitelist LiquidityLauncher",
    await fold.transferWhitelist(plan.liquidityLauncher),
    false,
  );
  assertEq(
    "BRACKEN whitelist LBPStrategy",
    await fold.transferWhitelist(plan.lbpStrategy),
    true,
  );
  assertEq(
    "BRACKEN whitelist PositionManager",
  ...
  if (auctionBalance > saleAmount) {
    throw new Error(
      `BRACKEN auction balance exceeds sale amount: ${auctionBalance} > ${saleAmount}`,
    );
  }
  if (currentBlock < BigInt(config.auction.claimBlock)) {
    assertEq("BRACKEN auction balance", auctionBalance, saleAmount);
  } else {
    console.log(
      `  ok BRACKEN auction balance <= sale amount (${auctionBalance}, ${formatFold(auctionBalance)})`,
    );
  }
  printValue(
    "auction BRACKEN balance",
    `${auctionBalance} (${formatFold(auctionBalance)})`,
  );

  const ccaStart = await fold.CCA_START();
  assertEq("BRACKEN.CCA_START", ccaStart, config.fold.ccaStart);
  printValue("CCA_START", formatTimestamp(ccaStart));

  const ccaEnd = await fold.CCA_END();
  assertEq("BRACKEN.CCA_END", ccaEnd, config.fold.ccaEnd);
  printValue("CCA_END", formatTimestamp(ccaEnd));

  const noMoreLocks = await fold.NO_MORE_LOCKS();
  assertEq("BRACKEN.NO_MORE_LOCKS", noMoreLocks, plan.fold.noMoreLocks);
  printValue("NO_MORE_LOCKS", formatTimestamp(noMoreLocks));

  const tgeTimestamp = await fold.tgeTimestamp();
  assertEq("BRACKEN.tgeTimestamp", tgeTimestamp, 0);
  ...
  const owner = address(await fold.owner(), "BRACKEN.owner");
  const pendingOwner = await optionalView("BRACKEN.pendingOwner", () =>
    fold.pendingOwner(),
  );
  printValue("BRACKEN owner", owner);
  if (pendingOwner !== undefined) printValue("BRACKEN pendingOwner", pendingOwner);
  ...
  if (owner === config.safe) {
    ...
    const normalizedPendingOwner = address(
      String(pendingOwner),
      "BRACKEN.pendingOwner",
    );
    if (allowPendingOwner && normalizedPendingOwner === config.safe) {
      console.log("  ok BRACKEN ownership is pending Safe acceptance");
    } else {
      throw new Error(
        `BRACKEN owner is ${owner}; expected accepted Safe ${config.safe}`,
      );
    }
  }
