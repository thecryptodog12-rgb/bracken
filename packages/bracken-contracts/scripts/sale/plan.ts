...
export async function buildSalePlan(
  ...
  const foldFactory = await ethers.getContractFactory("BrackenToken");
...
Bracken sale plan
  ...
  BRACKEN:          discovered at deploy
  CCA auction:   discovered from LBPStrategy.InitializerCreated
  auction BRACKEN:  ${plan.lbpSaleConfig.auctionAmount}
  LP BRACKEN reserve:${plan.lbpSaleConfig.reservedTokenAmountForLP}
  ...
  BRACKEN timestamps: start=${plan.fold.ccaStart} end=${plan.fold.ccaEnd} noMoreLocks=${plan.fold.noMoreLocks}
