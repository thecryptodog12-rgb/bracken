...
export type UpgradeTarget =
  "bondingRegistry" | "ciphernodeRegistry" | "bracken" | "e3RefundManager";
...
async function deployImplementation(
  ...
  if (target === "bracken") {
    const pricingFactory = await ethers.getContractFactory("BrackenPricing");
    ...
    const lifecycleFactory = await ethers.getContractFactory("BrackenLifecycle");
    ...
    const factory = await ethers.getContractFactory("Bracken", {
      libraries: {
        BrackenLifecycle: lifecycleLibrary,
        BrackenPricing: pricingLibrary,
      },
    });
...
function proxyFor(
  ...
  if (target === "bracken") return deployment.bracken;
...
function proxyAdminFor(
  ...
  if (target === "bracken") return deployment.brackenProxyAdmin;
