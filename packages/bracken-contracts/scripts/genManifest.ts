 ...
 * `bracken` binary can fetch it without being rebuilt — the compiled-in
 ...
 *
 * Contract keys are deliberately the `bracken.config.yaml` keys, not the
 * Solidity contract names, so `bracken config check` can compare an operator's
...
const DEPLOYED = path.join(
  REPO_ROOT,
  "packages/bracken-contracts/deployed_contracts.json",
);
...
/**
 * `bracken.config.yaml` key -> the deployed_contracts.json names that can
 ...
 * something `bracken.config.yaml` accepts.
 */
    ...
    if (chainId === undefined) {
      throw new Error(
        `${network}: no chain ID known for this network. Add it to CHAIN_IDS in ` +
          `scripts/genManifest.ts so \`bracken config check\` can verify it.`,
      );
    }
