...
This is the deployment-only checklist for the mainnet BRACKEN + Uniswap CCA launch.
...
```bash
packages/bracken-contracts/deploy/sale/mainnet-sale.config.json
```

Required values:

- [ ] `name = bracken-mainnet-cca`.
- [ ] `chainId = 1`.
...
- [ ] Config file is written under `packages/bracken-contracts/deploy/sale`.
- [ ] Infra file is written under `packages/bracken-contracts/deploy/sale`.
...
- [ ] BRACKEN says `discovered at deploy`.
- [ ] CCA auction says `discovered from LBPStrategy.InitializerCreated`.
- [ ] Auction BRACKEN is `120,000,000`.
- [ ] LP reserve is `30,000,000`.
- [ ] Total minted/distributed BRACKEN is `150,000,000`.
- [ ] Required raise is `400 ETH`.
- [ ] Floor price corresponds to `0.000012 ETH/BRACKEN`.
- [ ] Tick spacing corresponds to `1%` of floor price.
- [ ] CCA blocks line matches expected July 8 to July 10 window.
- [ ] BRACKEN timestamps line matches expected July 6 to July 10 lifecycle.
- [ ] BRACKEN `noMoreLocks` line is `1915884000` (`2030-09-17T14:00:00Z`).
- [ ] `claimBlock = migrationBlock`.
...
- [ ] BRACKEN address printed and recorded.
...
- [ ] Safe activation batch includes `BRACKEN.acceptOwnership()`.
- [ ] Safe activation batch includes `BRACKEN.setClaimSource(auction)`.
...
- [ ] Transaction 1 target is BRACKEN and calldata is `acceptOwnership()`.
- [ ] Transaction 2 target is BRACKEN and calldata is `setClaimSource(auction)`.
...
- [ ] `saleDeployer.protocolAdmin = Foundation Safe`.
- [ ] `BRACKEN.owner = Foundation Safe`.
- [ ] `BRACKEN.pendingOwner = zero`.
- [ ] `BRACKEN.CLAIM_SOURCE = auction`.
- [ ] `BRACKEN.BONDING_REGISTRY = BondingRegistry proxy`.
- [ ] Auction token is BRACKEN.
- [ ] Auction total supply is `120,000,000 BRACKEN`.
...
- [ ] BRACKEN auction balance is `120,000,000 BRACKEN`.
- [ ] BRACKEN total supply is `150,000,000 BRACKEN`.
- [ ] LBP reserve is `30,000,000 BRACKEN`.
- [ ] LBP strategy values validate.
- [ ] LBP recipient is Foundation Safe.
- [ ] LBP position recipient is Foundation Safe.
- [ ] LiquidityLauncher is not transfer-whitelisted on BRACKEN after distribution.
- [ ] LBPStrategy is transfer-whitelisted on BRACKEN.
- [ ] PositionManager is transfer-whitelisted on BRACKEN.
...
- [ ] Foundation Safe owners have the deployment file, plan file, Safe proposal,
      BRACKEN address, auction address, and Uniswap URL.
