...
export default buildModule("Bracken", (m) => {
  ...
  // size-constrained Bracken runtime.
  const brackenLifecycle = m.library("BrackenLifecycle");
  const brackenPricing = m.library("BrackenPricing");
  const brackenImpl = m.contract("Bracken", [], {
    libraries: {
  ...
  const initData = m.encodeFunctionCall(brackenImpl, "initialize", [
    owner,
    registry,
    bondingRegistry,
    e3RefundManager,
  ...
  const bracken = m.contract("TransparentUpgradeableProxy", [
    brackenImpl,
    owner,
    initData,
  ]);

  return { bracken, brackenLifecycle, brackenPricing };
}) as any;
