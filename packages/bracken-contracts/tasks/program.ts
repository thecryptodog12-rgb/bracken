...
// Wire the local MockE3ProgramHarness to Bracken so `publishInput` forwards to
...
export const setMockProgramBracken = task(
  "e3-program:setMockBracken",
  "Wire the mock test harness to Bracken for proof-aggregation tests",
)
  .setAction(async () => ({
    default: async (_args, hre) => {
      ...
      const brackenArgs = readDeploymentArgs("Bracken", network);
      if (!mockArgs?.address || !brackenArgs?.address) {
        throw new Error(
          "MockE3Program or Bracken deployment not found; deploy first.",
        );
      }
      ...
      const mockProgram = new ethers.Contract(
        ...
        [
          ...
          "function setBracken(address) external",
        ],
        signer,
      );
      const current: string = await mockProgram.bracken();
      if (current.toLowerCase() === brackenArgs.address.toLowerCase()) {
        console.log(
          `MockE3ProgramHarness already wired to ${brackenArgs.address}`,
        );
        return;
      }
      await mockProgram.setBracken(brackenArgs.address);
      console.log(
        `MockE3ProgramHarness ${mockArgs.address} → Bracken ${brackenArgs.address}`,
      );
    },
  }))
  .build();
