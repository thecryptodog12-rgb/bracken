...
async function getBrackenConnection(hre: any) {
  const { ethers } = await hre.network.connect();
  const [signer] = await ethers.getSigners();
  const chain = hre.globalOptions.network;
  const deployment = readDeploymentArgs("Bracken", chain);

  if (!deployment?.address) {
    throw new Error("Bracken deployment not found");
  }

  return {
    ethers,
    bracken: await ethers.getContractAt("Bracken", deployment.address, signer),
  };
}
...
export const requestCommittee = task(
  ...
  .setAction(async () => ({
    default: async (
      ...
      const { bracken } = await getBrackenConnection(hre);
      ...
      const [signer] = await ethers.getSigners();
      const brackenContract = bracken.connect(signer);
      const mockUSDCContract = mockUSDC.connect(signer);

      const brackenArgs = readDeploymentArgs(
        "Bracken",
        hre.globalOptions.network,
      );

      if (!brackenArgs) {
        throw new Error("Bracken deployment arguments not found");
      }
      ...
      const quoteParams = {
        ...
        expectedCryptoConfigId: await brackenContract.activeCryptoConfigId(),
        maxFee: MaxUint256,
      };

      const fee = await brackenContract.getE3Quote(quoteParams);
      ...
      const approveTx = await mockUSDCContract.approve(
        await brackenContract.getAddress(),
        fee,
      );
      await approveTx.wait();
      console.log("USDC approved");

      const tx = await brackenContract.request(requestParams);
      ...
      const brackenAddress = (await brackenContract.getAddress()).toLowerCase();
      const requestedTopic =
        brackenContract.interface.getEvent("E3Requested").topicHash;
      const requestedLog = receipt.logs.find(
        (log: Log) =>
          log.address.toLowerCase() === brackenAddress &&
      ...
      const requestedEvent = brackenContract.interface.parseLog(requestedLog);
      if (!requestedEvent) {
...
export const enableE3 = task("bracken:enableE3", "Enable an E3 program")
  .addOption({
    name: "e3Address",
    description: "address of the E3 program",
    defaultValue: ZeroAddress,
  ...
  .setAction(async () => ({
    default: async ({ e3Address }, hre) => {
      const { ethers, bracken } = await getBrackenConnection(hre);

      if (await bracken.e3Programs(e3Address)) {
        console.log(`E3 program already enabled: ${e3Address}`);
        return;
      }

      const ownerAddress = (await bracken.owner()).toLowerCase();
      ...
      if (!ownerSigner) {
        throw new Error(
          `Bracken owner ${ownerAddress} is not an available signer. Submit registerE3Program through the owner Safe.`,
        );
      }

      const tx = await bracken.connect(ownerSigner).registerE3Program(e3Address);
...
export const publishCiphertext = task(
  ...
  .setAction(async () => ({
    default: async (
      ...
      const tx = await bracken.publishCiphertextOutput(
        e3Id,
        dataToSend,
        commitmentToSend,
        proofToSend,
...
export const publishPlaintext = task(
  ...
  .setAction(async () => ({
    default: async ({ e3Id, data, dataFile, proof, proofFile }, hre) => {
      ...
      const tx = await bracken.publishPlaintextOutput(
        e3Id,
        dataToSend,
        proofToSend,
      );
