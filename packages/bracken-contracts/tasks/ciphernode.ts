...
export const ciphernodeAdd = task(
  ...
  .addOption({
    ...
    description:
      "amount of BRACKEN to bond (in wei, e.g., 1000000000000000000000 for 1000 BRACKEN)",
  ...
  .setAction(async () => ({
    default: async ({ ciphernodeBondAmount, ticketAmount }, hre) => {
      ...
      const { deployAndSaveBrackenTicketToken } =
        await import("../scripts/deployAndSave/brackenTicketToken");
      const { deployAndSaveBrackenToken } =
        await import("../scripts/deployAndSave/brackenToken");
      ...
      const { brackenToken } = await deployAndSaveBrackenToken({ hre });
      ...
      const ciphernodeBondToken = brackenToken.connect(bondOwner);
      const ticketToken = brackenTicketToken.connect(bondOwner);
      ...
      try {
        ...
        console.log(`BRACKEN balance: ${ethers.formatEther(foldBalance)}`);
        ...
        if (foldBalance < ciphernodeBondAmountBigInt) {
          throw new Error(
            `Insufficient BRACKEN balance. Need: ${ethers.formatEther(ciphernodeBondAmountBigInt)}, Have: ${ethers.formatEther(foldBalance)}`,
          );
        }
        ...
        await bondTx.wait();
        console.log(
          `Ciphernode bonded: ${ethers.formatEther(ciphernodeBondAmountBigInt)} BRACKEN`,
        );
        ...
        console.log(
          `Ciphernode Bond: ${ethers.formatEther(ciphernodeBond)} BRACKEN`,
        );
        console.log(
...
export const ciphernodeMintTokens = task(
  ...
  "Mint BRACKEN and USDC tokens for a ciphernode (for testing)",
)
  .addOption({
  ...
  .addOption({
    name: "foldAmount",
    description:
      "amount of BRACKEN to mint (in ether units, e.g., 2000 for 2000 BRACKEN)",
    defaultValue: "2000",
  })
  .addOption({
    name: "usdcAmount",
  ...
  .setAction(async () => ({
    default: async ({ ciphernodeAddress, foldAmount, usdcAmount }, hre) => {
      ...
      const brackenTokenContract = brackenToken.connect(signer);
      const mockUSDCContract = mockStableToken.connect(signer);

      try {
        console.log(`Minting tokens for: ${ciphernodeAddress}`);

        console.log(`Minting ${foldAmount} BRACKEN...`);
        const foldTx = await brackenTokenContract.mint(
          ciphernodeAddress,
          ethers.parseEther(foldAmount),
          ethers.encodeBytes32String("cn-alloc"),
        );
        await foldTx.wait();
        console.log(`${foldAmount} BRACKEN minted`);
        ...
        const foldBalance =
          await brackenTokenContract.balanceOf(ciphernodeAddress);
        const usdcBalance = await mockUSDCContract.balanceOf(ciphernodeAddress);

        console.log("\n=== Token Balances ===");
        console.log(`BRACKEN: ${ethers.formatEther(foldBalance)}`);
        console.log(`USDC: ${ethers.formatUnits(usdcBalance, 6)}`);

        const tgeTs = await brackenTokenContract.tgeTimestamp();
        if (tgeTs === 0n) {
          console.log("Firing TGE to enable transfers...");
          try {
            await (await brackenTokenContract.tge()).wait();
            console.log("BrackenToken TGE fired, transfers enabled");
          } catch (e: any) {
            console.warn(
...
export const ciphernodeAdminAdd = task(
  ...
  .addOption({
    name: "ciphernodeBondAmount",
    description:
      "amount of BRACKEN to bond (in ether units, e.g., 1000 for 1000 BRACKEN)",
    defaultValue: "1000",
  })
  .addOption({
    name: "ticketAmount",
  ...
  .setAction(async () => ({
    default: async (
      ...
      const brackenTokenConnected = brackenToken.connect(adminWallet);
      const mockUSDCConnected = mockUSDC.connect(adminWallet);

      const ticketTokenAddress = await brackenTicketToken.getAddress();

      try {
        const ciphernodeBondWei = ethers.parseEther(ciphernodeBondAmount);
        const ticketAmountWei = ethers.parseUnits(ticketAmount, 6);

        console.log("Step 1: Minting BRACKEN to the bond owner...");

        const foldTx = await brackenTokenConnected.mint(
          adminWallet.address,
          ciphernodeBondWei,
        ...
        const approveTx = await brackenTokenConnected.approve(
          await bondingRegistry.getAddress(),
          ciphernodeBondWei,
        );
        await approveTx.wait();
        ...
        await bondTx.wait();
        console.log(`Ciphernode bonded: ${ciphernodeBondAmount} BRACKEN`);
