...
// Addresses sourced from packages/bracken-contracts/deployed_contracts.json
...
// @bracken/contracts so they cannot drift from the deployed contracts.
...
import {
  ...
  Faucet__factory,
  Bracken__factory,
  BrackenTicketToken__factory,
  BrackenToken__factory,
} from '@bracken/contracts/types'

// E3 lifecycle stages — mirrors the Solidity `IBracken.E3Stage` enum exactly.
// Defined locally (rather than imported from @bracken/sdk) so the dashboard
...
export const CONTRACTS = {
  Bracken: envStr('VITE_BRACKEN_ADDRESS', NOT_DEPLOYED) as Address,
  ...
  // Testnet-only convenience faucet (BRACKEN + fee token). The zero address or an
...
// Bracken, CiphernodeRegistry and CRISPProgram, so it must be the earliest of
// those three deploy blocks (CiphernodeRegistry), not Bracken's: a later value
// would silently drop registry events emitted before Bracken was deployed.
...
export const brackenAbi = Bracken__factory.abi
...
export const ticketTokenAbi = BrackenTicketToken__factory.abi
export const ciphernodeBondTokenAbi = BrackenToken__factory.abi
