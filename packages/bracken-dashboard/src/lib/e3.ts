...
import { CONTRACTS, DEPLOY_BLOCK, E3Stage, TIMEOUTS, ciphernodeRegistryAbi, brackenAbi, publicClient } from './chain'
...
const BRACKEN_E3_REQUESTED = eventAbi(brackenAbi as any, 'E3Requested')
const BRACKEN_PLAINTEXT_PUBLISHED = eventAbi(brackenAbi as any, 'PlaintextOutputPublished')
const BRACKEN_REWARDS_DISTRIBUTED = eventAbi(brackenAbi as any, 'RewardsDistributed')
// Bracken E3StageChanged event is the reliable, program-agnostic signal for
...
const BRACKEN_E3_STAGE_CHANGED = eventAbi(brackenAbi as any, 'E3StageChanged')
...
// CRISP votes are NOT published through Bracken (its IBracken.InputPublished
...
// Other E3s on the same Bracken deployment run different programs and must not
...
export type E3FullDetails = E3Summary & {
  ...
  // amount currently held for the E3 — note Bracken zeroes it on settlement or
...
export async function fetchE3List(opts: FetchE3Opts = {}): Promise<E3Summary[]> {
  ...
  const logs = await getLogsChunked<any>(
    {
      address: CONTRACTS.Bracken,
      event: BRACKEN_E3_REQUESTED,
    },
    DEPLOY_BLOCK,
    head,
  )
...
export async function fetchE3Details(e3Id: bigint, toBlock?: bigint): Promise<E3FullDetails> {
  ...
  // registry; the key-publish moment from the Bracken E3StageChanged → KeyPublished
  ...
  const [requestedEvents, finalizedEvents, stageChanges] = await Promise.all([
    ...
    getLogsChunked<any>(
      {
        address: CONTRACTS.Bracken,
        event: BRACKEN_E3_STAGE_CHANGED,
        args: { e3Id },
      } as any,
      fromBlock,
      head,
  ...
  // Inputs come from the E3 program, not Bracken. We only understand CRISP's
  ...
  const [inputs, results, rewards] = await Promise.all([
    ...
    getLogsChunked<any>(
      {
        address: CONTRACTS.Bracken,
        event: BRACKEN_PLAINTEXT_PUBLISHED,
        args: { e3Id },
      } as any,
      fromBlock,
      head,
    ...
    getLogsChunked<any>(
      {
        address: CONTRACTS.Bracken,
        event: BRACKEN_REWARDS_DISTRIBUTED,
        args: { e3Id },
      } as any,
      fromBlock,
      head,
