...
2. Deploys all contracts (Bracken, CRISPProgram, verifiers, registries) via
...
The contract addresses are automatically configured in `bracken.config.yaml` and the server `.env`
