...
| **BondingRegistry**    | Handles ciphernode bonds (LOX) and ticket balances (tLOX)           |
...
| **LoxleyToken**        | LOX token used for ciphernode bonding                               |
| **LoxleyTicketToken**  | Non-transferable tLOX token representing ticket balances            |
...
| LoxleyToken (LOX)         | `0xTBD_NOT_DEPLOYED` | 25473449     |
...
| LoxleyToken (LOX)    | `0xTBD_NOT_DEPLOYED` | 11458986     |
...
| `acceptBondOwner(operator)`                             | Nominated owner        | Accept the position and move the LOX accounting          |
| `bondCiphernodeFor(operator, amount)`                   | Bond owner             | Bond LOX from the caller's wallet                        |
| `unbondCiphernodeFor(operator, amount)`                 | Bond owner             | Queue bonded LOX for exit                                |
...
| `totalBonded(account)`         | LOX that counts toward the locked-floor collateral of an owner    |
...
| **Bonded**      | LOX bonded but not yet registered in the registry                                  |
...
| Requirement           | Details                                                                                    |
| --------------------- | ------------------------------------------------------------------------------------------ |
| **LOX Tokens**        | Mainnet requires at least `32,000 LOX` for the ciphernode bond                             |
