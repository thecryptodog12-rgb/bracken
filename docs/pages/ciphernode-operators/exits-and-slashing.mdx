...
struct SlashPolicy {
  ...
  uint256 ciphernodeBondPenalty; // LOX to confiscate
...
| Penalty Type            | Effect                                           |
| ----------------------- | ------------------------------------------------ |
| Ticket penalty          | Burns tLOX from your balance                    |
| Ciphernode bond penalty | Confiscates LOX from your bond                  |
...
| Ciphernode bond | LOX enters exit queue                             |
...
The ticket collateral and LOX are paid to the bond owner, while the exit queue and any slashing
...
| `unbondCiphernodeFor(operator, amount)`                 | Bond owner     | Queue LOX for exit                                   |
