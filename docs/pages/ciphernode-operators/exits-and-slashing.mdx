...
import { Slashing } from '@loxley/diagrams'
...
struct SlashPolicy {
  ...
  uint256 ciphernodeBondPenalty; // LOXLEY to confiscate
...
| Penalty Type            | Effect                                           |
| ----------------------- | ------------------------------------------------ |
| Ticket penalty          | Burns tLOXLEY from your balance                  |
| Ciphernode bond penalty | Confiscates LOXLEY from your bond                |
...
| Ciphernode bond | LOXLEY enters exit queue                           |
...
The ticket collateral and LOXLEY are paid to the bond owner, while the exit queue and any slashing
...
| ------------------------------------------------------- | -------------- | ----------------------------------------------------- |
| `loxley ciphernode status`                              | Operator       | Check registration and exit status                    |
...
| `unbondCiphernodeFor(operator, amount)`                 | Bond owner     | Queue LOXLEY for exit                                 |
...
| Check                 | Command / Event                                 |
| --------------------- | ----------------------------------------------- |
| Exit queue status     | `loxley ciphernode status`                      |
