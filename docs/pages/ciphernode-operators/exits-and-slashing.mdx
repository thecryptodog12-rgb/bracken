...
import { Slashing } from '@bracken/diagrams'

# Exits, Rewards & Slashing

<Slashing />

The rest of this page is the commands and events behind each step.
...
struct SlashPolicy {
  ...
  uint256 ciphernodeBondPenalty; // BRACKEN to confiscate
...
| Penalty Type            | Effect                                           |
| ----------------------- | ------------------------------------------------ |
| Ticket penalty          | Burns tBRACKEN from your balance                 |
| Ciphernode bond penalty | Confiscates BRACKEN from your bond               |
...
| Ciphernode bond | BRACKEN enters exit queue                          |
...
The ticket collateral and BRACKEN are paid to the bond owner, while the exit queue and any slashing
...
| ------------------------------------------------------- | -------------- | ----------------------------------------------------- |
| `bracken ciphernode status`                             | Operator       | Check registration and exit status                    |
...
| `unbondCiphernodeFor(operator, amount)`                 | Bond owner     | Queue BRACKEN for exit                                |
...
| Check                 | Command / Event                                 |
| --------------------- | ----------------------------------------------- |
| Exit queue status     | `bracken ciphernode status`                     |
