# Building with the Loxley

Loxley smart contract acts as the central coordinator for all E3 operations. It manages computation
...
Loxley contract exposes these read-only functions for querying E3 state:
...
published, the CiphernodeRegistry calls `onCommitteePublished()` on the Loxley contract, which
...
Loxley uses a Lean Merkle Tree implementation. Each input is hashed using the PoseidonT3 Hash
...
2. Loxley uses the E3 Program contract to verify the proof and emits `CiphertextOutputPublished`.
...
```javascript
const loxleyContract = new ethers.Contract(loxleyAddress, loxleyAbi, signer)
...
const e3Id = receipt.logs
  .map((log) => loxleyContract.interface.parseLog(log))
...
```javascript
loxleyContract.on('PlaintextOutputPublished', (e3Id, plaintext, proof) => {
...
Loxley protocol will accept and decrypt whatever output is reported by your Compute Provider. In
...
If you are consuming Loxley outputs offchain, allow for sufficient block confirmations to be
