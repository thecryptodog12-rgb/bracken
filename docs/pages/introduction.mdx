## Welcome to the Bracken Developer Docs!

> **Status**
>
> Bracken is deployed on Robinhood Chain (chain id `4663`). Every contract address is listed on
...
**Here you'll find guides, references, and implementation resources for building on the Bracken
protocol.**
...
Bracken is an open-source protocol that coordinates Encrypted Execution Environments (E3s) through a
...
For example, the Bracken can coordinate sealed-bid auctions where participants submit encrypted
bids, the winning outcome is computed without revealing individual inputs, and the result is
publicly verifiable.

### Why Use the Bracken?

Bracken enables applications that coordinate computation across independent parties without exposing
...
Bracken coordinates encrypted execution using a combination of Fully Homomorphic Encryption (FHE),
...
[Cryptography](/cryptography). To compile and lint the Noir workspace, run `bracken noir`, and wire
the prover from Rust, see [Noir Circuits](/noir-circuits).

### What You'll Learn

This documentation covers everything you need to get started with the Bracken, including:

- Core concepts behind the Bracken protocol and E3s.
- Building applications on the Bracken protocol.
