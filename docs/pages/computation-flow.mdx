...
The five phases below are the same five stages, in detail.

The computation process within the Loxley protocol is decentralized and open, allowing any
...
1. **Define the E3 Program**: Author your Secure Process using the tooling described in
   [Writing the Secure Process](/write-secure-program). Loxley currently ships first-party support
...
Each new request to the Loxley contracts initiates a verifiable sortition process to select a
...
publishes the encrypted output back to Loxley contract. This must happen before the compute deadline
expires.

1. **Execution**: The CP retrieves encrypted inputs and executes the Secure Process defined in your
   E3 program.
2. **Publish Output**: The ciphertext output and accompanying proof are submitted to the Loxley
   contract via `publishCiphertextOutput`. Loxley contract calls your E3 Program's `verify` function
   to validate the proof before accepting the output.
...
accepting the output. The plaintext output can be queried from the Loxley contract's `getE3()`
function.
...
[Noir Circuits](/noir-circuits) (build, lint, `loxley noir`, Rust prover integration).
...
sentinel used for bounds-checking in `ILoxley`):
...
flow in `IE3RefundManager` and `ILoxley`.
