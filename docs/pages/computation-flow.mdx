...
import { Lifecycle } from '@bracken/diagrams'
...
The computation process within the Bracken protocol is decentralized and open, allowing any
...
1. **Define the E3 Program**: Author your Secure Process using the tooling described in
   [Writing the Secure Process](/write-secure-program). Bracken currently ships first-party support
...
Each new request to the Bracken contracts initiates a verifiable sortition process to select a
...
publishes the encrypted output back to Bracken contract. This must happen before the compute
deadline expires.

1. **Execution**: The CP retrieves encrypted inputs and executes the Secure Process defined in your
   E3 program.
2. **Publish Output**: The ciphertext output and accompanying proof are submitted to the Bracken
   contract via `publishCiphertextOutput`. Bracken contract calls your E3 Program's `verify`
...
accepting the output. The plaintext output can be queried from the Bracken contract's `getE3()`
function.
...
[Noir Circuits](/noir-circuits) (build, lint, `bracken noir`, Rust prover integration).
...
sentinel used for bounds-checking in `IBracken`):
...
flow in `IE3RefundManager` and `IBracken`.
