...
When a new E3 is requested, the Loxley contract calls `validate` on your program. This is where
...
- `customParams` carries application-specific configuration — CRISP uses it for voting options and
  credit mode
- The function returns an encryption scheme identifier so the Loxley contract knows which scheme
...
Each time a user submits encrypted data to the E3, the Loxley contract calls `publishInput`. This
...
After the compute provider publishes its result, the Loxley contract calls `verify` on your
