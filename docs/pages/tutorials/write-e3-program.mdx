...
Protocol), a private voting application built on the Bracken, to show how each component of an E3
...
Every E3 program implements the `IE3Program` interface from `@bracken/contracts`. It has three
functions that the Bracken contract calls at different stages of the E3 lifecycle:
...
When a new E3 is requested, the Bracken contract calls `validate` on your program. This is where you
...
- Only the Bracken contract (or the program owner) can call `validate`
- `customParams` carries application-specific configuration — CRISP uses it for voting options and
  credit mode
- The function returns an encryption scheme identifier so the Bracken contract knows which scheme
...
Each time a user submits encrypted data to the E3, the Bracken contract calls `publishInput`. This
...
function publishInput(uint256 e3Id, bytes memory data) external {
  ...
  if (bracken.getE3Stage(e3Id) != IBracken.E3Stage.KeyPublished) revert KeyNotPublished(e3Id);
...
After the compute provider publishes its result, the Bracken contract calls `verify` on your
...
sequenceDiagram
    ...
    participant E as Bracken contract
...
1. **Implement `IE3Program`** — start from the
   [MockE3Program](https://github.com/thecryptodog12-rgb/bracken/blob/main/packages/bracken-contracts/contracts/test/MockE3Program.sol)
