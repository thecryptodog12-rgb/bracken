...
Protocol), a private voting application built on the Loxley, to show how each component of an E3
...
Every E3 program implements the `IE3Program` interface from `@loxley/contracts`. It has three
functions that the Loxley contract calls at different stages of the E3 lifecycle:
...
When a new E3 is requested, the Loxley contract calls `validate` on your program. This is where you
...
- Only the Loxley contract (or the program owner) can call `validate`
- `customParams` carries application-specific configuration — CRISP uses it for voting options and
  credit mode
- The function returns an encryption scheme identifier so the Loxley contract knows which scheme the
  ciphernodes should use

---

## Step 2: publishInput — Validate User Inputs

Each time a user submits encrypted data to the E3, the Loxley contract calls `publishInput`. This is
where you enforce application-specific rules.
...
function publishInput(uint256 e3Id, bytes memory data) external {
  ...
  if (loxley.getE3Stage(e3Id) != ILoxley.E3Stage.KeyPublished) revert KeyNotPublished(e3Id);
...
After the compute provider publishes its result, the Loxley contract calls `verify` on your program.
CRISP's implementation checks the RISC Zero proof:
...
function verify(
  ...
  // The Secure Process derives this root from the ciphertexts it processed, and the proof envelope
  // carries that derived value. Compare it against the root this program accumulated on chain: the
  // protocol verifier does not, so this is the only check that the result covers your inputs.
...
sequenceDiagram
    ...
    participant E as Loxley contract
...
1. **Implement `IE3Program`** — start from the
   [MockE3Program](https://github.com/thecryptodog12-rgb/interfold/blob/rebrand/loxley/packages/loxley-contracts/contracts/test/MockE3Program.sol)
