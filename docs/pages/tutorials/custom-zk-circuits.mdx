...
**Mask vote** (for
[receipt-freeness](https://blog.theloxley.com/vote-masking-receipt-freeness-secret-ballots/)) — the
addend is the ciphertext already in the slot, so the vote there is unchanged. Additionally proves:
...
- **Mask vote**: Verifies a zero-value ballot, in every plaintext coefficient. The addend is the
  ...
  [receipt-freeness](https://blog.theloxley.com/vote-masking-receipt-freeness-secret-ballots/) —
...
[crisp-zk-inputs README](https://github.com/theloxley/loxley/tree/main/examples/CRISP/packages/crisp-zk-inputs)
for the full API.
...
you can reuse the user-data-encryption circuit and the standard Loxley SDK.
