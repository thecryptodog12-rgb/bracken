This section provides a breakdown of the Loxley protocol architecture, focusing on the core
...
- Interfaces with the Loxley contract
...
- Manages operator ciphernode bonds (LOXLEY tokens) and ticket balances (tLOXLEY)
...
- Reports computation results of the E3P to the Loxley contract

Loxley supports multiple CPs with different trust, scalability, and cost tradeoffs.
...
Every E3 transitions through a well-defined state machine managed by the Loxley contract:
...
decryption window) on the Loxley contract. The committee formation window is managed separately by
...
- **Loxley smart contracts**: To submit computation requests and retrieve results.
