...
description: 'Build client applications and coordination servers using the Loxley SDK'
---
...
for managing the E3 lifecycle. The Loxley SDK provides powerful TypeScript and React tools to
...
The Loxley SDK handles much of this complexity for you, providing:
...
```bash
pnpm add @loxley/sdk @loxley/react
```

### Basic TypeScript Client

Create a type-safe client that can interact with Loxley contracts:

```typescript
import { LoxleySDK, LoxleyEventType } from '@loxley/sdk'
...
For React applications, use the `useLoxleySDK` hook for seamless integration:
...
function E3Dashboard() {
  ...
  }, [isInitialized, onLoxleyEvent, off, LoxleyEventType, RegistryEventType]);
  ...
  if (!isInitialized) {
    return <div>Connecting to Loxley...</div>;
  }
...
sdk.onLoxleyEvent(LoxleyEventType.CIPHERTEXT_OUTPUT_PUBLISHED, (event) => {
  console.log('Encrypted computation result available:', event.data)
})

sdk.onLoxleyEvent(LoxleyEventType.PLAINTEXT_OUTPUT_PUBLISHED, (event) => {
...
```typescript
import { RegistryEventType } from '@loxley/sdk'
...
sdk.onLoxleyEvent(RegistryEventType.COMMITTEE_PUBLISHED, (event) => {
  console.log('Committee selection complete:', event.data)
})

sdk.onLoxleyEvent(RegistryEventType.COMMITTEE_FINALIZED, (event) => {
...
class E3CoordinationServer {
  private sdk: LoxleySDK

  constructor(privateKey: string, rpcUrl: string) {
    // Use the static factory for convenience
    this.sdk = LoxleySDK.create({
      rpcUrl,
  ...
  private setupEventListeners() {
    ...
    this.sdk.onLoxleyEvent(LoxleyEventType.E3_REQUESTED, async (event) => {
      const { e3Id } = event.data
      console.log(`New E3 requested: ${e3Id}`)
    })

    // Monitor ciphertext output publication
    this.sdk.onLoxleyEvent(LoxleyEventType.CIPHERTEXT_OUTPUT_PUBLISHED, async (event) => {
      console.log('Ciphertext output published:', event.data)
    })

    // Monitor plaintext output publication
    this.sdk.onLoxleyEvent(LoxleyEventType.PLAINTEXT_OUTPUT_PUBLISHED, async (event) => {
...
const prodSdk = LoxleySDK.create({
  rpcUrl: process.env.RPC_URL!,
  privateKey: process.env.PRIVATE_KEY! as `0x${string}`,
  chain: mainnet,
  contracts: {
    loxley: process.env.LOXLEY_CONTRACT_ADDRESS! as `0x${string}`,
...
The Loxley SDK abstracts away much of the complexity, allowing you to focus on your application's
unique business logic while ensuring robust, type-safe interaction with the Loxley protocol.
