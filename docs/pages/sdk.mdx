...
Loxley SDK (`@loxley/sdk`) and React helpers (`@loxley/react`) provide a batteries-included client
stack for requesting E3 computations, subscribing to protocol events, and coordinating custom
frontends or backend services. Both packages ship in this repository (see `packages/loxley-sdk` and
`packages/loxley-react`).

## Installation

```bash
pnpm add @loxley/sdk           # TypeScript SDK
pnpm add @loxley/react         # React hooks (optional)
```
...
const sdk = new LoxleySDK({
  ...
  walletClient,
  contracts: {
    loxley: import.meta.env.VITE_LOXLEY_ADDRESS,
...
It must match the `paramSet` index registered in the on-chain `Loxley` contract:
...
sdk.onLoxleyEvent(LoxleyEventType.E3_REQUESTED, e3Handler)
...
sdk.off(LoxleyEventType.E3_REQUESTED, e3Handler)
```
...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    onLoxleyEvent(LoxleyEventType.E3_REQUESTED, handle)
    return () => off(LoxleyEventType.E3_REQUESTED, handle)
  }, [isInitialized])
...
import { generatePublicKey, encryptNumber } from '@loxley/sdk'
...
import { EventListener, LoxleyEventType } from '@loxley/sdk/events'
```

All exports are also available from the main `@loxley/sdk` entry point.
...
Provide the Loxley, CiphernodeRegistry, and FeeToken addresses. The template exposes these via
`.env` / `loxley.config.yaml`. For multi-chain apps, instantiate separate SDK instances per chain.

## Working with the template

`loxley init` generates the default template (see [Project Template](/project-template)). The client
already wires `@loxley/sdk` and `@loxley/react`:

- `client/src/sdk.ts` builds the shared SDK instance
- `client/src/hooks/useE3.ts` uses `useLoxleySDK` to expose helper hooks throughout the UI
- `server/src/services/loxley.ts` instantiates the SDK with a server wallet using
  `createWalletClient`
...
SDK on top of or alongside the Loxley SDK. For example,
[CRISP](https://github.com/thecryptodog12-rgb/interfold/tree/rebrand/loxley/examples/CRISP) uses
...
that wraps or complements the core Loxley SDK.
...
- **Gas controls**: pass `gasLimit` to `requestE3`, `publishCiphertextOutput`, or other write
  methods if you want fixed limits when interacting with Loxley on L2s.
...
For additional API surface details see `packages/loxley-sdk/README.md` inside this repo.
