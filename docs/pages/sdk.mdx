...
The Loxley SDK (`@loxley/sdk`) and React helpers (`@loxley/react`) provide a
...
`packages/loxley-sdk` and `packages/loxley-react`).
...
`.env` / `loxley.config.yaml`. For multi-chain apps, instantiate separate SDK instances per
chain.

## Working with the template

`loxley init` generates the default template (see [Project Template](/project-template)). The
client already wires `@loxley/sdk` and `@loxley/react`:
...
[CRISP](https://github.com/theloxley/loxley/tree/main/examples/CRISP) uses `@crisp-e3/sdk`,
which provides:
