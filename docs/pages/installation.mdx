---
title: 'Installation'
description: 'Install and set up the Loxley CLI'
---

# Installation

This guide covers how to install the Loxley CLI and get your development environment set up.

## Prerequisites

Before installing Loxley, make sure you have the following dependency installed:
...
```bash
wget -qO- https://raw.githubusercontent.com/theloxley/loxley/main/install | bash
```
...
Running `loxleyup install` will install the latest version of the Loxley CLI. See
`loxleyup --help` for more options.
...
```bash
nix develop github:theloxley/loxley/{branch,tag,commit}
```

This will build from source using the nix package manager and provide a runtime environemnt for
loxley. It might take some time but the result will be cached on your local nix store for future
execution.

```bash
❯ nix develop github:theloxley/loxley/v0.2.2
❯ loxley --version
loxley 0.2.2
❯ bb --version
...
1. Visit the [releases page](https://github.com/theloxley/loxley/releases)
2. Download the appropriate release archive for your platform:
   - Linux: `loxley-linux-x86_64.tar.gz`
   - macOS: `loxley-macos-aarch64.tar.gz`
3. Extract the binary and place it in your PATH (e.g., `~/.local/bin` or `/usr/local/bin`)
4. Make sure the binary is executable: `chmod +x loxley`
...
If you need to build `loxley` from source:

1. Ensure you have Rust installed (using [rustup](https://rustup.rs/))
2. Clone the repository

```bash
git clone https://github.com/theloxley/loxley.git
cd loxley
```
...
Once you have the Loxley CLI installed, you're ready to:
