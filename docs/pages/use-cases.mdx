...
structure so you can quickly map Loxley building blocks to the outcomes you need.
...
- **Problem**: Public DAOs struggle to keep tallies, delegations, and vote rationales private during
  sensitive decisions.
- **Pattern**: Use the CRISP template (client, Rust server, Noir circuits) with the Loxley SDK.
...
- **Pattern**: Treat each agent as a Data Provider, push policy enforcement into the E3 Program's
  `publishInput` validation, and let the Loxley host shared state transitions (e.g., allocating
