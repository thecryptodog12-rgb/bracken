...
title: 'Noir Circuits'
description:
  'Toolchain, repo layout, compile/lint scripts, and integration hooks for Bracken Noir circuits'
---
...
[`agent/flow-trace/04_DKG_AND_COMPUTATION.md`](https://github.com/thecryptodog12-rgb/bracken/blob/main/agent/flow-trace/04_DKG_AND_COMPUTATION.md).
...
- **bb** — the [Barretenberg](https://barretenberg.aztec.network/docs/getting_started) proving
  backend, installed through the Bracken CLI
...
`bracken noir setup` fetches the Barretenberg version pinned in `crates/zk-prover/versions.json`
(`required_bb_version`), so your prover and your local builds agree:

```bash
bracken noir status
bracken noir setup
bracken noir setup --force   # reinstall
```

The CLI keeps this copy under `~/.bracken/noir` instead of on your `PATH`, so `bracken noir status`
is what reports the installed version.
...
[`scripts/README.md`](https://github.com/thecryptodog12-rgb/bracken/blob/main/scripts/README.md#circuit-builder)
and
[`circuits/benchmarks/README.md`](https://github.com/thecryptodog12-rgb/bracken/blob/main/circuits/benchmarks/README.md).
...
- **`e3-zk-prover`** (`crates/zk-prover`) — witness wiring, `bb prove` / `bb verify`, artifact paths
  aligned with `bracken noir setup`.
