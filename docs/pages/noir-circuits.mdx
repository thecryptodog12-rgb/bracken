...
title: 'Noir Circuits'
description:
  'Toolchain, repo layout, compile/lint scripts, and integration hooks for Loxley Noir circuits'
---
...
[`agent/flow-trace/04_DKG_AND_COMPUTATION.md`](https://github.com/thecryptodog12-rgb/interfold/blob/rebrand/loxley/agent/flow-trace/04_DKG_AND_COMPUTATION.md).
...
in the open-source tree—Noir sources are the source of truth; treat docs as orientation only.
...
- **bb** — the [Barretenberg](https://barretenberg.aztec.network/docs/getting_started) proving
  backend, installed through the Loxley CLI
...
`loxley noir setup` fetches the Barretenberg version pinned in `crates/zk-prover/versions.json`
(`required_bb_version`), so your prover and your local builds agree:

```bash
loxley noir status
loxley noir setup
loxley noir setup --force   # reinstall
```

The CLI keeps this copy under `~/.loxley/noir` instead of on your `PATH`, so `loxley noir status` is
what reports the installed version.
...
[`scripts/README.md`](https://github.com/thecryptodog12-rgb/interfold/blob/rebrand/loxley/scripts/README.md#circuit-builder)
and
[`circuits/benchmarks/README.md`](https://github.com/thecryptodog12-rgb/interfold/blob/rebrand/loxley/circuits/benchmarks/README.md).
...
- **`e3-zk-prover`** (`crates/zk-prover`) — witness wiring, `bb prove` / `bb verify`, artifact paths
  aligned with `loxley noir setup`.
