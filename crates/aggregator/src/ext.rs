...
use e3_events::{BusHandle, EType, BrackenEvent, BrackenEventData};
...
impl E3Extension for PublicKeyAggregatorExtension {
    fn on_event(&self, ctx: &mut E3Context, evt: &BrackenEvent) {
        ...
        let BrackenEventData::CiphernodeSelected(data) = evt.get_data() else {
            return;
        };
...
impl E3Extension for ThresholdPlaintextAggregatorExtension {
    fn on_event(&self, ctx: &mut E3Context, evt: &BrackenEvent) {
        if let BrackenEventData::AggregatorChanged(data) = evt.get_data() {
        ...
        if matches!(evt.get_data(), BrackenEventData::CiphernodeSelected(_)) {
            if let Some(ciphertext) = ctx.get_dependency(PENDING_CIPHERTEXT_OUTPUT_KEY).cloned() {
                self.try_start_plaintext(ctx, &ciphertext);
            }
            return;
        ...
        if let BrackenEventData::PublicKeyAggregated(data) = evt.get_data() {
        ...
        if let BrackenEventData::CommitteePublished(data) = evt.get_data() {
        ...
        let BrackenEventData::CiphertextOutputPublished(data) = evt.get_data() else {
            return;
        };
