...
async fn effects_enabled_gates_event_subscriptions() -> anyhow::Result<()> {
    ...
    let runner = e3_events::run_once::<EffectsEnabled>({
        ...
        move |_| {
            ...
            impl Handler<BrackenEvent> for Counter {
                type Result = ();
                fn handle(&mut self, msg: BrackenEvent, _: &mut Self::Context) -> Self::Result {
                    if matches!(msg.get_data(), BrackenEventData::TestEvent(_)) {
    ...
    bus.event_bus().try_send(
        BrackenEvent::<Unsequenced>::test_event("before-effects")
            .id(1)
            .seq(1)
            .build(),
    )?;
    ...
    bus.event_bus().try_send(
        BrackenEvent::<Unsequenced>::test_event("after-effects")
            .id(2)
            .seq(2)
            .build(),
    )?;
...
async fn immediate_subscriptions_receive_before_effects_enabled() -> anyhow::Result<()> {
    ...
    bus.event_bus().try_send(
        BrackenEvent::<Unsequenced>::test_event("during-replay")
            .id(1)
            .seq(1)
            .build(),
    )?;
