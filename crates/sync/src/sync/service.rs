...
use e3_events::{
    ...
    HistoricalEvmEventsReceived, HistoricalEvmSyncStart, HistoricalNetSyncStart, BrackenEvent,
    BrackenEventData, SeqAgg, StoreKeys, SyncEnded, Unsequenced,
};
#[cfg(test)]
...
pub async fn sync(
    ...
    // non-destructively via `bracken node validate`, which cross-checks the persisted committee
    ...
    let BrackenEventData::HistoricalNetSyncEventsReceived(event) =
...
async fn publish_reconciled_history(
    bus: &BusHandle,
    historical: Vec<BrackenEvent<Unsequenced>>,
) -> Result<()> {
...
#[cfg(test)]
async fn replay_eventstore_events(bus: &BusHandle, mut events: Vec<BrackenEvent>) -> Result<usize> {
