...
use e3_events::{
    ...
    EventContextSeq, EventStoreQueryBy, EventStoreQueryResponse, BrackenEvent, SeqAgg,
};
...
fn event_order_key(event: &BrackenEvent) -> (u128, AggregateId, u64) {
    (event.ts(), event.aggregate_id(), event.seq())
}

fn write_run(events: Vec<BrackenEvent>) -> Result<NamedTempFile> {
...
fn write_event(writer: &mut impl Write, event: &BrackenEvent) -> Result<()> {
...
fn read_event(reader: &mut impl Read) -> Result<Option<BrackenEvent>> {
...
impl RunMerger {
    ...
    fn next_event(&mut self) -> Result<Option<BrackenEvent>> {
...
mod tests {
    ...
    fn event(aggregate: u64, sequence: u64, timestamp: u128) -> BrackenEvent<Sequenced> {
        BrackenEvent::<Sequenced>::test_event("spooled")
