...
const GITHUB_REPO: &str = "gnosisguild/loxley";
const BINARY_NAME: &str = "loxley";

#[derive(Parser)]
#[command(
    name = "loxleyup",
    about = "Installer for the Loxley CLI tool",
    version = "0.1.0"
)]
struct Cli {
    #[command(subcommand)]
...
#[derive(Subcommand)]
enum Commands {
    /// Install the latest version of loxley
    Install {
        /// Install to /usr/local/bin instead of ~/.local/bin
        #[arg(long)]
        system: bool,
    },
    /// Update loxley to the latest version
    Update {
        /// Install to /usr/local/bin instead of ~/.local/bin
        #[arg(long)]
        system: bool,
    },
    /// Remove the installed loxley binary
    Uninstall {
