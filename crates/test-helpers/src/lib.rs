...
use e3_events::{
    ...
    HistoryCollector, BrackenEvent, BrackenEventData, Seed, Subscribe,
};
...
pub async fn find_bb() -> Option<PathBuf> {
    ...
    if let Ok(home) = std::env::var("HOME") {
        for path in [
            ...
            format!("{}/.bracken/noir/bin/bb", home),
        ] {
...
pub fn get_common_setup(
    ...
    let errors = HistoryCollector::<BrackenEvent>::new().start();
...
impl AddToCommittee {
    ...
    pub async fn add(&mut self, address: &str) -> Result<BrackenEventData> {
