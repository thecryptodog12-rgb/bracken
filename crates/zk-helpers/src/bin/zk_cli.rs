...
//!
//! **Share-computation (C2) configs.nr:** set `BRACKEN_CIRCUITS_ROOT` to the repo `circuits`
//! directory (or run from the Bracken repo so it is auto-discovered). After `pnpm build:circuits`,
...
//! `scripts/build-circuits.ts`). If `BRACKEN_CIRCUITS_ROOT` is set and those files are missing,
