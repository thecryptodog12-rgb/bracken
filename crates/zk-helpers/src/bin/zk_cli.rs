...
//!
//! **Share-computation (C2) configs.nr:** set `LOXLEY_CIRCUITS_ROOT` to the repo `circuits`
//! directory (or run from the Loxley repo so it is auto-discovered). After `pnpm build:circuits`,
...
//! `scripts/build-circuits.ts`). If `LOXLEY_CIRCUITS_ROOT` is set and those files are missing,
