...
macro_rules! impl_event_types {
    ($($variant:ident),* $(,)?) => {
        ...
        // Implement From<LoxleyEventData> for EventType
        impl From<&LoxleyEventData> for EventType {
            fn from(data: &LoxleyEventData) -> Self {
                match data {
                    $(LoxleyEventData::$variant(_) => EventType::$variant,)*
                }
            }
        }
        ...
        $(
            impl From<$variant> for LoxleyEventData {
                fn from(data: $variant) -> Self {
                    LoxleyEventData::$variant(data)
                }
            }
        )*
    };
...
pub struct LoxleyEvent<S: SeqState = Sequenced> {
    payload: LoxleyEventData,
    ctx: EventContext<S>,
}

impl<S> LoxleyEvent<S>
where
    S: SeqState,
{
    ...
    pub fn split(self) -> (LoxleyEventData, u128) {
        (self.payload, self.ctx.ts())
    }

    pub fn into_components(self) -> (LoxleyEventData, EventContext<S>) {
...
impl<S: SeqState> EventContextAccessors for LoxleyEvent<S> {
...
impl EventContextSeq for LoxleyEvent<Sequenced> {
    fn seq(&self) -> u64 {
        self.ctx.seq()
    }
}
...
impl LoxleyEvent<Sequenced> {
    pub fn clone_unsequenced(&self) -> LoxleyEvent<Unsequenced> {
        let ts = self.ts();
        let block = self.block();
        let data = self.clone().into_data();
        LoxleyEvent::new_with_timestamp(data, Some(self.ctx.clone()), ts, block, self.source())
    }
...
impl LoxleyEvent<Unsequenced> {
    pub fn into_sequenced(self, seq: u64) -> LoxleyEvent<Sequenced> {
        LoxleyEvent::<Sequenced> {
...
impl TryFrom<Vec<u8>> for LoxleyEvent<Unsequenced> {
    type Error = bincode::Error;

    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        LoxleyEvent::from_bytes(&value)
    }
}
...
mod serialization_tests {
    ...
    fn event_decode_rejects_trailing_bytes() {
        ...
        assert!(LoxleyEvent::<Unsequenced>::from_bytes(&bytes).is_err());
    }
}
...
impl LoxleyEvent<Sequenced> {
    ...
    /// test-helpers only utility function to create a new sequenced event
    pub fn from_data_ec(data: LoxleyEventData, ec: EventContext<Sequenced>) -> Self {
    ...
    pub fn strip_ts(&self) -> LoxleyEvent {
        LoxleyEvent::new_stored_event(self.get_data().clone(), 0, self.seq())
    }
}

impl<S: SeqState> Event for LoxleyEvent<S> {
    type Id = EventId;
    type Data = LoxleyEventData;
    ...
    fn get_data(&self) -> &LoxleyEventData {
        &self.payload
    }

    fn into_data(self) -> LoxleyEventData {
        self.payload
    }
}

impl ErrorEvent for LoxleyEvent<Unsequenced> {
    type ErrType = EType;
    ...
    fn from_error(
        ...
        let payload = LoxleyError::new(err_type, msg);
...
impl<S: SeqState> From<LoxleyEvent<S>> for EventId {
    fn from(value: LoxleyEvent<S>) -> Self {
        value.ctx.id()
    }
}
...
impl LoxleyEventData {
    pub fn get_e3_id(&self) -> Option<E3id> {
        match self {
            LoxleyEventData::AccusationQuorumReached(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::AccusationVote(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::AggregatorChanged(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ProofFailureAccusation(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ProofVerificationFailed(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ProofVerificationPassed(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::KeyshareCreated(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::E3Requested(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::E3RequestComplete(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::PublicKeyAggregated(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CiphertextOutputPublished(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::DecryptionKeyShared(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::DecryptionshareCreated(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::PlaintextAggregated(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::PkGenerationProofSigned(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::DkgProofSigned(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CiphernodeSelected(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ThresholdShareCreated(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ThresholdSharePending(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::EncryptionKeyPending(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::EncryptionKeyReceived(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteePublished(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteeRequested(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteeFinalizeRequested(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::PlaintextOutputPublished(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteeFinalized(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::TicketGenerated(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::TicketSubmitted(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::EncryptionKeyCreated(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ComputeResponse(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::TestEvent(ref data) => data.e3_id.clone(),
            LoxleyEventData::SignedProofFailed(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::DecryptionShareProofsPending(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ShareVerificationDispatched(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ShareVerificationComplete(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::SlashExecuted(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteeMemberExpelled(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteeMemberExcluded(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::E3Failed(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::E3StageChanged(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::DecryptionShareProofSigned(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::ShareDecryptionProofPending(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::PkAggregationProofPending(ref data) => Some(data.e3_id.clone()),
            ...
            LoxleyEventData::RewardClaimed(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteeFormationFailed(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteeActivationChanged(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::CommitteeViabilityUpdated(ref data) => Some(data.e3_id.clone()),
            LoxleyEventData::EvmLogObserved(ref data) => data.e3_id.clone(),
            LoxleyEventData::DkgFoldAttestationContextEstablished(ref data) => {
                Some(data.e3_id.clone())
            }
            _ => None,
        }
...
impl WithAggregateId for LoxleyEventData {
    fn get_aggregate_id(&self) -> AggregateId {
        if let LoxleyEventData::EvmLogObserved(event) = self {
...
impl<S: SeqState> LoxleyEvent<S> {
    pub fn get_e3_id(&self) -> Option<E3id> {
        self.payload.get_e3_id()
    }
}

impl<S: SeqState> WithAggregateId for LoxleyEvent<S> {
    fn get_aggregate_id(&self) -> AggregateId {
...
impl TryFrom<&LoxleyEvent<Sequenced>> for LoxleyError {
    type Error = anyhow::Error;
    fn try_from(value: &LoxleyEvent) -> Result<Self, Self::Error> {
...
impl TryFrom<LoxleyEvent<Sequenced>> for LoxleyError {
    ...
    fn try_from(value: LoxleyEvent<Sequenced>) -> Result<Self, Self::Error> {
        if let LoxleyEventData::LoxleyError(data) = value.payload.clone() {
            Ok(data)
        } else {
            Err(anyhow::anyhow!("Not an loxley error {:?}", value))
        }
    }
}
impl From<LoxleyEvent<Sequenced>> for EventContext<Sequenced> {
    fn from(value: LoxleyEvent) -> Self {
        (&value).into()
    }
}
impl From<&LoxleyEvent<Sequenced>> for EventContext<Sequenced> {
...
// Add convenience method to LoxleyEvent
impl<S: SeqState> LoxleyEvent<S> {
...
impl<S: SeqState> fmt::Display for LoxleyEvent<S> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let t = self.event_type();
        let colorized_debug = colorize_event_ids(self);

        let s = match t.as_str() {
            "LoxleyError" => format!("{} {}", colorize(t, Color::Red), colorized_debug),
...
impl EventConstructorWithTimestamp for LoxleyEvent<Unsequenced> {
    fn new_with_timestamp(
        ...
        let payload: LoxleyEventData = data;
        let id = EventId::hash(&payload);
...
impl<S: SeqState> TestEventBuilder<S> {
    ...
    /// Ensure the event holds the given LoxleyEventData object. This overrides all other params
    /// aiside from seq(n)
    pub fn data(mut self, data: impl Into<LoxleyEventData>) -> Self {
        self.data = Some(data.into());
        self
    }

    fn get_built_event(self) -> LoxleyEvent<Unsequenced> {
...
impl TestEventBuilder<Unsequenced> {
    /// Build the event
    pub fn build(self) -> LoxleyEvent<Unsequenced> {
...
impl TestEventBuilder<Sequenced> {
    /// Build the event
    pub fn build(self) -> LoxleyEvent<Sequenced> {
        let seq = self.seq;
