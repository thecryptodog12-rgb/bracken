...
use super::{LoxleyEvent, Unsequenced};
...
#[rtype(result = "()")]
pub struct LoxleyError {
    pub err_type: EType,
    pub message: String,
}

impl Display for LoxleyError {
...
impl FromError for LoxleyError {
    fn from_error(err_type: EType, error: impl Into<String>) -> Self {
        Self {
            err_type,
            message: error.into(),
...
pub fn trap<F>(err_type: EType, bus: &dyn ErrorDispatcher<LoxleyEvent<Unsequenced>>, runner: F)
where
...
impl ErrorDispatcher<LoxleyEvent<Unsequenced>> for PanicDispatcher {
...
impl ErrorDispatcher<LoxleyEvent<Unsequenced>> for WarningDispatcher {
...
impl ErrorDispatcher<LoxleyEvent<Unsequenced>> for LogErrorDispatcher {
    fn err(&self, err_type: EType, error: anyhow::Error) {
