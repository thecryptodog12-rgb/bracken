...
use crate::{
    ...
    EventStoreQueryResponse, BrackenEvent, Seq, SequenceIndex, Sequenced, Ts, Unsequenced,
};
...
impl<I: SequenceIndex, L: EventLog> EventStore<I, L> {
    ...
    ) -> Result<Option<BrackenEvent<Sequenced>>> {
        let ts = event.ts();
    ...
    fn collect_events(
        &self,
        iter: Box<dyn Iterator<Item = (u64, BrackenEvent<Unsequenced>)>>,
        filter: Option<EventStoreFilter>,
        limit: Option<u64>,
    ) -> Vec<BrackenEvent<Sequenced>> {
    ...
    pub fn query_by_ts(
        ...
        filter: Option<EventStoreFilter>,
        limit: Option<u64>,
    ) -> Result<Vec<BrackenEvent<Sequenced>>> {
                ...
                if seq != next_sequence {
                    bail!(
                        "Event log integrity failure during index reconciliation: expected sequence \
                         {next_sequence}, got {seq}. Halting; run `bracken node validate` and \
            ...
            if page_len == 0 {
                bail!(
                    "Event log integrity failure during index reconciliation: log head is {head}, \
                     ...
                     `bracken node validate` and recover from a verified backup or controlled \
                     resync."
                );
            }
...
mod tests {
    ...
    // Test helpers
    // ---------------------------------------------------------------------------
    fn make_event(ts: u128, source: EventSource) -> BrackenEvent<Unsequenced> {
    ...
    fn make_distinct_event(ts: u128, source: EventSource) -> BrackenEvent<Unsequenced> {
        BrackenEvent::<Unsequenced>::new_with_timestamp(
    ...
    fn make_local_event(ts: u128) -> BrackenEvent<Unsequenced> {
        make_event(ts, EventSource::Local)
    }

    fn make_network_event(ts: u128) -> BrackenEvent<Unsequenced> {
    ...
    fn populated_store(events: &[BrackenEvent<Unsequenced>]) -> EventStore<MockIndex, MockLog> {
