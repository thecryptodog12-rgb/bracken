...
use super::*;
use e3_events::{
    E3Failed, E3RequestComplete, E3Stage, E3StageChanged, FailureReason, BrackenEvent,
...
fn with_e3_id(label: &str, id: E3id) -> BrackenEvent {
...
fn from_data(data: impl Into<BrackenEventData>) -> BrackenEvent {
    BrackenEvent::<Sequenced>::test_event("x")
        .data(data)
        .seq(1)
        .build()
}
...
fn event_without_e3_id_is_ignored() {
    let msg = BrackenEvent::<Sequenced>::test_event("no-id").seq(1).build();
    assert_eq!(
        RequestRouter::route(&msg, &HashSet::new()),
...
fn e3_request_complete_triggers_teardown() {
    // BrackenEventData::get_e3_id() now returns Some(e3_id) for E3RequestComplete,
...
fn e3_failed(id: E3id, reason: FailureReason) -> BrackenEvent {
    from_data(E3Failed {
        e3_id: id,
