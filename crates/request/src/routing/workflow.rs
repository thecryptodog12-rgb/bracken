...
use e3_events::{E3Stage, E3id, Event, BrackenEvent, BrackenEventData};
...
///
/// Classifies an incoming [`BrackenEvent`] into a [`RoutingDecision`] based purely on the
...
impl RequestRouter {
    ...
    pub fn route(msg: &BrackenEvent, completed: &HashSet<E3id>) -> RoutingDecision {
        ...
        if matches!(
            msg.get_data(),
            BrackenEventData::Shutdown(_) | BrackenEventData::EffectsEnabled(_)
        ) {
        ...
        if completed.contains(&e3_id) {
            ...
            let is_late_terminal = match msg.get_data() {
                ...
                BrackenEventData::E3Failed(data) if data.reason.ends_without_slashing() => true,
                ...
                BrackenEventData::PlaintextOutputPublished(_) => true,
                _ => false,
            };
            if is_late_terminal {
                return RoutingDecision::Ignore;
        ...
        let post_forward = match msg.get_data() {
            ...
            BrackenEventData::PlaintextAggregated(_) => PostForward::PublishComplete,
            BrackenEventData::E3StageChanged(data)
            ...
            BrackenEventData::E3Failed(data) if data.reason.ends_without_slashing() => {
                PostForward::PublishComplete
            }
            BrackenEventData::E3RequestComplete(_) => PostForward::Teardown,
            _ => PostForward::None,
        };
