...
async fn test_notified_of_document() -> Result<()> {
    ...
    let Some(BrackenEventData::DocumentReceived(DocumentReceived { value: doc, .. })) =
        events.iter().find_map(|event| match event.get_data() {
            data @ BrackenEventData::DocumentReceived(_) => Some(data),
            _ => None,
        })
    else {
        bail!("No event sent");
...
async fn notification_cannot_relabel_payload_for_another_e3() -> Result<()> {
    ...
    let error: BrackenError = error_events.events.first().unwrap().try_into()?;
    ...
    assert!(
        !events
            .iter()
            .any(|event| matches!(event.get_data(), BrackenEventData::DocumentReceived(_))),
