...
use e3_events::{
    CorrelationId, DocumentMeta, EventContextAccessors, EventSource, BrackenEvent, Sequenced,
    Unsequenced,
};
...
pub enum GossipData {
    GossipBytes(Vec<u8>), // Serialized BrackenEvent
...
impl TryFrom<BrackenEvent<Sequenced>> for GossipData {
...
impl TryFrom<GossipData> for BrackenEvent<Unsequenced> {
    type Error = anyhow::Error;
    fn try_from(value: GossipData) -> Result<Self, Self::Error> {
        ...
        Ok(BrackenEvent::from_bytes(&bytes)?.with_source(EventSource::Net))
    }
}
...
mod tests {
    use e3_events::{
        EventConstructorWithTimestamp, EventSource, BrackenEvent, Sequenced, TestEvent, Unsequenced,
    };

    use super::GossipData;

    #[test]
    fn test_bracken_event_gossip_lifecycle() -> anyhow::Result<()> {
        ...
        let event: BrackenEvent<Sequenced> = event.into_sequenced(90210);
        ...
        let event: BrackenEvent<Unsequenced> = gossip_data.try_into()?;
