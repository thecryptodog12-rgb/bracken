...
//! Configuration bootstrapping for `loxley setup`.
//!
//! Generates an `loxley.config.yaml` from compiled-in contract deployment
//! addresses. This is a **dev-bootstrap tool** — it couples to the Solidity
//! build artifacts at compile time and is only invoked by the `loxley setup`
...
// Generated by build.rs from packages/loxley-contracts/deployed_contracts.json
...
/// are rotated via the `_dnsaddr.bootstrap.loxley.network` TXT records, so this value
/// does not change between CLI releases.
const BOOTSTRAP_PEER: &str = "/dnsaddr/bootstrap.loxley.network";
...
pub fn execute(rpc_url: &str, address: &Address, config_dir: &PathBuf) -> Result<AppConfig> {
    ...
    let config_path = config_dir.join("loxley.config.yaml");
...
"#,
        ...
        address,
        rpc_url,
        get_contract_info("Loxley")?.address,
        get_contract_info("Loxley")?.deploy_block,
