...
//! Configuration bootstrapping for `bracken setup`.
//!
//! Generates an `bracken.config.yaml` from compiled-in contract deployment
//! addresses. This is a **dev-bootstrap tool** — it couples to the Solidity
//! build artifacts at compile time and is only invoked by the `bracken setup`
...
// Generated by build.rs from packages/bracken-contracts/deployed_contracts.json
...
/// are rotated via the `_dnsaddr.bootstrap.bracken.network` TXT records, so this value
/// does not change between CLI releases.
const BOOTSTRAP_PEER: &str = "/dnsaddr/bootstrap.bracken.network";
...
pub fn execute(rpc_url: &str, address: &Address, config_dir: &PathBuf) -> Result<AppConfig> {
    ...
    let config_path = config_dir.join("bracken.config.yaml");
...
"#,
        ...
        address,
        rpc_url,
        get_contract_info("Bracken")?.address,
        get_contract_info("Bracken")?.deploy_block,
