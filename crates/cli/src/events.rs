...
use e3_events::{compute_seq_cursor, CorrelationId, BrackenEvent, SeqAgg, SeqCursor};
...
async fn fetch_events(
    ...
    since: Option<u64>,
    limit: Option<u64>,
) -> Result<(Vec<BrackenEvent>, SeqCursor)> {
...
fn print_events(out: Console, events: Vec<BrackenEvent>) -> Result<()> {
    for event in events {
