...
use e3_events::{compute_seq_cursor, CorrelationId, LoxleyEvent, SeqAgg, SeqCursor};
...
async fn fetch_events(
    ...
    since: Option<u64>,
    limit: Option<u64>,
) -> Result<(Vec<LoxleyEvent>, SeqCursor)> {
...
fn print_events(out: Console, events: Vec<LoxleyEvent>) -> Result<()> {
    for event in events {
