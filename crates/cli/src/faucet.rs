...
/// Calls `faucet()` on the configured Faucet contract, sending BRACKEN + fee
...
pub async fn execute(out: Console, config: &AppConfig, selection: Option<&str>) -> Result<()> {
    ...
    if !needs_fold && !needs_fee_token {
        log!(
            out,
            "Nothing to claim: {:#x} already holds at least {} BRACKEN and {} fee tokens.",
            recipient,
    ...
    if needs_fold {
        ...
        if faucet_fold < amount_fold {
            bail!(
                "Faucet is out of BRACKEN (has {}, needs {}). Ask an admin to refund it.",
    ...
    log!(
        ...
        if needs_fold {
            format!("{} BRACKEN", format_units(amount_fold, 18))
        } else {
            String::new()
        },
        if needs_fold && needs_fee_token {
