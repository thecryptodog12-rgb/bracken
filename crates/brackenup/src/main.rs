...
const GITHUB_REPO: &str = "gnosisguild/bracken";
const BINARY_NAME: &str = "bracken";

#[derive(Parser)]
#[command(
    name = "brackenup",
    about = "Installer for the Bracken CLI tool",
    version = "0.1.0"
)]
struct Cli {
    #[command(subcommand)]
...
#[derive(Subcommand)]
enum Commands {
    /// Install the latest version of bracken
    Install {
        /// Install to /usr/local/bin instead of ~/.local/bin
        #[arg(long)]
        system: bool,
    },
    /// Update bracken to the latest version
    Update {
        /// Install to /usr/local/bin instead of ~/.local/bin
        #[arg(long)]
        system: bool,
    },
    /// Remove the installed bracken binary
    Uninstall {
