...
//! - Compiled Hardhat artifacts: `cd packages/bracken-contracts && npx hardhat compile`
//!
...
sol! {
    ...
    contract SlashingManager {
        ...
        function setBracken(address newBracken) external;
...
fn contracts_artifacts_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../packages/bracken-contracts/artifacts/contracts")
}
...
async fn deploy_and_configure(
    ...
    // Deploy noop for bracken (void functions)
        ...
        None => {
            println!(
                "skipping: contract artifacts not found \
                 (run `npx hardhat compile` in packages/bracken-contracts)"
            );
            return;
        }
    };
