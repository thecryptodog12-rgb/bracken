...
use e3_events::{E3id, EvmLogObserved, BrackenEventData};
...
mod tests {
    ...
    fn names_known_logs_and_extracts_indexed_e3_id() {
        ...
        let event = observe("Bracken", &LogData::default(), &[topic0, e3_id], 31337);
    ...
    fn unknown_logs_remain_lossless_and_explicit() {
        ...
        let event = observe("Bracken", &data, &[B256::ZERO], 1);
        match event {
            BrackenEventData::EvmLogObserved(event) => {
                assert!(!event.known);
