...
//! Loxley contract publication boundary.
...
/// Consumes events from the event bus and calls EVM methods on the Loxley contract.
pub struct LoxleySolWriter<P> {
...
impl<P: Provider + WalletProvider + Clone + 'static> LoxleySolWriter<P> {
    pub fn new(
        bus: &BusHandle,
        provider: EthProvider<P>,
        contract_address: Address,
    ...
    pub fn attach(bus: &BusHandle, provider: EthProvider<P>, contract_address: Address) {
        let addr = LoxleySolWriter::new(bus, provider, contract_address)
            .expect("failed to create LoxleySolWriter")
            .start();
        bus.subscribe_all(
            &[
...
impl<P: Provider + WalletProvider + Clone + 'static> Actor for LoxleySolWriter<P> {
