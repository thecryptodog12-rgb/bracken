...
//! Pure translation of `Bracken.sol` logs into `BrackenEventData`.

use crate::contracts::IBracken;
...
struct E3RequestedWithChainId(pub IBracken::E3Requested, pub u64);
...
struct CiphertextOutputPublishedWithChainId(pub IBracken::CiphertextOutputPublished, pub u64);
...
impl From<CiphertextOutputPublishedWithChainId> for e3_events::CiphertextOutputPublished {
    fn from(value: CiphertextOutputPublishedWithChainId) -> Self {
        e3_events::CiphertextOutputPublished {
            ...
            // XXX: Ciphertext is an array of bytes this needs to be coordinated with bracken
            // contract
...
impl From<CiphertextOutputPublishedWithChainId> for BrackenEventData {
    fn from(value: CiphertextOutputPublishedWithChainId) -> Self {
...
struct E3FailedWithChainId(pub IBracken::E3Failed, pub u64);
...
impl From<E3FailedWithChainId> for BrackenEventData {
    fn from(value: E3FailedWithChainId) -> Self {
...
struct E3StageChangedWithChainId(pub IBracken::E3StageChanged, pub u64);
...
impl From<E3StageChangedWithChainId> for BrackenEventData {
    fn from(value: E3StageChangedWithChainId) -> Self {
...
struct PlaintextOutputPublishedWithChainId(pub IBracken::PlaintextOutputPublished, pub u64);
...
impl From<PlaintextOutputPublishedWithChainId> for BrackenEventData {
    fn from(value: PlaintextOutputPublishedWithChainId) -> Self {
...
struct InputPublishedWithChainId(pub IBracken::InputPublished, pub u64);
...
impl From<InputPublishedWithChainId> for BrackenEventData {
    fn from(value: InputPublishedWithChainId) -> Self {
        InputPublished::from(value).into()
    }
}

struct RewardsDistributedWithChainId(pub IBracken::RewardsDistributed, pub u64);
...
impl From<RewardsDistributedWithChainId> for BrackenEventData {
    fn from(value: RewardsDistributedWithChainId) -> Self {
        RewardsDistributed::from(value).into()
    }
}

struct RewardCreditedWithChainId(pub IBracken::RewardCredited, pub u64);
...
impl From<RewardCreditedWithChainId> for BrackenEventData {
    fn from(value: RewardCreditedWithChainId) -> Self {
        RewardCredited::from(value).into()
    }
}

struct RewardClaimedWithChainId(pub IBracken::RewardClaimed, pub u64);
...
impl From<RewardClaimedWithChainId> for BrackenEventData {
    fn from(value: RewardClaimedWithChainId) -> Self {
...
pub(crate) fn extractor(
    ...
    topics: &[B256],
    chain_id: u64,
) -> Result<Option<BrackenEventData>> {
    let topic0 = topics.first();
    match topic0 {
        Some(&IBracken::E3Requested::SIGNATURE_HASH) => {
            let event = IBracken::E3Requested::decode_log_data(data)
        ...
        Some(&IBracken::CiphertextOutputPublished::SIGNATURE_HASH) => {
            let mut event = IBracken::CiphertextOutputPublished::decode_log_data(data)
            ...
            Ok(Some(BrackenEventData::from(
                CiphertextOutputPublishedWithChainId(event, chain_id),
            )))
        }
        Some(&IBracken::InputPublished::SIGNATURE_HASH) => {
            let mut event = IBracken::InputPublished::decode_log_data(data)
        ...
        Some(&IBracken::E3Failed::SIGNATURE_HASH) => {
            let mut event = IBracken::E3Failed::decode_log_data(data)
            ...
            Ok(Some(BrackenEventData::from(E3FailedWithChainId(
                event, chain_id,
            ))))
        }
        Some(&IBracken::E3StageChanged::SIGNATURE_HASH) => {
            let mut event = IBracken::E3StageChanged::decode_log_data(data)
            ...
            Ok(Some(BrackenEventData::from(E3StageChangedWithChainId(
                event, chain_id,
            ))))
        }
        Some(&IBracken::PlaintextOutputPublished::SIGNATURE_HASH) => {
            let mut event = IBracken::PlaintextOutputPublished::decode_log_data(data)
        ...
        Some(&IBracken::RewardsDistributed::SIGNATURE_HASH) => {
            let mut event = IBracken::RewardsDistributed::decode_log_data(data)
        ...
        Some(&IBracken::RewardCredited::SIGNATURE_HASH) => {
            let mut event = IBracken::RewardCredited::decode_log_data(data)
        ...
        Some(&IBracken::RewardClaimed::SIGNATURE_HASH) => {
            let mut event = IBracken::RewardClaimed::decode_log_data(data)
        ...
        _ => {
            trace!(
                topic=?topic0,
                "Preserving event without a typed Bracken.sol decoder"
            );
            Ok(Some(crate::domain::evm_log_observation::observe(
                "Bracken", data, topics, chain_id,
            )))
        }
    }
}
...
mod tests {
    ...
    fn test_extractor_decodes_e3_stage_changed() {
        let event = IBracken::E3StageChanged {
        ...
        let out = extractor(
            &log_data,
            &[IBracken::E3StageChanged::SIGNATURE_HASH, e3_id_topic],
            7,
        )
        .unwrap();
        match out {
            Some(BrackenEventData::E3StageChanged(data)) => {
    ...
    fn test_extractor_decodes_input_and_reward_events() {
        let input = IBracken::InputPublished {
            e3Id: U256::from(8),
        ...
        match out {
            Some(BrackenEventData::InputPublished(event)) => {
        ...
        let reward = IBracken::RewardCredited {
            e3Id: U256::from(8),
        ...
        match out {
            Some(BrackenEventData::RewardCredited(event)) => {
                assert_eq!(event.e3_id, E3id::new("8", 10));
    ...
    fn test_extractor_rejects_mismatched_reward_arrays() {
        let event = IBracken::RewardsDistributed {
            e3Id: U256::from(8),
    ...
    fn test_extractor_decodes_plaintext_publication() {
        let event = IBracken::PlaintextOutputPublished {
            e3Id: U256::from(18),
        ...
        match out {
            Some(BrackenEventData::PlaintextOutputPublished(event)) => {
    ...
    fn e3_request_keeps_the_contract_sortition_timepoint() {
        let event = IBracken::E3Requested {
            e3Id: U256::from(19),
            e3: IBracken::E3 {
                seed: U256::ZERO,
                committeeSize: 0,
    ...
    fn malformed_e3_requested_is_rejected() {
        let malformed = LogData::new_unchecked(
            vec![IBracken::E3Requested::SIGNATURE_HASH],
            Default::default(),
        );
