...
//! Bracken contract publication boundary.
...
use e3_events::{
    ...
    EType, EffectsEnabled, EventType, BrackenEvent, BrackenEventData, PlaintextAggregated, Proof,
    Shutdown,
};
...
/// Consumes events from the event bus and calls EVM methods on the Bracken contract.
pub struct BrackenSolWriter<P> {
...
impl<P: Provider + WalletProvider + Clone + 'static> BrackenSolWriter<P> {
    pub fn new(
        bus: &BusHandle,
        provider: EthProvider<P>,
        contract_address: Address,
    ...
    pub fn attach(bus: &BusHandle, provider: EthProvider<P>, contract_address: Address) {
        let addr = BrackenSolWriter::new(bus, provider, contract_address)
            .expect("failed to create BrackenSolWriter")
            .start();
        bus.subscribe_all(
            &[
...
impl<P: Provider + WalletProvider + Clone + 'static> Actor for BrackenSolWriter<P> {
