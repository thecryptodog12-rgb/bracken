...
impl<P: Provider + WalletProvider + Clone + 'static> Handler<BrackenEvent> for BrackenSolWriter<P> {
    type Result = ();
    ...
    fn handle(&mut self, msg: BrackenEvent, ctx: &mut Self::Context) -> Self::Result {
        match msg.into_data() {
            ...
            BrackenEventData::PlaintextAggregated(data) => {
...
impl<P: Provider + WalletProvider + Clone + 'static> Handler<Shutdown> for BrackenSolWriter<P> {
    type Result = ();
