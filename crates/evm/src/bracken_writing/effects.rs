...
//! Bracken contract reads and transaction effects.

use super::*;

pub(in crate::actors::bracken_sol_writer) async fn publish_plaintext_output<
        ...
        || {
            ...
            async move {
                ...
                let current_nonce = provider
                    ...
                    .pending()
                    .await?;
                let contract = IBracken::new(contract_address, provider.provider());
...
pub(in crate::actors::bracken_sol_writer) async fn should_publish_plaintext<
    P: Provider + WalletProvider + Clone,
>(
    provider: EthProvider<P>,
    contract_address: Address,
...
pub(in crate::actors::bracken_sol_writer) async fn process_e3_failure<
    P: Provider + WalletProvider + Clone,
>(
    provider: EthProvider<P>,
    contract_address: Address,
