...
use e3_events::{
    ...
    HistoricalEvmEventsReceived, HistoricalEvmSyncStart, BrackenEvent, BrackenEventData, SyncEnded,
    TestEvent,
};
...
async fn evm_reader() -> Result<()> {
    ...
    let history = history_collector
        .send(GetEvents::<BrackenEvent>::new())
        .await?;

    let msgs: Vec<_> = history
        .into_iter()
        .filter_map(|evt| match evt.into_data() {
            BrackenEventData::TestEvent(data) => Some(data.msg),
            _ => None,
        })
        .collect();
