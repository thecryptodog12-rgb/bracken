...
import { RegistryEventType, type CommitteePublishedData } from '@bracken/sdk/events'
...
let sdkInstance: BrackenSDK | null = null

async function createPrivateSDK(): Promise<BrackenSDK> {
  if (sdkInstance) return sdkInstance

  const { PRIVATE_KEY, CIPHERNODE_REGISTRY_CONTRACT, BRACKEN_CONTRACT, FEE_TOKEN_CONTRACT, RPC_URL } = getCheckedEnvVars()

  sdkInstance = BrackenSDK.create({
    rpcUrl: RPC_URL,
    privateKey: PRIVATE_KEY as `0x${string}`,
    contracts: {
      bracken: BRACKEN_CONTRACT as `0x${string}`,
...
async function runProgram(e3Id: bigint): Promise<void> {
  ...
  const { BRACKEN_CONTRACT, E3_PROGRAM_ADDRESS } = getCheckedEnvVars()
  ...
  const e3ProgramParams = (await publicClient.readContract({
    address: BRACKEN_CONTRACT as `0x${string}`,
    abi: [
      {
  ...
  try {
    ...
    await callFheRunner(
      e3Id,
      {
        ...
        brackenAddress: BRACKEN_CONTRACT,
...
async function setupEventListeners() {
  ...
  await sdk.onBrackenEvent(RegistryEventType.COMMITTEE_PUBLISHED, handleCommitteePublishedEvent)
...
async function startServer() {
  try {
    ...
    app.listen(PORT, '0.0.0.0', () => {
      console.log(`🚀 Bracken coordination server listening on port ${PORT}`)
