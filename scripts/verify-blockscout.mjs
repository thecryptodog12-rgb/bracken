  ...
  const walk = (dir) => {
    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (e.isDirectory()) {
        ...
        const buildInfo = a.buildInfoId
          ? path.join(ROOT, "artifacts", "build-info", `${a.buildInfoId}.json`)
          : null;
        byName.set(a.contractName, { sourceName: a.sourceName, inputSourceName: a.inputSourceName, buildInfo });
      }
    }
  };
...
function closure(sources, remappings, entry) {
  ...
  const resolve = (from, spec) => {
    ...
    for (const r of parsed) {
      ...
      if (!best || r.context.length > best.context.length || (r.context.length === best.context.length && r.prefix.length > best.prefix.length)) {
        best = r;
      }
    }
...
async function verifyOne(name, address, art) {
  ...
  if (!text.includes("verification started")) {
    return { name, status: "afgewezen", detail: text.replace(/\s+/g, " ").slice(0, 120) };
  }
