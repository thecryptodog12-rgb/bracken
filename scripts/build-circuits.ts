...
import { BFV_PARAMS } from '../packages/bracken-contracts/scripts/protocol/constants'
import {
  ALL_COMMITTEES,
  ALL_GROUPS,
  ALL_PRESETS,
...
interface BuildOptions {
  ...
  /** Skip writing BFV_DKG_H/T into `packages/bracken-contracts/scripts/utils.ts`. */
...
class NoirCircuitBuilder {
  ...
  private patchUtilsTs(preset: CircuitPreset, committee: CircuitCommittee): void {
    ...
    const path = join(this.rootDir, 'packages', 'bracken-contracts', 'scripts', 'utils.ts')
 ...
 */`
  ...
  /** Writes the circuit-bound constants consumed by Bracken. */
  private writeActiveCryptoConfig(preset: CircuitPreset, committee: CircuitCommittee): void {
    ...
    const configId = keccak256(
      AbiCoder.defaultAbiCoder().encode(['bytes32', 'bytes32', 'bytes32'], [id('fhe.rs:BFV'), paramSetHash, id('bracken-bfv-v1')]),
    )
    const path = join(this.rootDir, 'packages', 'bracken-contracts', 'contracts', 'lib', 'ActiveCryptoConfig.sol')
...
`
   ...
   * - the Rust integration tests (cross-checked against `BRACKEN_COMMITTEE_SIZE`).
  ...
  private sanitizePaths(jsonFile: string): void {
    try {
      ...
      const sanitized = content
        .replace(/"path"\s*:\s*"[^"]*[/\\](bracken[/\\]circuits[/\\][^"]+)"/g, '"path":"$1"')
        .replace(/"path"\s*:\s*"(?:\/[^"]*|[A-Za-z]:\\[^"]*)[/\\](circuits[/\\][^"]+)"/g, '"path":"bracken/$1"')
...
Options:
  ...
  --skip-utils-patch  Don't rewrite BFV_DKG_H/T in packages/bracken-contracts/scripts/utils.ts
