...
class VerifierGenerator {
  ...
  private defaultVerifierDir(committee: CircuitCommittee): string {
    const honkBase = join(this.rootDir, 'packages', 'loxley-contracts', 'contracts', 'verifiers', 'bfv', 'honk')
   ...
   * project's standard formatting. Run from `packages/loxley-contracts` so
  ...
  private formatSolidity(content: string, outputPath: string): string {
    const contractsDir = join(this.rootDir, 'packages', 'loxley-contracts')
...
circuits and places them in packages/loxley-contracts/contracts/verifiers/bfv/honk/.
