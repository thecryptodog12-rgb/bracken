...
class VerifierGenerator {
  ...
  private defaultVerifierDir(committee: CircuitCommittee): string {
    const honkBase = join(this.rootDir, 'packages', 'bracken-contracts', 'contracts', 'verifiers', 'bfv', 'honk')
   ...
   * project's standard formatting. Run from `packages/bracken-contracts` so
  ...
  private formatSolidity(content: string, outputPath: string): string {
    const contractsDir = join(this.rootDir, 'packages', 'bracken-contracts')
...
circuits and places them in packages/bracken-contracts/contracts/verifiers/bfv/honk/.
