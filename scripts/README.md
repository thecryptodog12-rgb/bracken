...
This directory contains utility scripts for the Loxley project.
...
- `--skip-utils-patch` - Skip rewriting `BFV_DKG_H` / `BFV_THRESHOLD_T` in
  `packages/loxley-contracts/scripts/utils.ts`
...
The generated `.sol` files under `packages/loxley-contracts/contracts/verifiers/bfv/honk/` are
...
6. **Outputs / verifies** at `packages/loxley-contracts/contracts/verifiers/bfv/honk/`:
...
4. Commit the resulting diff under `packages/loxley-contracts/contracts/verifiers/bfv/honk/`.
...
✅ Generated 2 Solidity verifier(s) in:
   packages/loxley-contracts/contracts/verifiers/bfv/honk/
```
