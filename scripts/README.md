...
This directory contains utility scripts for the Bracken project.
...
- `--skip-utils-patch` - Skip rewriting `BFV_DKG_H` / `BFV_THRESHOLD_T` in
  `packages/bracken-contracts/scripts/utils.ts`
...
The generated `.sol` files under `packages/bracken-contracts/contracts/verifiers/bfv/honk/` are
...
6. **Outputs / verifies** at `packages/bracken-contracts/contracts/verifiers/bfv/honk/`:
...
4. Commit the resulting diff under `packages/bracken-contracts/contracts/verifiers/bfv/honk/`.
...
✅ Generated 2 Solidity verifier(s) in:
   packages/bracken-contracts/contracts/verifiers/bfv/honk/
```
