...
echo "evm"
(cd "${REPO_ROOT}/packages/loxley-contracts" && pnpm compile:contracts)
...
if [[ "$CRISP_SKIP_PROOF_AGGREGATION" == "true" ]]; then
  LOXLEY_FEATURES="--features test-only-skip-proof-aggregation"
fi
echo "Building and installing loxley CLI (${LOXLEY_FEATURES:-no extra features})..."
# Always reinstall: `-f` makes `cargo install` replace in place unconditionally, so a stale binary
# from an earlier checkout (or an earlier profile) cannot silently survive. `--bin loxley`
# matches ci.yml and deploy/local/start.sh, and keeps this pinned to one binary if crates/cli
# ever grows a second.
# shellcheck disable=SC2086
(cd "${REPO_ROOT}" && cargo install --locked --path crates/cli --bin loxley -f $LOXLEY_FEATURES)
