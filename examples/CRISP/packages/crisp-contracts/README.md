...
### CRISP-only deploy (Bracken already deployed)

```bash
pnpm deploy:contracts          # production RISC0 verifier
pnpm deploy:contracts:full     # also deploy Bracken stack (no ZK unless ENABLE_ZK_VERIFICATION=true)
```

## CRISP Program

This is the main logic of CRISP - an bracken program for secure voting.

It exposes two main functions:

- `validate` - that is called when a new E3 instance is requested on Bracken (`Bracken.request`).
- `verify` - that is called when the ciphertext output is published on Bracken
  (`Bracken.publishCiphertextOutput`). This function ensures that the ciphertext output is valid.
...
- `validateInput` - validate the input data that is submitted to the E3 instance. It is called by
  the Bracken contract when a new input is published (`Bracken.publishInput`). In CRISP, the data
  ...
  [Greco](https://github.com/thecryptodog12-rgb/bracken/tree/main/circuits/crates/libs/greco). See the Greco
