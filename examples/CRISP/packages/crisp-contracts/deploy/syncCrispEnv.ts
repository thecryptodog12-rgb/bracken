...
import { readDeploymentArgs } from '@bracken/contracts/scripts'
...
export function syncCrispEnvFromDeployments(chain: string): void {
  const brackenAddress = deploymentAddress('Bracken', chain)
  ...
  if (!brackenAddress) missing.push('Bracken')
  ...
  const serverUpdates: Record<string, string> = {
    BRACKEN_ADDRESS: brackenAddress!,
