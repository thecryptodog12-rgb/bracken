...
import { IDecryptionVerifier } from "@loxley/contracts/contracts/interfaces/IDecryptionVerifier.sol";
import { IPkVerifier } from "@loxley/contracts/contracts/interfaces/IPkVerifier.sol";
...
contract MockLoxley {
  ...
  function requestWithParams(address program, uint256 numOptions, bytes memory params) external {
    e3s[nextE3Id] = E3({
      seed: 0,
      committeeSize: ILoxley.CommitteeSize.Minimum,
      requestBlock: 0,
  ...
  function getE3Stage(uint256) external view returns (ILoxley.E3Stage) {
