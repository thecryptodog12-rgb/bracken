...
use e3_sdk::evm_helpers::contracts::{
    CommitteeSize, BrackenContract, BrackenRead, BrackenWrite,
};
    ...
    if msg.contains(INSUFFICIENT_CIPHERNODES_SELECTOR) {
        ...
        return anyhow!(
            "request_e3 reverted: InsufficientCiphernodes — CommitteeSize::{label} (e3_committee_size={size}) \
             ...
             nodes are active. Default dev config uses Minimum (N=3, cn1–cn3 in bracken.config.yaml).",
            label = label,
...
pub async fn initialize_crisp_round(
    ...
    let contract = BrackenContract::new(
        &CONFIG.http_rpc_url,
        &CONFIG.private_key,
    ...
    info!("Requesting E3 on contract: {}", CONFIG.bracken_address);
