 ...
 * actually points at. The client already knows the Bracken address from the round state, and a
 * round names its own program, so no extra configuration reaches the browser.
 *
 * @param client The public client.
 * @param brackenAddress Bracken contract for this round.
 * @param e3Id The round.
 * @returns The CRISP program address.
 */
export const getCrispProgramAddress = async (client: PublicClient, brackenAddress: Address, e3Id: bigint): Promise<Address> => {
  const e3 = await client.readContract({
    address: brackenAddress,
    abi: BRACKEN_ABI,
    functionName: 'getE3',
    args: [e3Id],
  })
